...
public partial class run_effective_trait_set_regression : SceneTree
{
    ...
    {
        TraitDef sharp = null;
        TraitDef guarded = null;
        EffectiveTraitSet set = null;
        Godot.Collections.Array<BattleEffectiveTraitInstanceState> projection = null;
        try
        {
            sharp = BuildTrait("sharp_edge", "equipment_roll");
            guarded = BuildTrait("guarded_grip", "equipment_fixed");
            set = new(
            ...
            projection = set.ToBattleEffectiveInstances();
        ...
        {
            DisposeEffectiveTraitSetInputs(set);
            DisposeBattleProjection(projection);
            BattleTestFixture.DisposeTrait(sharp);
            BattleTestFixture.DisposeTrait(guarded);
        }
    }
    ...
    private static void DisposeEffectiveTraitSetInputs(EffectiveTraitSet set)
    {
        if (set == null)
            return;
        foreach (EffectiveTraitInstance instance in set.Instances)
        {
            if (instance?.RollValues == null)
                continue;
            foreach (TraitRollValueState rollValue in instance.RollValues)
                GodotSharpCleanup.DisposeGodotObject(rollValue);
            instance.RollValues.Clear();
        }
    }

    private static void DisposeBattleProjection(
        Godot.Collections.Array<BattleEffectiveTraitInstanceState> projection
    )
    {
        if (projection == null)
            return;
        foreach (BattleEffectiveTraitInstanceState traitInstance in projection)
            BattleTestFixture.DisposeBattleEffectiveTraitInstance(traitInstance);
        projection.Clear();
    }
