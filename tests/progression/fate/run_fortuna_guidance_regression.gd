...
const FortunaGuidanceService = preload("res://scripts/systems/battle/fate/FortunaGuidanceService.cs")
...
func _test_fortuna_guidance_unlock_chain_feeds_rank_2_to_5() -> void:
	...
	_assert_true(
		not _is_achievement_unlocked(party_state, &"fortuna_guidance_true"),
		"用于授予 fortune_marked 的第一次事件不应顺带解锁 guidance_true。"
	)

	var rank_1_result: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
	...
	var blocked_rank_2: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
	...
	_assert_true(_is_achievement_unlocked(party_state, &"fortuna_guidance_true"), "再次命中条件后应解锁 guidance_true。")
	_assert_true(party_state.pending_character_rewards.is_empty(), "guidance 成就本身不应排入额外 reward 队列。")

	var rank_2_result: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
	...
	var blocked_rank_3: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
	...
	_assert_true(devout_unlocks.has(&"fortuna_guidance_devout"), "低血+强 debuff 活下来并赢战后应解锁 guidance_devout。")
	_assert_true(_is_achievement_unlocked(party_state, &"fortuna_guidance_devout"), "campaign achievement 记录应保留 guidance_devout。")

	var rank_3_result: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
	...
	var blocked_rank_4: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
	...
	_assert_true(_is_achievement_unlocked(party_state, &"fortuna_guidance_exalted"), "高位威胁区大成功应解锁 guidance_exalted。")

	var rank_4_result: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
	...
	var blocked_rank_5: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
	...
	var chapter_unlocks: Array[StringName] = guidance.handle_chapter_completed({
		"chapter_id": &"chapter_01",
		"member_ids": [HERO_ID],
		"had_permanent_death": false,
	})
	_assert_true(chapter_unlocks.has(&"fortuna_guidance_blessed"), "章节无永久死亡且出现过 Fortuna 事件时应解锁 guidance_blessed。")
	_assert_true(_is_achievement_unlocked(party_state, &"fortuna_guidance_blessed"), "campaign achievement 记录应保留 guidance_blessed。")

	var rank_5_result: Dictionary = faith.execute_devotion(party_state, HERO_ID, FORTUNA_DEITY_ID)
...
func _build_member_state() -> PartyMemberState:
	...
	member_state.progression.unit_base_attributes.set_attribute_value(&"faith_luck_bonus", 0)
	return member_state
...
func _get_faith_luck_bonus(party_state: PartyState) -> int:
	return _get_custom_stat(party_state, &"faith_luck_bonus")
