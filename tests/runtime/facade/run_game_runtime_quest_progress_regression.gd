...
func _test_runtime_quest_commands_and_battle_progress_pipeline() -> void:
	...
	var growth_strength_before: int = int(
		growth_member_state.progression.unit_base_attributes.get_attribute_value(&"strength")
	)
	...
	_assert_eq(
		facade.get_party_state().get_member_state(growth_reward_member_id).progression.unit_base_attributes.get_attribute_value(&"strength"),
...
func _inject_string_key_only_quest_def(game_session) -> void:
	...
	game_session.install_test_content_def_string_key(&"quest", String(string_key_quest.quest_id), string_key_quest)
...
func _find_any_uncleared_encounter_anchor(world_data: Dictionary):
	for encounter_option in world_data.get("encounter_anchors", []):
		if encounter_option == null or bool(encounter_option.is_cleared):
			continue
		return encounter_option
	return null
...
func _build_reward_entries(reward_options: Array) -> Array[Dictionary]:
	var reward_entries: Array[Dictionary] = []
	for reward_option in reward_options:
		if reward_option is Dictionary:
			reward_entries.append((reward_option as Dictionary).duplicate(true))
