...
func _test_settlement_handler_routes_actions_and_modal_state() -> void:
	...
	_assert_true(QuestProviderContentRules.is_supported_provider_id(&"service_contract_board"), "任务板 provider 应来自共享 quest provider 白名单。")
	_assert_true(QuestProviderContentRules.is_supported_provider_id(&"service_bounty_registry"), "悬赏署 provider 应来自共享 quest provider 白名单。")
