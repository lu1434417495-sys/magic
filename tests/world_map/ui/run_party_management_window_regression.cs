...
public partial class run_party_management_window_regression : SceneTree
{
    ...
    private async Task TestWindowUsesHalfViewportWithMinimumSize()
    {
        ...
        partyState = BuildPartyState(new[] { new StringName("hero") });
        try
        {
            window.ShowParty(partyState);
            await ProcessFrames(1);
    ...
    private async Task TestMemberDetailsTolerateMissingSkillAndOccupiedSlots()
    {
        ...
        try
        {
            ...
            equipmentInstance = EquipmentInstanceState.CreateInstance(
                "iron_greatsword",
                "eq_party_window_001"
            );
            GodotRefCountedDisposer.DisposeIfValid(hero.equipment_state);
