...
public partial class run_progression_text_snapshot_regression : SceneTree
{
    ...
    private void Run()
    {
        ...
        {
            GD.Print("Progression text snapshot regression: PASS");
            Quit(0);
            return;
        }
        ...
        GD.Print($"Progression text snapshot regression: FAIL ({_failures.Count})");
        Quit(1);
    }

    private void TestPartyTextSnapshotRendersProgressionState()
    {
        string textSnapshot = GameTextSnapshotRenderer.render_full_snapshot(
            new GDictionary
            {
    ...
    private void AssertContains(string text, string fragment, string message)
    {
        if (text == null || !text.Contains(fragment))
        {
            _failures.Add($"{message} | missing={fragment} | text={text}");
        }
    }
}
