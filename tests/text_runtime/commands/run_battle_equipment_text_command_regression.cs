...
public partial class run_battle_equipment_text_command_regression : SceneTree
{
    ...
    public override void _Initialize()
    {
        ...
        AssertBattleEquipRequirementFailure(requirementResult.snapshot, requirementResult.snapshot_text);
        ...
        AssertBattleDuplicateExplicitEquip(duplicateExplicitResult.snapshot);
        ...
        AssertBattleDuplicateUnequipRoundTrip(duplicateUnequipResult.snapshot, runner);

        PrimeActiveUnitAp(runner, 4);
        GameTextCommandResult equipResult = RunCommand(runner, "battle equip head leather_cap");
        AssertSuccessfulBattleEquip(equipResult.snapshot, equipResult.snapshot_text);
        ...
        AssertBattleEquipApFailure(apResult.snapshot, apResult.snapshot_text);

        PrimeActiveUnitAp(runner, 2);
        string otherUnitId = FindOtherBattleUnitId(runner.GetSession().BuildSnapshot());
        GameTextCommandResult targetResult = RunCommandExpectFail(
            runner,
            $"battle equip main_hand bronze_sword target_unit_id={otherUnitId}"
        );
        AssertBattleEquipSelfOnlyFailure(targetResult.snapshot, targetResult.snapshot_text);

        RunCommand(runner, "warehouse capacity 1");
        PrimeActiveUnitAp(runner, 2);
        GameTextCommandResult fullResult = RunCommandExpectFail(runner, "battle unequip head");
        AssertBattleUnequipBackpackFullFailure(fullResult.snapshot, fullResult.snapshot_text);
        ...
        AssertBattleTwoHandedWeaponLinkage(twoHandedResult.snapshot);
        ...
        AssertBattleVersatileFreeOffhandLinkage(versatileResult.snapshot);
        ...
        AssertBattleOffhandForcesVersatileOneHanded(offhandResult.snapshot);
        AssertPartyEquipmentNotUpdatedDuringBattle(offhandResult.snapshot, activeMemberId);
        ...
        AssertBattleBodyArmorEquippedWithoutHpBonus(bodyArmorEquipResult.snapshot);
        GDictionary bodyArmorEquipReport = FindLatestChangeEquipmentReport(
            bodyArmorEquipResult.snapshot
        );
        ...
        AssertBattleUnequipBodyArmorTurnEndWithoutHpClamp(
            bodyArmorUnequipResult.snapshot,
    ...
    private void AdvanceToManualBattleTurn(GameTextCommandRunner runner, int maxTicks = 64)
    {
        for (int index = 0; index < maxTicks; index++)
        {
            ...
            if (!DictBool(battleSnapshot, "active", false))
                break;
    ...
    private void AssertSuccessfulBattleEquip(GDictionary snapshot, string textSnapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
        ...
        GDictionary unit = FindBattleUnit(Dict(snapshot, "battle"), DictString(report, "unit_id"));
        GDictionary equipped = FindEquippedEntry(ArrayValue(unit, "equipment"), "head");
    ...
    private void AssertBattleEquipApFailure(GDictionary snapshot, string textSnapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
    ...
    private void AssertBattleEquipSelfOnlyFailure(GDictionary snapshot, string textSnapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
    ...
    private void AssertBattleEquipRequirementFailure(GDictionary snapshot, string textSnapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
        ...
        GDictionary hudEntry = FindBattleHudBackpackEntry(snapshot, RestrictedTestHelmId.ToString());
    ...
    private void AssertBattleEquipStringKeyOnlyItemNotFound(GDictionary snapshot, string textSnapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
    ...
    private void AssertBattleDuplicateItemOnlyRequiresInstance(GameTextCommandResult result)
    {
        ...
        _test.Eq(CountBattleBackpackItem(result.snapshot, DuplicateTestCharmId.ToString()), 2, "item_id-only 失败后两个重复实例都应留在 battle-local 背包。");
    }

    private void AssertBattleDuplicateExplicitEquip(GDictionary snapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
        ...
        GDictionary unit = FindBattleUnit(Dict(snapshot, "battle"), DictString(report, "unit_id"));
        GDictionary equipped = FindEquippedEntry(ArrayValue(unit, "equipment"), "necklace");
    ...
    private void AssertBattleUnequipBackpackFullFailure(GDictionary snapshot, string textSnapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
    ...
    private void AssertBattleTwoHandedWeaponLinkage(GDictionary snapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
        ...
        GDictionary unit = FindBattleUnit(Dict(snapshot, "battle"), DictString(report, "unit_id"));
        GDictionary mainEntry = FindEquippedEntry(ArrayValue(unit, "equipment"), "main_hand");
    ...
    private void AssertBattleVersatileFreeOffhandLinkage(GDictionary snapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
        ...
        GDictionary unit = FindBattleUnit(Dict(snapshot, "battle"), DictString(report, "unit_id"));
        GDictionary mainEntry = FindEquippedEntry(ArrayValue(unit, "equipment"), "main_hand");
        GDictionary offhandEntry = FindEquippedEntry(ArrayValue(unit, "equipment"), "off_hand");
    ...
    private void AssertBattleOffhandForcesVersatileOneHanded(GDictionary snapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
    ...
    private void AssertPartyEquipmentNotUpdatedDuringBattle(GDictionary snapshot, string memberId)
    {
        GDictionary member = FindPartyMember(ArrayValue(Dict(snapshot, "party"), "members"), memberId);
        _test.True(member.Count > 0, "战斗中 party 快照应包含当前行动成员。");
        GDictionary partyMain = FindEquippedEntry(ArrayValue(member, "equipment"), "main_hand");
        GDictionary partyOffhand = FindEquippedEntry(ArrayValue(member, "equipment"), "off_hand");
    ...
    private void AssertPartyEquipmentWrittenBackAfterBattle(GDictionary snapshot, string memberId)
    {
        ...
        GDictionary mainEntry = FindEquippedEntry(ArrayValue(member, "equipment"), "main_hand");
        GDictionary offhandEntry = FindEquippedEntry(ArrayValue(member, "equipment"), "off_hand");
        GDictionary headEntry = FindEquippedEntry(ArrayValue(member, "equipment"), "head");
        GDictionary bodyEntry = FindEquippedEntry(ArrayValue(member, "equipment"), "body");
    ...
    private void AssertBattleBodyArmorEquippedWithoutHpBonus(GDictionary snapshot)
    {
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
    ...
    {
        GDictionary battleSnapshot = Dict(snapshot, "battle");
        GDictionary report = FindLatestChangeEquipmentReport(snapshot);
        ...
        GDictionary unit = FindBattleUnit(battleSnapshot, DictString(report, "unit_id"));
    ...
    private static GDictionary FindLatestChangeEquipmentReport(GDictionary snapshot)
    {
        GArray reports = ArrayValue(Dict(snapshot, "battle"), "report_entries");
        for (int index = reports.Count - 1; index >= 0; index--)
        {
            GDictionary report = reports[index].AsGodotDictionary();
    ...
    {
        foreach (Variant unitValue in ArrayValue(battleSnapshot, "units"))
        {
    ...
    private static GDictionary FindBattleHudBackpackEntry(GDictionary snapshot, string itemId)
    {
        GDictionary equipmentPanel = Dict(Dict(Dict(snapshot, "battle"), "hud"), "equipment_panel");
        foreach (Variant entryValue in ArrayValue(equipmentPanel, "backpack_entries"))
        {
    ...
    private static string FindOtherBattleUnitId(GDictionary snapshot)
    {
        GDictionary battleSnapshot = Dict(snapshot, "battle");
        string activeUnitId = DictString(battleSnapshot, "active_unit_id");
        foreach (Variant unitValue in ArrayValue(battleSnapshot, "units"))
    ...
    private static GDictionary FindPartyMember(GArray entries, string memberId)
    {
    ...
    private static GDictionary FindEquippedEntry(GArray entries, string slotId)
    {
        foreach (Variant entryValue in entries)
        {
            GDictionary entry = entryValue.AsGodotDictionary();
    ...
    private static bool ArrayHasString(Variant value, string expected)
    {
        if (value.VariantType != Variant.Type.Array)
            return false;
        foreach (Variant entry in value.AsGodotArray())
        {
            if (entry.AsString() == expected)
                return true;
        }
        return false;
    }

    private static int CountBattleBackpackItem(GDictionary snapshot, string itemId)
    {
        GDictionary backpack = Dict(Dict(snapshot, "battle"), "party_backpack");
        int total = 0;
        foreach (Variant stackValue in ArrayValue(backpack, "stacks"))
        {
            GDictionary stack = stackValue.AsGodotDictionary();
            if (DictString(stack, "item_id") == itemId)
                total += DictInt(stack, "quantity");
        }
        foreach (Variant instanceValue in ArrayValue(backpack, "equipment_instances"))
        {
            GDictionary instance = instanceValue.AsGodotDictionary();
