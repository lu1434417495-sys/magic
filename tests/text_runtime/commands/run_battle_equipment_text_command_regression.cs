...
public partial class run_battle_equipment_text_command_regression : SceneTree
{
    ...
    public override void _Initialize()
    {
        ...
        try
        {
            ...
            RunCommandAndDispose(runner, "game new test");
            RunCommandAndDispose(runner, "warehouse capacity 10");
            RunCommandAndDispose(runner, "warehouse add bronze_sword 1");
            RunCommandAndDispose(runner, "warehouse add leather_cap 1");
            RunCommandAndDispose(runner, "warehouse add leather_jerkin 1");
            RunCommandAndDispose(runner, "warehouse add iron_greatsword 1");
            RunCommandAndDispose(runner, $"warehouse add {VersatileTestWeaponId} 1");
            RunCommandAndDispose(runner, $"warehouse add {OffhandTestItemId} 1");
            RunCommandAndDispose(runner, $"warehouse add {RestrictedTestHelmId} 1");
            RunCommandAndDispose(runner, "battle start settlement");
            RunCommandAndDispose(runner, "battle confirm");
            ...
            using GameTextCommandResult stringKeyResult = RunCommandExpectFail(
                runner,
            ...
            PrimeActiveUnitAp(runner, 2);
            using GameTextCommandResult requirementResult = RunCommandExpectFail(
                runner,
            ...
            using GameTextCommandResult duplicateItemOnlyResult = RunCommandExpectFail(
                runner,
            ...
            PrimeActiveUnitAp(runner, 4);
            using GameTextCommandResult duplicateExplicitResult = RunCommand(
                runner,
            ...
            PrimeActiveUnitAp(runner, 4);
            using GameTextCommandResult duplicateUnequipResult = RunCommand(
                runner,
            ...
            PrimeActiveUnitAp(runner, 4);
            using GameTextCommandResult equipResult = RunCommand(runner, "battle equip head leather_cap");
            AssertSuccessfulBattleEquip(equipResult.SnapshotTyped, equipResult.snapshot_text);

            PrimeActiveUnitAp(runner, 1);
            using GameTextCommandResult apResult = RunCommandExpectFail(
                runner,
            ...
            PrimeActiveUnitAp(runner, 2);
            IReadOnlyDictionary<string, object> targetSnapshot = runner.GetSession().BuildSnapshotTyped();
            string otherUnitId = FindOtherBattleUnitId(targetSnapshot);
            using GameTextCommandResult targetResult = RunCommandExpectFail(
                runner,
            ...
            RunCommandAndDispose(runner, "warehouse capacity 1");
            PrimeActiveUnitAp(runner, 2);
            using GameTextCommandResult fullResult = RunCommandExpectFail(runner, "battle unequip head");
            AssertBattleUnequipBackpackFullFailure(fullResult.SnapshotTyped, fullResult.snapshot_text);

            RunCommandAndDispose(runner, "warehouse capacity 10");
            PrimeActiveUnitAp(runner, 6);
            using GameTextCommandResult twoHandedResult = RunCommand(
                runner,
            ...
            PrimeActiveUnitAp(runner, 6);
            using GameTextCommandResult versatileResult = RunCommand(
                runner,
            ...
            PrimeActiveUnitAp(runner, 6);
            using GameTextCommandResult offhandResult = RunCommand(
                runner,
            ...
            PrimeActiveUnitAp(runner, 4);
            using GameTextCommandResult bodyArmorEquipResult = RunCommand(
                runner,
            ...
            IReadOnlyDictionary<string, object> bodyArmorEquipReport =
                FindLatestChangeEquipmentReport(bodyArmorEquipResult.SnapshotTyped);
            PrimeActiveUnitHpAndAp(runner, DictInt(bodyArmorEquipReport, "hp_max_after"), 2);
            using GameTextCommandResult bodyArmorUnequipResult = RunCommand(runner, "battle unequip body");
            ...
            using GameTextCommandResult finishResult = RunCommand(runner, "battle finish player");
    ...
    private void AdvanceToManualBattleTurn(GameTextCommandRunner runner, int maxTicks = 64)
    {
        ...
        {
            IReadOnlyDictionary<string, object> snapshot = runner.GetSession().BuildSnapshotTyped();
            ...
            if (DictString(activeUnit, "control_mode") == "manual")
                return;
            RunCommandAndDispose(runner, "battle tick 1");
        }
    ...
    private void AssertBattleDuplicateExplicitEquip(IReadOnlyDictionary<string, object> snapshot)
    {
        IReadOnlyDictionary<string, object> report = FindLatestChangeEquipmentReport(snapshot);
    ...
    private void AssertBattleTwoHandedWeaponLinkage(IReadOnlyDictionary<string, object> snapshot)
    {
        IReadOnlyDictionary<string, object> report = FindLatestChangeEquipmentReport(snapshot);
        ...
        _test.True(ArrayHasString(mainEntry, "occupied_slot_ids", "off_hand"), "双手武器 battle-local 装备 entry 应联动占用副手。");
    }
    ...
    private static string FindOtherBattleUnitId(IReadOnlyDictionary<string, object> snapshot)
    {
        IReadOnlyDictionary<string, object> battleSnapshot = Dict(snapshot, "battle");
        string activeUnitId = DictString(battleSnapshot, "active_unit_id");
        foreach (object unitValue in ArrayValue(battleSnapshot, "units"))
    ...
    private static bool ArrayHasString(
    ...
    )
    {
        if (
            ...
            || rawValue is not IReadOnlyList<object> values
        )
            return false;
        foreach (object entry in values)
        {
            if (ValueString(entry) == expected)
                return true;
        }
        return false;
    }
    ...
    private GameTextCommandResult RunCommand(GameTextCommandRunner runner, string commandText)
    {
        ...
        if (!result.ok)
        {
            Console.Out.WriteLine(result.Render());
            _test.Fail($"命令失败：{commandText} | {result.message}");
        }
        return result;
    }

    private void RunCommandAndDispose(GameTextCommandRunner runner, string commandText)
    {
        using GameTextCommandResult result = RunCommand(runner, commandText);
    }
