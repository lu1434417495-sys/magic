...
const DETERMINISTIC_BATTLE_DAMAGE_RESOLVER_SCRIPT = preload("res://tests/battle_runtime/helpers/DeterministicBattleDamageResolver.cs")
...
func _run() -> void:
	...
	_test_black_contract_push_options_pay_their_selected_cost_and_force_hit_without_crit()
	_test_doom_shift_marks_self_and_swaps_with_nearby_ally()
	_test_black_crown_seal_is_boss_only_once_per_battle_and_applies_both_lock_options()
...
func _test_misstep_to_scheme_grants_bonus_calamity_without_duplicate_critical_fail_events() -> void:
	...
	var fate_event_bus: BattleFateEventBus = runtime.get_fate_event_bus()
	var seen_events: Array[StringName] = []
	var event_callback: Callable = func(event_type: StringName, _payload: Dictionary) -> void:
		seen_events.append(event_type)
	fate_event_bus.connect(&"EventDispatched", event_callback)
	fate_event_bus.dispatch(
		&"critical_fail",
	...
	_assert_eq(
		_seen_event_count(seen_events, &"critical_fail"),
		1,
	...
	_assert_true(
		triggered_event_ids.has(&"borrowed_road"),
		"失手成筹不应冲掉 Borrowed Road 的大失败计数。"
	)
	if fate_event_bus != null and fate_event_bus.is_connected(&"EventDispatched", event_callback):
		fate_event_bus.disconnect(&"EventDispatched", event_callback)
	low_luck_service.dispose()
	low_luck_context.clear()
	runtime.dispose()
...
func _test_normal_attack_skill_critical_fail_keeps_fate_report_and_event() -> void:
	...
	caster.attribute_snapshot.set_value(&"hidden_luck_at_birth", -5)
	...
	var seen_events: Array[StringName] = []
	runtime.get_fate_event_bus().connect(&"EventDispatched", func(event_type: StringName, _payload: Dictionary) -> void:
		seen_events.append(event_type)
	)
	...
	_assert_true(
		_has_fate_attack_report_entry_with_tag(batch.report_entries, &"critical_fail"),
...
func _test_multi_unit_damage_skill_routes_through_fate_attack_resolution() -> void:
	...
	first_enemy.attribute_snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.ARMOR_CLASS_ID(), 0)
	var second_enemy := _build_unit(&"multi_route_enemy_b", "后排敌人", &"enemy", Vector2i(4, 1), 1)
	second_enemy.attribute_snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.ARMOR_CLASS_ID(), 0)
	_add_unit(runtime, state, caster)
	...
	_assert_true(
		_seen_event_count(seen_events, &"critical_fail") > 0
			or _seen_event_count(seen_events, &"ordinary_miss") > 0,
...
func _test_ground_variant_multi_unit_skill_routes_through_fate_attack_resolution() -> void:
	...
	caster.attribute_snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.STAMINA_MAX_ID(), 60)
	...
	third_enemy.attribute_snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.ARMOR_CLASS_ID(), 0)
	_add_unit(runtime, state, caster)
	_add_unit(runtime, state, first_enemy)
	_add_unit(runtime, state, second_enemy)
...
func _test_black_contract_push_options_pay_their_selected_cost_and_force_hit_without_crit() -> void:
...
func _test_doom_shift_marks_self_and_swaps_with_nearby_ally() -> void:
	...
	var origin_coord: Vector2i = caster.coord
	var ally_coord: Vector2i = ally.coord
...
func _test_black_crown_seal_is_boss_only_once_per_battle_and_applies_both_lock_options() -> void:
...
func _build_unit(
	...
	unit.attribute_snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.AURA_MAX_ID(), 4)
...
func _has_fate_attack_report_entry(report_entries_option) -> bool:
	for report_entry_option in report_entries_option:
		if report_entry_option is not Dictionary:
			continue
		var report_entry := report_entry_option as Dictionary
		if String(report_entry.get("entry_type", "")) == "fate_attack_resolution":
			return true
	return false


func _has_fate_attack_report_entry_with_tag(report_entries_option, event_tag: StringName) -> bool:
	for report_entry_option in report_entries_option:
		if report_entry_option is not Dictionary:
			continue
		var report_entry := report_entry_option as Dictionary
		if String(report_entry.get("entry_type", "")) != "fate_attack_resolution":
			continue
		var event_tags_option = report_entry.get("event_tags", [])
		if event_tags_option is Array and (event_tags_option as Array).has(String(event_tag)):
			return true
	return false
