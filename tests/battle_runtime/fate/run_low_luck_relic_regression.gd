...
const GAME_RUNTIME_SETTLEMENT_COMMAND_HANDLER_SCRIPT = preload("res://scripts/systems/game_runtime/GameRuntimeSettlementCommandHandler.cs")
const ITEM_CONTENT_REGISTRY_SCRIPT = preload("res://scripts/player/warehouse/ItemContentRegistry.cs")
const LOW_LUCK_EVENT_SERVICE_SCRIPT = preload("res://scripts/systems/battle/fate/LowLuckEventService.cs")
const LOW_LUCK_RELIC_RULES_SCRIPT = preload("res://scripts/systems/fate/LowLuckRelicRules.cs")
const MISFORTUNE_BLACK_OMEN_SERVICE_SCRIPT = preload("res://scripts/systems/progression/MisfortuneBlackOmenService.cs")
...
const PROGRESSION_DATA_UTILS_SCRIPT = preload("res://scripts/player/progression/ProgressionDataUtils.cs")
...
func _test_dead_road_lantern_reveals_hidden_paths_and_grants_black_omen_mark() -> void:
	...
	member_state.equipment_state.set_equipped_entry(
		"special_trinket",
		LowLuckRelicRules.item_dead_road_lantern(),
		_build_slot_array("special_trinket"),
	...
	var black_omen_result = black_omen_service.try_run_hook(
		MisfortuneBlackOmenService.HOOK_DEAD_ROAD_LANTERN_BLACK_OMEN_PATH_VALUE(),
		{
			"member_id": HERO_ID,
	...
	_assert_eq(
		member_state.progression.unit_base_attributes.get_attribute_value(MisfortuneBlackOmenService.DOOM_MARKED_STAT_ID_VALUE()),
		1,
...
func _build_equipped_member_snapshot(item_defs: Dictionary, item_id: StringName):
	...
	if item_id == LowLuckRelicRules.item_dead_road_lantern():
		slot_id = "special_trinket"
	elif item_id == LowLuckRelicRules.item_blood_debt_shawl():
		slot_id = "cloak"
	elif item_id == LowLuckRelicRules.item_reverse_fate_amulet():
		slot_id = "necklace"
	elif item_id == LowLuckRelicRules.item_black_star_wedge():
		slot_id = "badge"
	else:
		slot_id = "necklace"
	member_state.equipment_state.set_equipped_entry(
...
func _build_battle_unit(
	...
	unit.attribute_snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.HP_MAX_ID(), hp_max)
	unit.attribute_snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.MP_MAX_ID(), 30)
...
func _extract_first_damage_event(result: Dictionary) -> Dictionary:
	var damage_events_option: Variant = result.get("damage_events", [])
	if damage_events_option is not Array or (damage_events_option as Array).is_empty():
		return {}
	if (damage_events_option as Array)[0] is Dictionary:
		return (damage_events_option as Array)[0] as Dictionary
	return {}


func _assert_fixed_loot_entry(loot_entries_option: Variant, item_id: StringName, message: String) -> void:
	var found_entry = _find_loot_entry(loot_entries_option, item_id)
...
func _find_loot_entry(loot_entries_option: Variant, item_id: StringName) -> Dictionary:
	if loot_entries_option is not Array:
		return {}
	for loot_entry_option in loot_entries_option:
