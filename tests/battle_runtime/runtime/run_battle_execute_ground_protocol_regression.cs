...
public partial class run_battle_execute_ground_protocol_regression : LifecycleTestSceneTree
{
    ...
    private void TestGroundExecutePreviewDenied()
    {
        ...
        BattleCommand command = MakeGroundCommand(source, target.GetAnchorCoord(), skill);
        BattlePreview preview = fixture.Runtime.PreviewCommand(command);

        _test.False(preview.allowed, "地面技能携带 execute 时 preview 应直接拒绝。");
        _test.Eq(target.GetCurrentHp(), 10, "地面 execute preview 不应伤害目标。");
    ...
    private void TestGroundExecuteIssueDeniedBeforeCost()
    {
        ...
        _test.Eq(source.GetCurrentAp(), 2, "地面 execute 被拒绝时不应消耗 AP。");
        _test.Eq(source.GetCurrentMp(), 20, "地面 execute 被拒绝时不应消耗 MP。");
    ...
    private void TestGroundExecuteDoesNotMutateDamageOrStatus()
    {
        ...
        _test.True(target.IsAlive(), "地面 execute 被拒绝时不应杀死目标。");
        _test.Eq(target.GetCurrentHp(), 10, "地面 execute 被拒绝时不应改变目标 HP。");
    ...
    )
    {
        ...
        unit.AddKnownActiveSkill("test_ground_execute");
        unit.SetKnownSkillLevelTyped("test_ground_execute", 17);
        return unit;
    }
