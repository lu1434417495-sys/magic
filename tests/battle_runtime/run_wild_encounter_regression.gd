...
func _test_game_runtime_facade_battle_overflow_feedback_surfaces_in_message_and_snapshot() -> void:
	...
	_assert_eq(String(loot_snapshot.get("loot_summary_text", "")), "%s x2" % beast_hide_label, "headless snapshot 应暴露本次战斗的 loot 摘要。")
	_assert_eq(String(loot_snapshot.get("overflow_summary_text", "")), "%s x2" % beast_hide_label, "headless snapshot 应暴露本次战斗的 overflow 摘要。")
	...
	_assert_true(text_snapshot.find("loot_summary=%s x2" % beast_hide_label) >= 0, "text snapshot 应渲染本次战斗 loot 摘要。")
	_assert_true(text_snapshot.find("overflow_summary=%s x2" % beast_hide_label) >= 0, "text snapshot 应渲染本次战斗 overflow 摘要。")
