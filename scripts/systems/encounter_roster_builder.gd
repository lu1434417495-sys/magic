...
const BEAST_ATTRIBUTE_DICE_COUNT := 5
...
func _build_enemy_snapshot_from_template(
	...
	for attribute_id in UNIT_BASE_ATTRIBUTES_SCRIPT.BASE_ATTRIBUTE_IDS:
		snapshot.set_value(attribute_id, int(base_attributes.get(attribute_id, BEAST_ATTRIBUTE_DICE_FLOOR)))
	...
	snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.ATTACK_BONUS, int(stats.get("attack_bonus", 4)))
	snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.ARMOR_CLASS, int(stats.get("armor_class", 12)))
	snapshot.set_value(ATTRIBUTE_SERVICE_SCRIPT.ARMOR_AC_BONUS, int(stats.get("armor_ac_bonus", 0)))
...
func _resolve_enemy_base_attributes(
	...
	if template != null and template.has_method("has_tag") and template.has_tag(ENEMY_TEMPLATE_DEF_SCRIPT.TAG_BEAST):
		return _build_beast_base_attributes(template, encounter_anchor, unit_index, build_context)
	...
	for attribute_id in UNIT_BASE_ATTRIBUTES_SCRIPT.BASE_ATTRIBUTE_IDS:
		resolved[attribute_id] = maxi(int(configured.get(attribute_id, BEAST_ATTRIBUTE_DICE_FLOOR)), BEAST_ATTRIBUTE_DICE_FLOOR)
	return resolved


func _build_beast_base_attributes(
	template,
	encounter_anchor,
	unit_index: int,
	build_context: Dictionary
	...
	# Beast six attributes must be stable for the same spawned unit and battle seed.
	rng.seed = _build_beast_attribute_seed(template, encounter_anchor, unit_index, build_context)
	var rolled_attributes: Dictionary = {}
	for attribute_id in UNIT_BASE_ATTRIBUTES_SCRIPT.BASE_ATTRIBUTE_IDS:
		rolled_attributes[attribute_id] = _roll_beast_base_attribute(rng)
	return rolled_attributes


func _build_beast_attribute_seed(
	template,
	encounter_anchor,
	unit_index: int,
	build_context: Dictionary
	...
	var anchor_id := String(encounter_anchor.entity_id) if encounter_anchor != null else "wild"
	var template_id := String(template.template_id) if template != null else "enemy"
	var battle_seed := int(build_context.get("battle_seed", 0))
	var growth_stage := int(build_context.get(
		"growth_stage",
		encounter_anchor.growth_stage if encounter_anchor != null else 0
	))
	return ("%s:%s:%d:%d:%d" % [
...
func _roll_beast_base_attribute(rng: RandomNumberGenerator) -> int:
	var total := BEAST_ATTRIBUTE_DICE_OFFSET
	for _i in BEAST_ATTRIBUTE_DICE_COUNT:
		total += rng.randi_range(1, BEAST_ATTRIBUTE_DICE_SIDES)
	return maxi(total, BEAST_ATTRIBUTE_DICE_FLOOR)
