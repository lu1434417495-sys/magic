...
public partial class ProfessionAssignmentService : RefCounted
{
    ...
    public bool can_assign_core_skill_to_profession(StringName skill_id, StringName profession_id)
    {
        ...
        if (!GdInterop.GetBool(skillProgress, "is_core"))
            return false;
        if (!SkillEffectiveMaxLevelRules.is_at_effective_max_level(skillDef, Variant.From(skillProgress), _unit_progress))
            return false;

        StringName assignedProfessionId = GdInterop.GetStringName(skillProgress, "assigned_profession_id");
    ...
    public bool remove_core_skill_from_profession(StringName skill_id, StringName profession_id)
    {
        ...
        GodotObject skillProgress = GetSkillProgress(skill_id);
        if (skillProgress != null && GdInterop.GetStringName(skillProgress, "assigned_profession_id") == profession_id)
    ...
    public bool can_promote_non_core_to_core(StringName skill_id, StringName profession_id)
    {
        ...
        GodotObject skillProgress = GetSkillProgress(skill_id);
        SkillDef skillDef = GetSkillDef(skill_id);
        if (professionProgress == null || professionDef == null || skillProgress == null || skillDef == null)
            return false;

        int currentCharacterLevel = GetEffectiveCharacterLevel();
        _unit_progress.Call("sync_active_core_skill_ids");
        if (GdInterop.GetArray(_unit_progress, "active_core_skill_ids").Count >= currentCharacterLevel)
            return false;
        if (GdInterop.GetArray(professionProgress, "core_skill_ids").Count >= GdInterop.GetInt(professionProgress, "rank"))
            return false;
        ...
        foreach (Variant rawTag in skillDef.tags)
        {
    ...
    public Godot.Collections.Array<StringName> get_profession_core_skills(StringName profession_id)
    {
        ...
        foreach (Variant rawSkillId in GdInterop.GetArray(professionProgress, "core_skill_ids"))
        {
    ...
    {
        GodotObject skillProgress = GetSkillProgress(skill_id);
        return skillProgress == null ? new StringName("") : GdInterop.GetStringName(skillProgress, "assigned_profession_id");
    }
    ...
    private static GDictionary IndexSkillDefs(Variant skillDefs)
    {
        ...
        {
            GDictionary defs = skillDefs.AsGodotDictionary();
            foreach (Variant key in defs.Keys)
            {
                if (defs[key].AsGodotObject() is not SkillDef skillDef)
                    continue;
                StringName indexedId = !GdInterop.IsEmpty(skillDef.skill_id) ? skillDef.skill_id : ProgressionDataUtils.to_string_name(key);
                indexedDefs[indexedId] = skillDef;
            }
        }
        else if (skillDefs.VariantType == Variant.Type.Array)
        {
            foreach (Variant rawSkillDef in skillDefs.AsGodotArray())
            {
                if (rawSkillDef.AsGodotObject() is SkillDef skillDef && !GdInterop.IsEmpty(skillDef.skill_id))
    ...
    private static GDictionary IndexProfessionDefs(Variant professionDefs)
    {
        ...
        {
            GDictionary defs = professionDefs.AsGodotDictionary();
            foreach (Variant key in defs.Keys)
            {
                if (defs[key].AsGodotObject() is not ProfessionDef professionDef)
                    continue;
                StringName indexedId = !GdInterop.IsEmpty(professionDef.profession_id) ? professionDef.profession_id : ProgressionDataUtils.to_string_name(key);
                indexedDefs[indexedId] = professionDef;
            }
        }
        else if (professionDefs.VariantType == Variant.Type.Array)
        {
            foreach (Variant rawProfessionDef in professionDefs.AsGodotArray())
            {
                if (rawProfessionDef.AsGodotObject() is ProfessionDef professionDef && !GdInterop.IsEmpty(professionDef.profession_id))
    ...
    private GodotObject GetSkillProgress(StringName skillId)
    {
        if (_unit_progress == null)
            return null;
        Variant result = _unit_progress.Call("get_skill_progress", skillId);
    ...
    private GodotObject GetProfessionProgress(StringName professionId)
    {
        if (_unit_progress == null)
            return null;
        Variant result = _unit_progress.Call("get_profession_progress", professionId);
        return result.VariantType == Variant.Type.Object ? result.AsGodotObject() : null;
    }
    ...
    private void RemoveSkillFromAllProfessions(StringName skillId, StringName exceptProfessionId = default)
    {
        if (_unit_progress == null)
            return;

        GDictionary professions = GdInterop.GetDictionary(_unit_progress, "professions");
        foreach (Variant professionKey in professions.Keys)
        {
    ...
    private static Godot.Collections.Array<StringName> GetProfessionAcceptedTags(ProfessionDef professionDef)
    {
        ...
        if (professionDef.unlock_requirement != null)
            AppendTagRules(acceptedTags, seenTags, professionDef.unlock_requirement.required_tag_rules);
        ...
        Godot.Collections.Array<TagRequirement> tagRules)
    {
        ...
        {
            if (tagRule == null || GdInterop.IsEmpty(tagRule.tag) || seenTags.ContainsKey(tagRule.tag))
                continue;
    ...
    private int GetEffectiveCharacterLevel()
    {
        ...
        int rankTotal = 0;
        foreach (Variant rawProfessionProgress in GdInterop.GetDictionary(_unit_progress, "professions").Values)
        {
            GodotObject professionProgress = rawProfessionProgress.VariantType == Variant.Type.Object ? rawProfessionProgress.AsGodotObject() : null;
            if (professionProgress != null)
