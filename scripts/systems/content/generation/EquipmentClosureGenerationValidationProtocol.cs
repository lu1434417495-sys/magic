...
internal static class EquipmentClosureGenerationValidationProtocol
{
    ...
    )
    {
        ...
        return WriteJson(indented: true, writer =>
        {
            ...
            foreach (EquipmentClosureGenerationValidationStageReport stage in report.Stages)
            {
                writer.WriteStartObject();
                WriteStageSummary(writer, stage);
                writer.WritePropertyName("diagnostics");
                writer.WriteStartArray();
                foreach (ContentJsonDiagnostic diagnostic in stage.Diagnostics)
                {
                    writer.WriteStartObject();
                    WriteDiagnostic(writer, stage.Stage, diagnostic);
    ...
    internal static string FormatNdjson(
        EquipmentClosureGenerationValidationReport report
    )
    {
        ArgumentNullException.ThrowIfNull(report);
        ...
        foreach (EquipmentClosureGenerationValidationStageReport stage in report.Stages)
        {
            foreach (ContentJsonDiagnostic diagnostic in stage.Diagnostics)
            {
                output.Append(WriteJson(indented: false, writer =>
                {
                    ...
                    writer.WriteString(
                        "protocol",
                        EquipmentClosureGenerationValidationReport.ProtocolId
                    );
                    WriteDiagnostic(writer, stage.Stage, diagnostic);
                    writer.WriteEndObject();
                }));
    ...
    )
    {
        ...
        if (report.RejectedStage.HasValue)
        {
            writer.WriteString(
                "rejected_stage",
                EquipmentClosureGenerationValidationStageCodec.ToWireValue(
    ...
    private static void WriteStageSummary(
        Utf8JsonWriter writer,
        EquipmentClosureGenerationValidationStageReport stage
    )
    {
        writer.WriteString(
            "stage",
            EquipmentClosureGenerationValidationStageCodec.ToWireValue(stage.Stage)
        );
    ...
    )
    {
        writer.WriteString(
            "stage",
            EquipmentClosureGenerationValidationStageCodec.ToWireValue(stage)
        );
