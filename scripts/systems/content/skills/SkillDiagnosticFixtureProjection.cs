...
/// <summary>
/// Test-only projection for synthetic validator inputs that intentionally bypass the production
/// JSON boundary. Production skill authoring loads JSON directly and never calls this helper.
/// </summary>
internal static partial class SkillDiagnosticFixtureProjection
{
    private const string InvalidFixtureRule = "skill.fixture.invalid_input";

    internal static ContentImportStageResult<SkillImportModel> TryProject(
    ...
    )
    {
        ...
        if (skill == null)
            return Failure(context, "Skill diagnostic fixture must not be null.", "");
        ...
        return SkillJsonImportParser.NormalizeDiagnosticFixture(context, dto);
    }

    internal static ContentImportStageResult<CombatSkillImportModel> TryProjectCombat(
        JsonContentEntryContext context,
        CombatSkillDef combat,
        StringName fallbackSkillId
    )
    {
        string rootId = Text(fallbackSkillId);
        if (!SkillJsonImportValueRules.IsSnakeCaseId(rootId))
            rootId = "skill_resource_projection";
        var root = new SkillDef
        {
            skill_id = rootId,
            display_name = rootId,
            combat_profile = combat,
        };
        ContentImportStageResult<SkillImportModel> result = TryProject(context, root);
        return result.HasValue && result.Value.CombatProfile != null
            ? ContentImportStageResult<CombatSkillImportModel>.Success(result.Value.CombatProfile)
            : ContentImportStageResult<CombatSkillImportModel>.Failure(result.Diagnostics);
    }

    internal static ContentImportStageResult<CombatEffectImportModel> TryProjectEffect(
        JsonContentEntryContext context,
        CombatEffectDef effect,
        StringName fallbackSkillId
    )
    {
        var combat = new CombatSkillDef { skill_id = fallbackSkillId };
        combat.effect_defs.Add(effect);
        ContentImportStageResult<CombatSkillImportModel> result =
            TryProjectCombat(context, combat, fallbackSkillId);
        return result.HasValue && result.Value.EffectDefs.Count == 1
            ? ContentImportStageResult<CombatEffectImportModel>.Success(
                result.Value.EffectDefs[0]
            )
            : ContentImportStageResult<CombatEffectImportModel>.Failure(result.Diagnostics);
    }
    ...
    )
    {
        ...
        for (int index = 0; index < values.Count; index++)
        {
            ...
            {
                AddInvalid(context, $"/attribute_modifiers/{index}", "Attribute modifier must be a non-null AttributeModifier fixture.", diagnostics);
                continue;
            }
    ...
    private static void AddInvalid(JsonContentEntryContext context, string pointer, string message, List<ContentJsonDiagnostic> diagnostics) =>
        diagnostics.Add(new ContentJsonDiagnostic(InvalidFixtureRule, message, context.SourceLabel, context.JsonPointer + pointer));
    private static ContentImportStageResult<SkillImportModel> Failure(JsonContentEntryContext context, string message, string pointer) =>
        ContentImportStageResult<SkillImportModel>.Failure(new ContentJsonDiagnostic(InvalidFixtureRule, message, context.SourceLabel, context.JsonPointer + pointer));
}
