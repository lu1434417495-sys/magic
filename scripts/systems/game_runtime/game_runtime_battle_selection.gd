...
func get_selected_battle_skill_required_coord_count() -> int:
	...
	if skill_def != null and skill_def.combat_profile != null:
		...
		if selection_mode == &"multi_unit":
			...
			return maxi(int(skill_def.combat_profile.get_effective_max_target_count(skill_level)), int(skill_def.combat_profile.min_target_count))
...
func _build_battle_skill_selection_status(skill_def, active_unit) -> String:
	...
	if selection_mode == &"multi_unit":
		var skill_level := _get_unit_skill_level(active_unit, skill_def.skill_id)
		var min_target_count := maxi(int(skill_def.combat_profile.min_target_count), 1)
		var max_target_count := maxi(int(skill_def.combat_profile.get_effective_max_target_count(skill_level)), min_target_count)
