...
func open_party_warehouse_window(entry_label: String) -> void:
	...
	if party_warehouse_service != null:
		...
		var equipment_instance_id_allocator := Callable(game_session, "allocate_equipment_instance_id") if game_session != null and game_session.has_method("allocate_equipment_instance_id") else Callable()
