...
public partial class GameRuntimeQuestCommandHandler : RefCounted
{
    ...
    private static readonly StringName InvalidQuestDisplayNameMessage = "任务配置缺少 display_name，当前无法执行命令。";
    ...
    public Dictionary CommandAcceptQuest(StringName questId, bool allowReaccept = false)
    {
        ...
        if (partyState != null && partyState.Call("has_claimable_quest", questId).AsBool())
            return CommandError(string.Format("任务《{0}》已完成，奖励待领取，当前不可再次接取。", questLabel));
        var hasCompleted = partyState != null && partyState.Call("has_completed_quest", questId).AsBool();
        var isRepeatable = DictionaryGet(questData,"is_repeatable", false).AsBool();
        ...
        if (!characterManagement.Call("accept_quest", questId, GetWorldStep(), effectiveAllowReaccept).AsBool())
            return CommandError(string.Format("当前无法接取任务《{0}》。", questLabel));
        SetPartyState(characterManagement.Call("get_party_state"));
        var persistError = PersistPartyState();
        var message = hasCompleted && effectiveAllowReaccept
    ...
    public Dictionary CommandProgressQuest(StringName questId, StringName objectiveId, int progressDelta = 1, Dictionary payload = null)
    {
        ...
        var summary = ApplyQuestProgressEventsToParty(new Godot.Collections.Array { eventData }, "quest");
        var progressedQuestIds = summary.GetValueOrDefault("progressed_quest_ids", new Godot.Collections.Array()).AsGodotArray();
        ...
        if (!hasProgressed)
            return CommandError(string.Format("当前无法推进任务《{0}》的目标 {1}。", questLabel, objectiveId));
        var persistError = PersistPartyState();
    ...
    public Dictionary CommandSubmitQuestItem(StringName questId, StringName objectiveId = default)
    {
        ...
        if (string.IsNullOrEmpty(questLabel))
            return InvalidQuestDisplayNameError();
        var submitResult = characterManagement.Call("submit_item_objective", questId, objectiveId, GetWorldStep()).AsGodotDictionary();
        if (!submitResult.GetValueOrDefault("ok", false).AsBool())
        {
            var itemId = ProgressionDataUtils.to_string_name(submitResult.GetValueOrDefault("item_id", ""));
            var itemLabel = GetItemDisplayName(itemId);
            var requiredQuantity = Mathf.Max(submitResult.GetValueOrDefault("required_quantity", 0).AsInt32(), 0);
            var errorCode = submitResult.GetValueOrDefault("error_code", "").AsString();
            switch (errorCode)
            {
                ...
                case "quest_def_missing":
                    return CommandError(string.Format("任务《{0}》缺少目标配置，当前无法提交。", questLabel));
                case "invalid_submit_item_objective":
                    return CommandError(string.Format("任务《{0}》包含无效的物资提交目标，当前无法提交。", questLabel));
                case "objective_already_complete":
                    return CommandError(string.Format("任务《{0}》的物资目标已完成，无需重复提交。", questLabel));
                case "submit_item_missing_inventory":
                    return CommandError(string.Format("共享仓库缺少{0} x{1}，无法提交给任务《{2}》。", itemLabel, requiredQuantity, questLabel));
                case "submit_item_commit_failed":
                    return CommandError(string.Format("当前无法从共享仓库扣除任务《{0}》所需物资。", questLabel));
                case "quest_progress_failed":
                    return CommandError(string.Format("共享仓库扣除已回滚，当前无法推进任务《{0}》。", questLabel));
                default:
                    return CommandError(string.Format("任务《{0}》当前没有可提交的物资目标。", questLabel));
            }
        }
        SetPartyState(characterManagement.Call("get_party_state"));
        var itemId = ProgressionDataUtils.to_string_name(submitResult.GetValueOrDefault("item_id", ""));
        var itemLabel = GetItemDisplayName(itemId);
        var submittedQuantity = Mathf.Max(submitResult.GetValueOrDefault("submitted_quantity", 0).AsInt32(), 0);
        var claimableQuestIds = submitResult.GetValueOrDefault("claimable_quest_ids", new Array()).AsGodotArray();
        var message = string.Format("已为任务《{0}》提交 {1} x{2}。", questLabel, itemLabel, submittedQuantity);
        if (claimableQuestIds.Contains(questId))
            message = string.Format("已为任务《{0}》提交 {1} x{2}，奖励待领取。", questLabel, itemLabel, submittedQuantity);
        var persistError = PersistPartyState();
        if (persistError != Error.Ok)
        {
            message = string.Format("{0} 但队伍状态持久化失败。", message);
        ...
        result["objective_id"] = submitResult.GetValueOrDefault("objective_id", "").AsString();
        result["item_id"] = successItemId.ToString();
    ...
    public Dictionary CommandClaimQuest(StringName questId)
    {
        ...
        if (string.IsNullOrEmpty(questLabel))
            return InvalidQuestDisplayNameError();
        var claimResult = characterManagement.Call("claim_quest_reward", questId, GetWorldStep()).AsGodotDictionary();
        if (!claimResult.GetValueOrDefault("ok", false).AsBool())
        {
            var errorCode = claimResult.GetValueOrDefault("error_code", "").AsString();
            switch (errorCode)
            {
                case "quest_not_claimable":
                    return CommandError(string.Format("当前没有可领取的任务《{0}》奖励。", questLabel));
                case "quest_def_missing":
                    return CommandError(string.Format("任务《{0}》缺少奖励配置，当前无法领取。", questLabel));
                case "invalid_quest_display_name":
                    return InvalidQuestDisplayNameError();
                case "invalid_gold_amount":
                    return CommandError(string.Format("任务《{0}》包含无效的金币奖励配置，当前无法领取。", questLabel));
                case "invalid_item_reward":
                    return CommandError(string.Format("任务《{0}》包含无效的物品奖励配置，当前无法领取。", questLabel));
                case "invalid_item_display_name":
                    return CommandError(string.Format("任务《{0}》引用的物品奖励缺少 display_name，当前无法领取。", questLabel));
                case "invalid_pending_character_reward":
                    return CommandError(string.Format("任务《{0}》包含无效的角色奖励配置，当前无法领取。", questLabel));
                case "item_reward_missing_def":
                    return CommandError(string.Format("任务《{0}》引用了缺失的物品奖励配置，当前无法领取。", questLabel));
                case "reward_overflow":
                    return CommandError(string.Format("共享仓库空间不足，领取任务《{0}》奖励会溢出，当前无法领取。", questLabel));
                case "quest_reward_commit_failed":
                    return CommandError(string.Format("任务《{0}》奖励写入共享仓库失败，当前无法领取。", questLabel));
                case "unsupported_reward_types":
                    var unsupportedTypes = StringNameArrayToStringArray(claimResult.GetValueOrDefault("unsupported_reward_types", new Array()).AsGodotArray());
                    var unsupportedText = unsupportedTypes.Count > 0 ? string.Join("。", unsupportedTypes) : "未知奖励";
                    return CommandError(string.Format("任务《{0}》包含暂不支持的奖励类型：{1}。", questLabel, unsupportedText));
                default:
        ...
        SetPartyState(characterManagement.Call("get_party_state"));
        var persistError = PersistPartyState();
        var goldDelta = claimResult.GetValueOrDefault("gold_delta", 0).AsInt32();
        ...
        result["item_rewards"] = claimResult.GetValueOrDefault("item_rewards", new Array()).AsGodotArray().Duplicate(true);
        result["pending_character_rewards"] = claimResult.GetValueOrDefault("pending_character_rewards", new Array()).AsGodotArray().Duplicate(true);
        return result;
    }
    ...
    private void SetPartyState(Variant partyState)
    {
        if (HasRuntime())
    ...
    {
        return HasRuntime() ? (Error)_runtime.Call("persist_party_state").AsInt32() : Error.Unavailable;
    }
    ...
    {
        return HasRuntime() ? _runtime.Call("get_item_display_name", itemId).AsString() : itemId.ToString();
    }

    private Dictionary ApplyQuestProgressEventsToParty(Godot.Collections.Array eventVariants, string sourceDomain = "quest")
    {
        return HasRuntime() ? _runtime.Call("apply_quest_progress_events_to_party", eventVariants, sourceDomain).AsGodotDictionary() : new Dictionary();
    }

    private Dictionary GetQuestDefData(StringName questId)
    {
        return HasRuntime() ? _runtime.Call("_get_quest_def_data", questId).AsGodotDictionary() : new Dictionary();
    }

    private string ResolveQuestLabel(StringName questId, Dictionary questData)
    {
        return HasRuntime() ? _runtime.Call("_resolve_quest_label", questId, questData).AsString() : "";
    }

    private string BuildQuestClaimRewardSummaryText(Dictionary claimResult)
    {
        return HasRuntime() ? _runtime.Call("_build_quest_claim_reward_summary_text", claimResult).AsString() : "";
    }

    private Array<String> StringNameArrayToStringArray(Godot.Collections.Array values)
    {
        return HasRuntime() ? _runtime.Call("_string_name_array_to_string_array", values).AsGodotArray<String>() : new Array<String>();
    }
