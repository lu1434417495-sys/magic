...
func get_charge_path_step_aoe_effect_def(
	cast_variant: CombatCastVariantDef,
	...
	if cast_variant == null:
		return null
	var skill_level := -1
	if skill_def != null and active_unit != null and _has_runtime():
		skill_level = int(_runtime.get_unit_skill_level(active_unit, skill_def.skill_id))
	for effect_def in cast_variant.effect_defs:
		if effect_def != null and effect_def.effect_type == PATH_STEP_AOE_EFFECT_TYPE:
			if skill_level >= 0 and not _is_effect_unlocked_for_skill_level(effect_def, skill_level):
				continue
			return effect_def
	return null
...
func _is_effect_unlocked_for_skill_level(effect_def: CombatEffectDef, skill_level: int) -> bool:
	if effect_def == null:
		return false
...
func _resolve_charge_blocker(
	...
	if not reserved_coord_set.has(forward_coord):
		...
		if _runtime.get_grid_service().move_unit(_runtime.get_state(), blocker, forward_coord):
			...
			var fall_layers := _calculate_fall_layers_for_coord(blocker, forward_coord)
			if fall_layers > 0:
				var fall_result: Dictionary = _runtime.get_damage_resolver().resolve_fall_damage(blocker, fall_layers)
				var fall_damage := int(fall_result.get("damage", 0))
				var fall_shield_absorbed := int(fall_result.get("shield_absorbed", 0))
				if fall_damage > 0 or fall_shield_absorbed > 0:
					if fall_damage > 0:
						batch.log_lines.append("%s 的前推使 %s 跌落 %d 层，受到 %d 点坠落伤害。" % [
							active_unit.display_name,
							blocker.display_name,
							fall_layers,
							fall_damage,
						])
						if fall_shield_absorbed > 0:
							batch.log_lines.append("%s 的护盾吸收了 %d 点坠落伤害。" % [
								blocker.display_name,
								fall_shield_absorbed,
							])
					else:
						batch.log_lines.append("%s 的前推使 %s 跌落 %d 层，但被护盾吸收了 %d 点坠落伤害。" % [
							active_unit.display_name,
							blocker.display_name,
							fall_layers,
							fall_shield_absorbed,
		...
		var forward_cell: BattleCellState = _runtime.get_grid_service().get_cell(_runtime.get_state(), forward_coord) as BattleCellState
		var blocker_cell: BattleCellState = _runtime.get_grid_service().get_cell(_runtime.get_state(), blocker.coord) as BattleCellState
		var height_diff := 0
		if forward_cell != null and blocker_cell != null:
			height_diff = absi(int(blocker_cell.current_height) - int(forward_cell.current_height))

		var env_result: Dictionary
		var env_damage_label: String
		if height_diff > 1:
			env_result = _runtime.get_damage_resolver().resolve_fall_damage(blocker, height_diff)
			env_damage_label = "撞向高地"
		else:
			env_result = _runtime.get_damage_resolver().resolve_fall_damage(blocker, 1)
			env_damage_label = "撞向障碍物"
		var env_damage := int(env_result.get("damage", 0))
		var env_shield := int(env_result.get("shield_absorbed", 0))
		if env_damage > 0 or env_shield > 0:
			if env_damage > 0:
				batch.log_lines.append("%s 的冲锋将 %s %s，受到 %d 点碰撞伤害。" % [
					active_unit.display_name,
					blocker.display_name,
					env_damage_label,
					env_damage,
				...
				batch.log_lines.append("%s 的冲锋将 %s %s，但被护盾吸收了 %d 点碰撞伤害。" % [
					active_unit.display_name,
					blocker.display_name,
					env_damage_label,
					env_shield,
				])
			if bool(env_result.get("shield_broken", false)):
				batch.log_lines.append("%s 的护盾被击碎。" % blocker.display_name)
			_runtime.append_changed_unit_id(batch, blocker.unit_id)
			if not blocker.is_alive:
				_runtime.clear_defeated_unit(blocker, batch)
...
func _calculate_fall_layers_for_coord(unit: BattleUnitState, target_coord: Vector2i) -> int:
	if unit == null or not _has_runtime():
		return 0
	var current_cell: BattleCellState = _runtime.get_grid_service().get_cell(_runtime.get_state(), unit.coord) as BattleCellState
	var target_cell: BattleCellState = _runtime.get_grid_service().get_cell(_runtime.get_state(), target_coord) as BattleCellState
	if current_cell == null or target_cell == null:
		return 0
	return maxi(int(current_cell.current_height) - int(target_cell.current_height), 0)
