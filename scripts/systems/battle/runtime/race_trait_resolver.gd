...
static func apply_to_unit(unit_state: BattleUnitState, context: PassiveSourceContext) -> void:
	if unit_state == null or context == null:
		return
	_apply_identity_def_projection(unit_state, context.race_def, unit_state.race_trait_ids)
	_apply_identity_def_projection(unit_state, context.subrace_def, unit_state.subrace_trait_ids)
