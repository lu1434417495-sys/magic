...
{
	[Export] public StringName component_id { get; set; } = "";
	[Export] public StringName role_label { get; set; } = "";
	[Export] public StringName damage_tag { get; set; } = "";
	[Export] public int base_power { get; set; } = 0;
	[Export] public int dice_count { get; set; } = 0;
	[Export] public int dice_sides { get; set; } = 0;
	[Export] public double ring_weight { get; set; } = 1.0;
	[Export] public StringName save_profile_id { get; set; } = "";
	[Export] public bool can_crit { get; set; } = false;
	[Export] public double mastery_weight { get; set; } = 1.0;
	[Export] public int ring_min { get; set; } = 0;
	[Export] public int ring_max { get; set; } = 3;
	[Export] public GDictionary ring_damage_scale_bp { get; set; } = new();
	...
	public double get_damage_scale(int distance_from_anchor)
	{
		...
		var raw_value = ring_damage_scale_bp.ContainsKey(distance_from_anchor)
			? ring_damage_scale_bp[distance_from_anchor]
			: ring_damage_scale_bp.ContainsKey(key)
				? ring_damage_scale_bp[key]
	...
	public int get_average_base_damage(int distance_from_anchor)
	{
		...
		return Math.Max((int)Math.Round((base_power + dice_average) * get_damage_scale(distance_from_anchor)), 0);
	}

	public int get_worst_case_base_damage(int distance_from_anchor)
	{
		var dice_worst = Math.Max(dice_count, 0) * Math.Max(dice_sides, 0);
		return Math.Max((int)Math.Round((base_power + dice_worst) * get_damage_scale(distance_from_anchor)), 0);
	}
	...
	private static double _variant_to_double(Variant value, double fallback)
	{
		return value.VariantType switch
		{
			Variant.Type.Int => value.AsInt32(),
			Variant.Type.Float => value.AsDouble(),
			Variant.Type.String => double.TryParse(value.AsString(), out var parsed) ? parsed : fallback,
			_ => fallback,
		};
	}
}
