...
public partial class BattleEdgeService : RefCounted
{
    ...
    public void ensure_runtime_edge_faces(GodotObject state)
    {
        ...
        state.Set("runtime_edge_faces", build_edge_faces_for_cells(
    ...
    public GDictionary build_edge_faces_for_cells(GDictionary cells, Vector2I map_size, GDictionary cell_columns)
    {
        var edgeFaces = new GDictionary();
        GDictionary resolvedColumns = cell_columns != null && cell_columns.Count > 0
            ? cell_columns
        ...
        for (int y = 0; y < maxY; y++)
        {
            for (int x = 0; x < maxX; x++)
            {
                ...
                edgeFaces[BuildEdgeKey(originCoord, DirectionIndexEast)] = BuildEdgeFace(cells, resolvedColumns, originCoord, originCell, DirectionEast);
                edgeFaces[BuildEdgeKey(originCoord, DirectionIndexSouth)] = BuildEdgeFace(cells, resolvedColumns, originCoord, originCell, DirectionSouth);
            }
        }
        return edgeFaces;
    }
    ...
    public Godot.Collections.Array<BattleEdgeFaceState> get_all_edge_faces(GodotObject state)
    {
        ...
        foreach (Variant edgeFaceVariant in GdInterop.GetDictionary(state, "runtime_edge_faces").Values)
        {
            if (edgeFaceVariant.VariantType == Variant.Type.Object && edgeFaceVariant.AsGodotObject() is BattleEdgeFaceState edgeFace)
            {
    ...
    public BattleEdgeFaceState get_edge_face(GodotObject state, Vector2I from_coord, Vector2I to_coord)
    {
        ...
        ensure_runtime_edge_faces(state);
        return get_edge_face_from_cache(GdInterop.GetDictionary(state, "runtime_edge_faces"), from_coord, to_coord);
    }

    public BattleEdgeFaceState get_edge_face_by_origin(GodotObject state, Vector2I origin_coord, Vector2I direction)
    {
        if (state == null)
        {
            return null;
    ...
    public BattleEdgeFaceState get_edge_face_from_cache(GDictionary edge_faces, Vector2I from_coord, Vector2I to_coord)
    {
    ...
    public bool is_traversable_in_cache(GDictionary edge_faces, Vector2I from_coord, Vector2I to_coord)
    {
    ...
    public bool blocks_occupancy_in_cache(GDictionary edge_faces, Vector2I from_coord, Vector2I to_coord)
    {
        return blocks_occupancy_for_edge_face(get_edge_face_from_cache(edge_faces, from_coord, to_coord));
    }
    ...
    public bool has_feature_between(GodotObject state, Vector2I from_coord, Vector2I to_coord, StringName feature_kind)
    {
        ...
        Vector2I direction)
    {
        ...
        int fromHeight = GetColumnTopHeight(GetDictionaryValue(cellColumns, originCoord), originCell);
        int toHeight = GetColumnTopHeight(GetDictionaryValue(cellColumns, neighborCoord), neighborCell);
    ...
    private static void ApplyAuthoredFeature(BattleEdgeFaceState edgeFace, BattleEdgeFeatureState featureState)
    {
    ...
    private static int GetColumnTopHeight(Variant columnVariant, BattleCellState fallbackSurfaceCell = null)
    {
        if (columnVariant.VariantType == Variant.Type.Array)
        {
            GArray column = columnVariant.AsGodotArray();
            for (int index = column.Count - 1; index >= 0; index--)
            {
                Variant cellVariant = column[index];
                if (cellVariant.VariantType == Variant.Type.Object && cellVariant.AsGodotObject() is BattleCellState layerCell)
                {
                    return layerCell.stack_layer;
                }
            }
        }
        return fallbackSurfaceCell != null ? fallbackSurfaceCell.current_height : BoundaryRenderHeight;
    }

    private static Godot.Collections.Array<int> BuildExposedLayerHeights(int fromHeight, int toHeight)
    {
    ...
    private static BattleCellState GetCell(GDictionary cells, Vector2I coord)
    {
        ...
        return value.VariantType == Variant.Type.Object ? value.AsGodotObject() as BattleCellState : null;
    }

    private static BattleEdgeFaceState GetEdgeFaceFromDictionary(GDictionary edgeFaces, Vector3I key)
    {
        ...
        return value.VariantType == Variant.Type.Object ? value.AsGodotObject() as BattleEdgeFaceState : null;
    }

    private static Variant GetDictionaryValue(GDictionary values, Variant key)
    {
