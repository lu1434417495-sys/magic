...
func build_repeat_attack_stage_hit_check(
	...
	if repeat_attack_effect != null and repeat_attack_effect.params != null:
		...
		var penalty_value := int(repeat_attack_effect.params.get("follow_up_attack_penalty", 0))

		# 计算等级相关的免惩罚段数
		var penalty_free_stages := 0
		var level_stages_map: Dictionary = repeat_attack_effect.params.get("penalty_free_stages_by_level", {})
		if not level_stages_map.is_empty() and active_unit != null and skill_def != null:
			var skill_level := int(active_unit.known_skill_level_map.get(skill_def.skill_id, 0))
			var best_level := -1
			for level_key in level_stages_map.keys():
				var level_val := int(level_key)
				if level_val <= skill_level and level_val > best_level:
					best_level = level_val
					penalty_free_stages = int(level_stages_map.get(level_key, 0))

		if stage_index < penalty_free_stages:
			stage_penalty = 0
		elif bool(repeat_attack_effect.params.get("exponential_penalty", false)):
			stage_penalty = int(pow(2, stage_index)) * penalty_value
		else:
			stage_penalty = maxi(stage_index, 0) * penalty_value
...
func build_skill_attack_check(
	...
	if active_unit != null and skill_def != null:
		if active_unit.known_skill_level_map.has(skill_def.skill_id):
			skill_level = int(active_unit.known_skill_level_map.get(skill_def.skill_id, 0))
