...
func collect_combat_profile_target_coords(
	...
	if _is_self_target_collection(combat_profile):
	...
	var area_value := maxi(int(combat_profile.area_value), 0)
...
func _is_self_target_collection(combat_profile: CombatSkillDef) -> bool:
	if combat_profile == null:
		return false
	...
	if StringName(combat_profile.target_team_filter) == &"self":
		return true
	return StringName(combat_profile.area_pattern) == &"self"
