...
extends "res://scripts/systems/battle_unit_factory_runtime.gd"
...
var _skill_mastery_resolution_events: Array[Dictionary] = []
...
func _validate_change_equipment_command(active_unit: BattleUnitState, command: BattleCommand) -> Dictionary:
	...
	if operation == BattleCommand.EQUIPMENT_OPERATION_EQUIP:
		...
		var item_rule := _resolve_change_equipment_item_rule(resolved_item_id, slot_id, command)
...
func _resolve_change_equipment_item_rule(item_id: StringName, slot_id: StringName, command: BattleCommand) -> Dictionary:
...
func _get_change_equipment_item_def(item_id: StringName):
	...
	if normalized == &"" or _item_defs == null:
		return null
	if _item_defs.has(normalized):
		return _item_defs.get(normalized)
	return _item_defs.get(String(normalized))


func _has_change_equipment_item_catalog() -> bool:
	return _item_defs != null and not _item_defs.is_empty()
...
func _get_change_equipment_backpack_capacity() -> int:
	if _character_gateway == null or not _character_gateway.has_method("get_party_state"):
		return -1
	var party_state = _character_gateway.call("get_party_state")
	if party_state == null or not (party_state is Object):
		return -1
...
func _handle_skill_command(active_unit: BattleUnitState, command: BattleCommand, batch: BattleEventBatch) -> void:
	...
	_skill_mastery_resolution_events.clear()
	var applied := false
...
func _apply_unit_skill_result(
	...
	record_skill_mastery_resolution(active_unit, target_unit, result)
...
func _record_vajra_body_mastery_from_incoming_damage(
	...
	var mastery_source_id := _resolve_first_allowed_skill_mastery_source(VAJRA_BODY_SKILL_ID, mastery_source_ids)
	...
	if mastery_amount <= 0:
		return
	if _character_gateway == null:
		return
	if _is_vajra_body_low_hp_training_window(target_unit) and _character_gateway != null:
		_character_gateway.record_achievement_event(
			target_unit.source_member_id,
	...
	_append_progression_delta_to_batch(target_unit, delta, batch)
...
func _resolve_first_allowed_skill_mastery_source(skill_id: StringName, source_ids: Array[StringName]) -> StringName:
...
func _apply_ground_unit_effects(
	...
	for target_unit in _collect_units_in_coords(effect_coords):
		...
		record_skill_mastery_resolution(source_unit, target_unit, result)
		var shield_result := _apply_unit_shield_effects(
			source_unit,
			target_unit,
			skill_def,
...
func _grant_skill_mastery_if_needed(active_unit: BattleUnitState, skill_id: StringName, batch: BattleEventBatch) -> void:
	...
	var mastery_amount := _resolve_skill_mastery_amount_from_resolution_events()
	if mastery_amount <= 0:
		return
...
func record_skill_mastery_resolution(
	source_unit: BattleUnitState,
	target_unit: BattleUnitState,
	result: Dictionary
) -> void:
	...
	if source_unit.source_member_id == &"":
		return
	if not _is_skill_mastery_qualifying_result(result):
		return
	var amount := _resolve_skill_mastery_target_amount(source_unit, target_unit)
	if amount <= 0:
		return
	_skill_mastery_resolution_events.append({
...
func _resolve_skill_mastery_amount_from_resolution_events() -> int:
	var total := 0
	for event_variant in _skill_mastery_resolution_events:
		if event_variant is not Dictionary:
			continue
...
func _is_skill_mastery_qualifying_result(result: Dictionary) -> bool:
...
func _resolve_skill_mastery_target_amount(source_unit: BattleUnitState, target_unit: BattleUnitState) -> int:
	if source_unit == null or target_unit == null:
		return 0
...
func _consume_turn_cooldown_delta(unit_state: BattleUnitState) -> bool:
	...
	if elapsed_tu % TU_GRANULARITY != 0:
		...
		return false
	return _advance_unit_cooldowns(unit_state, elapsed_tu)
...
func _apply_unit_status_periodic_ticks(
	...
	if _state == null or _state.timeline == null or unit_state == null or elapsed_tu <= 0:
		return {"changed": false, "defeat_source_unit_id": ""}
	var changed := false
	var defeat_source_unit_id: StringName = &""
	var current_tu := int(_state.timeline.current_tu)
...
func _resolve_encounter_resolution() -> StringName:
	...
	if _state.winner_faction_id == &"draw":
		return &"draw"
	return &"resolved"
