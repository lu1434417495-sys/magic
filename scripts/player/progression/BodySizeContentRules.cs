...
public partial class BodySizeContentRules : RefCounted
{
    ...
    public static StringName get_category_for_body_size(int bodySize)
    {
        return BODY_SIZE_TO_CATEGORY.TryGetValue(bodySize, out var result)
            ? result.AsStringName()
    ...
    {
        return is_valid_body_size_category(category) && get_body_size_for_category(category) == bodySize;
    }
