...
public partial class QuestProviderContentRules : RefCounted
{
    ...
    public static bool IsSupportedProviderId(Variant value) => SUPPORTED_PROVIDER_IDS.ContainsKey(NormalizeStringName(value));
    public static Godot.Collections.Dictionary SupportedProviderIds() { var d = new Godot.Collections.Dictionary(); foreach (var k in SUPPORTED_PROVIDER_IDS.Keys) d[k] = SUPPORTED_PROVIDER_IDS[k]; return d; }
    public static bool is_supported_provider_id(Variant value) => IsSupportedProviderId(value);
    ...
    public static string SupportedProviderLabel()
    {
        ...
        foreach (var key in SUPPORTED_PROVIDER_IDS.Keys) labels.Add((string)(StringName)key);
        labels.Sort();
    ...
    public static StringName NormalizeStringName(Variant value)
    {
        if (value.VariantType == Variant.Type.StringName) return value.AsStringName();
        if (value.VariantType == Variant.Type.String)
        {
            var text = value.AsString().StripEdges();
            return string.IsNullOrEmpty(text) ? new StringName("") : new StringName(text);
        }
        return new StringName("");
    }

    public static StringName normalize_string_name(Variant value) => NormalizeStringName(value);
}
