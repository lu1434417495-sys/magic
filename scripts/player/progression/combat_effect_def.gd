...
## 字段说明：在编辑器中暴露成长系数属性唯一标识配置，便于策划或关卡制作者在不改代码的情况下调整该脚本行为。
@export var scaling_attribute_id: StringName = &""
## 字段说明：在编辑器中暴露防御属性唯一标识配置，便于策划或关卡制作者在不改代码的情况下调整该脚本行为。
@export var defense_attribute_id: StringName = &""
