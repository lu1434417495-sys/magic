...
public partial class UnitReputationState : RefCounted
{
    ...
    {
        if (state_id == MORALITY) return morality;
    ...
    {
        if (state_id == MORALITY) morality = value;
        else custom_states[state_id] = value;
    }
    ...
    {
        if (!_hfs(data, new Godot.Collections.Array<string> { "morality", "custom_states" })) return null;
        var csv = data["custom_states"];
        if (csv.VariantType != Variant.Type.Dictionary) return null;
        if (data["morality"].VariantType != Variant.Type.Int) return null;
        var pcs = _parse_int_map(csv.AsGodotDictionary());
        if (pcs == null) return null;
        return new UnitReputationState { morality = data["morality"].AsInt32(), custom_states = pcs };
    }
    ...
    private static Godot.Collections.Dictionary _parse_int_map(Godot.Collections.Dictionary v) { var p = new Godot.Collections.Dictionary(); var s = new Godot.Collections.Dictionary(); foreach (var r in v.Keys) { var kt = r.VariantType; if (kt != Variant.Type.String && kt != Variant.Type.StringName) return null; var pk = ProgressionDataUtils.to_string_name(r); if ((string)pk == "" || s.ContainsKey(pk)) return null; if (v[r].VariantType != Variant.Type.Int) return null; s[pk] = true; p[pk] = v[r].AsInt32(); } return p; }
}
