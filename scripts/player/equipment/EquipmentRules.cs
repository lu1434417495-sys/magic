...
public partial class EquipmentRules : RefCounted
{
    ...
    {
        return SLOT_ORDER().Contains(ProgressionDataUtils.to_string_name(slot_id));
    }

    public static Godot.Collections.Array<StringName> normalize_slot_ids(Variant values)
    {
        var normalized = new Godot.Collections.Array<StringName>();
        var seen = new Godot.Collections.Dictionary();
        foreach (StringName raw in ProgressionDataUtils.to_string_name_array(values))
        {
            if (!is_valid_slot(raw))
            {
                continue;
            }
    ...
    public static string get_slot_label(StringName slot_id)
    {
        ...
        if (normalized == MainHandId) return "主手";
        if (normalized == OffHandId) return "副手";
        if (normalized == HeadId) return "头部";
        if (normalized == BodyId) return "身躯";
        if (normalized == HandsId) return "手部";
        if (normalized == FeetId) return "脚部";
        if (normalized == CloakId) return "披风";
        if (normalized == NecklaceId) return "项链";
        if (normalized == Ring1Id) return "戒指一";
        if (normalized == Ring2Id) return "戒指二";
        if (normalized == SpecialTrinketId) return "特殊饰品";
        if (normalized == BadgeId) return "徽章";
