...
{
	private const string DefaultMainWorldSettlementBundlePath = "res://data/configs/world_map/shared/main_world_default_settlement_bundle.tres";
	private const string DefaultMainWorldWildSpawnBundlePath = "res://data/configs/world_map/shared/main_world_default_wild_spawn_bundle.tres";
	private const string DefaultMainWorldSettlementNamePoolPath = "res://data/configs/world_map/shared/main_world_settlement_name_pool.tres";
	private const string DefaultMainWorldTownNamePoolPath = "res://data/configs/world_map/shared/main_world_town_name_pool.tres";
	private const string DefaultMainWorldCityNamePoolPath = "res://data/configs/world_map/shared/main_world_city_name_pool.tres";
	private const string DefaultMainWorldCapitalNamePoolPath = "res://data/configs/world_map/shared/main_world_capital_name_pool.tres";
	private const string DefaultMainWorldMetropolisNamePoolPath = "res://data/configs/world_map/shared/main_world_metropolis_name_pool.tres";
	...
	)
	{
		...
		foreach (Variant presetVariant in presets)
		{
			if (presetVariant.VariantType != Variant.Type.Dictionary)
			{
				errors.Add("World preset entry must be a Dictionary.");
				continue;
			}

			var preset = presetVariant.AsGodotDictionary();
			...
			{
				errors.Add($"World preset {presetId} failed to load generation config {generationConfigPath}.");
				continue;
			}
			AddRange(errors, validate_generation_config(
				generationConfig,
	...
	)
	{
		...
		{
			errors.Add($"World generation config {label} has invalid world_size_in_chunks {worldSizeInChunks}.");
		}
		...
		{
			errors.Add($"World generation config {label} has starting_wild_spawn_min_distance greater than max distance.");
		}
		...
		var settlementIds = ValidateSettlementLibrary(settlementResources, facilityIds, label, errors);
		ValidateSettlementDistribution(config.settlement_distribution, settlementIds, label, errors);
		ValidateWildSpawnRules(wildSpawnRules, config, enemy_templates, wild_encounter_rosters, label, errors);
	...
	)
	{
		...
		{
			var bundle = LoadResource<WorldMapSettlementBundle>(DefaultMainWorldSettlementBundlePath, label, errors);
			if (bundle != null)
			{
	...
	)
	{
		...
		if (generationConfig.inject_default_main_world_content)
		{
			var bundle = LoadResource<WorldMapWildSpawnBundle>(DefaultMainWorldWildSpawnBundlePath, label, errors);
			if (bundle != null)
			{
	...
	)
	{
		...
		foreach (Variant facilityVariant in facilityResources)
		{
			if (facilityVariant.AsGodotObject() is not FacilityConfig facility)
			{
				errors.Add($"World generation config {label} has non-FacilityConfig facility entry.");
				continue;
			}
			...
			{
				errors.Add($"World generation config {label} has duplicate facility_id {facilityId}.");
			}
			ids.Add(facilityId);
			...
			if (string.IsNullOrWhiteSpace(facility.interaction_type) && facility.bound_service_npcs.Count == 0)
			{
				errors.Add($"World facility {facilityId} in {label} must declare interaction_type or bound service NPCs.");
			}
	...
	)
	{
		...
		foreach (var npcResource in npcResources)
		{
			...
			{
				errors.Add($"World facility {facilityId} in {label} has non-FacilityNpcConfig service NPC.");
				continue;
			}
			...
			{
				errors.Add($"World facility {facilityId} NPC {npcId} in {label} is missing service_type.");
			}
			if (string.IsNullOrWhiteSpace(npc.interaction_script_id))
			{
				errors.Add($"World facility {facilityId} NPC {npcId} in {label} is missing interaction_script_id.");
			}
		}
	}
	...
	{
		var ids = new HashSet<string>();
		foreach (Variant settlementVariant in settlementResources)
		{
			if (settlementVariant.AsGodotObject() is not SettlementConfig settlement)
			{
				errors.Add($"World generation config {label} has non-SettlementConfig settlement entry.");
				continue;
			}
			var settlementId = (settlement.get_template_id() ?? string.Empty).Trim();
			if (string.IsNullOrEmpty(settlementId))
			{
				errors.Add($"World generation config {label} has settlement missing settlement_id.");
				continue;
			}
			if (ids.Contains(settlementId))
			{
				errors.Add($"World generation config {label} has duplicate settlement_id {settlementId}.");
			}
			...
			foreach (string facilityIdValue in settlement.guaranteed_facility_ids)
			{
				...
				if (string.IsNullOrEmpty(facilityId))
				{
					errors.Add($"World settlement {settlementId} in {label} has empty guaranteed facility id.");
				}
				else if (!facilityIds.Contains(facilityId))
				{
					errors.Add($"World settlement {settlementId} in {label} references missing guaranteed facility {facilityId}.");
				}
			}
			ValidateOptionalFacilityPool(settlement.optional_facility_pool, facilityIds, settlementId, label, errors);
		}
		return ids;
	}
	...
	)
	{
		...
		foreach (var slotResource in slots)
		{
			...
			{
				errors.Add($"World settlement {settlementId} in {label} has non-FacilitySlotConfig slot.");
				continue;
			}
			...
			{
				errors.Add($"World settlement {settlementId} in {label} has duplicate slot_id {slotId}.");
			}
			slotIds.Add(slotId);
			if (string.IsNullOrWhiteSpace(slot.slot_tag))
			{
				errors.Add($"World settlement {settlementId} slot {slotId} in {label} is missing slot_tag.");
			}
		}
	}
	...
	)
	{
		foreach (var entryResource in pool)
		{
			...
			{
				errors.Add($"World settlement {settlementId} in {label} has non-WeightedFacilityEntry optional facility.");
				continue;
			}
			var facilityId = (entry.get_facility_template_id() ?? string.Empty).Trim();
			if (string.IsNullOrEmpty(facilityId))
			{
				errors.Add($"World settlement {settlementId} in {label} has optional facility missing facility_id.");
			}
			else if (!facilityIds.Contains(facilityId))
			{
				errors.Add($"World settlement {settlementId} in {label} references missing optional facility {facilityId}.");
			}
			if (entry.weight <= 0)
			{
				errors.Add($"World settlement {settlementId} in {label} has optional facility {facilityId} with non-positive weight.");
			}
		}
	}
	...
	)
	{
		foreach (var ruleResource in distribution)
		{
			...
			{
				errors.Add($"World generation config {label} has non-SettlementDistributionRule entry.");
				continue;
			}
			var settlementId = (rule.get_settlement_template_id() ?? string.Empty).Trim();
			if (string.IsNullOrEmpty(settlementId))
			{
				errors.Add($"World generation config {label} has distribution rule missing settlement_id.");
			}
			else if (!settlementIds.Contains(settlementId))
			{
				errors.Add($"World generation config {label} settlement distribution references missing settlement {settlementId}.");
			}
			if (string.IsNullOrWhiteSpace(rule.faction_id))
			{
				errors.Add($"World generation config {label} settlement distribution for {settlementId} is missing faction_id.");
			}
		}
	}
	...
	)
	{
		...
		foreach (Variant ruleVariant in ruleResources)
		{
			if (ruleVariant.AsGodotObject() is not WildSpawnRule rule)
			{
			...
			{
				errors.Add($"World generation config {label} has wild spawn rule missing region_tag.");
			}
			if (string.IsNullOrEmpty(enemyRosterTemplateId) && string.IsNullOrEmpty(encounterProfileId))
			{
				errors.Add($"World generation config {label} wild spawn rule {regionTag} must declare enemy_roster_template_id or encounter_profile_id.");
			}
			if (!string.IsNullOrEmpty(enemyRosterTemplateId) && HasEntries(enemyTemplates) && !HasStringNameKey(enemyTemplates, enemyRosterTemplateId))
			{
				errors.Add($"World generation config {label} wild spawn rule {regionTag} references missing enemy roster template {enemyRosterTemplateId}.");
			}
			if (!string.IsNullOrEmpty(encounterProfileId) && HasEntries(wildEncounterRosters) && !HasStringNameKey(wildEncounterRosters, encounterProfileId))
			{
				errors.Add($"World generation config {label} wild spawn rule {regionTag} references missing encounter profile {encounterProfileId}.");
			}
			if (rule.density_per_chunk <= 0)
			{
				errors.Add($"World generation config {label} wild spawn rule {regionTag} has non-positive density_per_chunk.");
			}
			if (rule.vision_range < 0)
			{
				errors.Add($"World generation config {label} wild spawn rule {regionTag} has negative vision_range.");
			}
			if (rule.min_distance_to_settlement < 0)
			{
				errors.Add($"World generation config {label} wild spawn rule {regionTag} has negative min_distance_to_settlement.");
			}
			if (rule.chunk_coords.Count == 0)
			{
				errors.Add($"World generation config {label} wild spawn rule {regionTag} has empty chunk_coords.");
			}
			else
			{
				foreach (Vector2I coord in rule.chunk_coords)
				{
					if (coord.X < 0 || coord.Y < 0 || coord.X >= worldSizeInChunks.X || coord.Y >= worldSizeInChunks.Y)
					{
						errors.Add($"World generation config {label} wild spawn rule {regionTag} has chunk_coord {coord} outside world chunk range {worldSizeInChunks}.");
					}
				}
			}
		}
	...
	)
	{
		...
		foreach (var submapResource in submaps)
		{
			...
			{
				errors.Add($"World generation config {label} has non-MountedSubmapConfig mounted_submaps entry.");
				continue;
			}
			var submapId = submap.submap_id.ToString().Trim();
			if (string.IsNullOrEmpty(submapId))
			{
				errors.Add($"World generation config {label} has mounted submap missing submap_id.");
				continue;
			}
			if (ids.ContainsKey(submapId))
			{
				errors.Add($"World generation config {label} has duplicate mounted submap_id {submapId}.");
			}
			ids[submapId] = true;
			var path = (submap.generation_config_path ?? string.Empty).Trim();
			if (string.IsNullOrEmpty(path))
			{
				errors.Add($"World mounted submap {submapId} in {label} is missing generation_config_path.");
				continue;
			}
			if (visitedPaths.ContainsKey(path))
			{
				errors.Add($"World mounted submap {submapId} in {label} creates recursive generation_config_path {path}.");
				continue;
			}
			var submapConfig = LoadCachedResource(path);
			if (submapConfig == null)
			{
				errors.Add($"World mounted submap {submapId} in {label} failed to load generation_config_path {path}.");
				continue;
			}
			if (submapConfig is not WorldMapGenerationConfig)
			{
				errors.Add($"World mounted submap {submapId} in {label} expected {path} to use WorldMapGenerationConfig.");
				continue;
			}
			AddRange(errors, ValidateGenerationConfigInternal(
				submapConfig,
				path,
				enemyTemplates,
	...
	)
	{
		...
		foreach (var eventResource in events)
		{
			...
			{
				errors.Add($"World generation config {label} has non-WorldEventConfig world_events entry.");
				continue;
			}
			...
			{
				errors.Add($"World generation config {label} has duplicate world event_id {eventId}.");
			}
			ids.Add(eventId);
			var coord = worldEvent.world_coord;
			if (coord.X < 0 || coord.Y < 0 || coord.X >= worldSizeCells.X || coord.Y >= worldSizeCells.Y)
			{
				errors.Add($"World event {eventId} in {label} has world_coord {coord} outside world cell range {worldSizeCells}.");
			}
			if (worldEvent.event_type == WorldEventTypeEnterSubmap)
			{
				...
				{
					errors.Add($"World event {eventId} in {label} with event_type enter_submap is missing target_submap_id.");
				}
				else if (!mountedSubmapIds.ContainsKey(target))
				{
					errors.Add($"World event {eventId} in {label} references missing target_submap_id {target}.");
				}
			}
		}
	}

	private static void ValidateNamePool(string resourcePath, string label, Godot.Collections.Array<string> errors)
	{
		...
		if (names.Count == 0)
		{
			errors.Add($"World generation config {label} has empty settlement name pool {resourcePath}.");
		}
	}

	private static T LoadResource<T>(string resourcePath, string label, Godot.Collections.Array<string> errors) where T : Resource
	{
		...
		{
			errors.Add($"World generation config {label} expected {resourcePath} to use {typeof(T).Name}.");
			return null;
		}
	...
	private static Resource LoadCachedResource(string resourcePath)
	{
		...
		if (ResourceCache.TryGetValue(resourcePath, out var cachedResource) && GodotObject.IsInstanceValid(cachedResource))
		{
	...
	private static string GetDictionaryString(GDictionary dictionary, string key)
	{
		...
		return dictionary.ContainsKey(stringNameKey) ? dictionary[stringNameKey].ToString() : string.Empty;
	}

	private static void AddRange(Godot.Collections.Array<string> target, Godot.Collections.Array<string> source)
	{
		foreach (var item in source)
		{
			target.Add(item);
