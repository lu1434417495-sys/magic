...
func _register_brain_resource(resource_path: String) -> void:
	...
	if resource == null:
		_validation_errors.append("Failed to load enemy brain config %s." % resource_path)
		return
	if resource.get_script() != ENEMY_AI_BRAIN_DEF_SCRIPT:
		_validation_errors.append("Enemy brain config %s is not an EnemyAiBrainDef." % resource_path)
		return

	var brain := resource as EnemyAiBrainDef
	if brain == null:
		_validation_errors.append("Enemy brain config %s failed to cast to EnemyAiBrainDef." % resource_path)
		return
	if brain.brain_id == &"":
		_validation_errors.append("Enemy brain config %s is missing brain_id." % resource_path)
		return
...
func _register_template_resource(resource_path: String) -> void:
	var resource := load(resource_path)
	if resource == null:
		_validation_errors.append("Failed to load enemy template config %s." % resource_path)
		return
	if resource.get_script() != ENEMY_TEMPLATE_DEF_SCRIPT:
		_validation_errors.append("Enemy template config %s is not an EnemyTemplateDef." % resource_path)
		return

	var template := resource as EnemyTemplateDef
	if template == null:
		_validation_errors.append("Enemy template config %s failed to cast to EnemyTemplateDef." % resource_path)
		return
	if template.template_id == &"":
		_validation_errors.append("Enemy template config %s is missing template_id." % resource_path)
		return
...
func _register_wild_encounter_roster_resource(resource_path: String) -> void:
	var resource := load(resource_path)
	if resource == null:
		_validation_errors.append("Failed to load wild encounter roster config %s." % resource_path)
		return
	if resource.get_script() != WILD_ENCOUNTER_ROSTER_DEF_SCRIPT:
		_validation_errors.append("Wild encounter roster config %s is not a WildEncounterRosterDef." % resource_path)
		return

	var roster := resource as WildEncounterRosterDef
	if roster == null:
		_validation_errors.append("Wild encounter roster config %s failed to cast to WildEncounterRosterDef." % resource_path)
		return
	if roster.profile_id == &"":
		_validation_errors.append("Wild encounter roster config %s is missing profile_id." % resource_path)
		return
