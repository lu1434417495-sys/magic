...
- The durable output is strict `battle_sim_profiles` JSON that can be committed as game config.
...
1. Evaluate candidate score-profile genomes by rendering a temporary
   BattleSim profile JSON entry into `res://.tmp_tuner/profiles/`.
...
8. Export:
   ...
   - `.tmp_tuner/p6_champion_score_profile.json`
...
/home/luchaoli/venvs/cuda-op/bin/python -m tools.battle_sim_tuner.export_score_profile \
  ...
  --output data/configs/json/battle_sim/profiles/p6_champion_score_profile.json \
  --profile-id p6_champion_score_profile
```
...
External BattleSim profile JSON documents can be included in the same verification
table with `--extra-profile label=res://path/to/profile.json`.
...
The supported game-config artifact is a strict BattleSim profile JSON document.
...
`BattleSimProfileContentRegistry` discovers the exported JSON, validates it, and
projects it to an immutable `BattleSimProfileDefinition`. Simulations select that
definition by `profile_id`; neither the simulation nor its report retains the JSON
path. Promotion into non-simulation enemy AI remains a separate reviewed content
change.
