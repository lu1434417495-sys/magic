...
本文档是装备能力系统中 `BattleSkillAvailabilityService` 和 `SkillEntryId` 一等化的详细落地设计。它拆自 `docs/design/weapons/equipment_ability_system.md`，原因是这条迁移会穿过大量存量战斗代码，不能只在总文档中写成“选择态和命令态新增 SkillEntryId”。
