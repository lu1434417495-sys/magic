...
- `WildSpawnRule` 生成 `EncounterAnchorData`：`region_tag`、`monster_name`、`enemy_roster_template_id`、`encounter_profile_id`、`density_per_chunk`、`min_distance_to_settlement`、`vision_range`、可选 `chunk_coords`。
...
`settlement_state` 至少要能表示 `visited`，服务模块可继续扩展研究、商店库存、阶段状态等字段。修改据点状态时必须写回 `active_world_data.settlements[i].settlement_state`，然后重建 lookup。
...
enemy_roster_template_id, region_tag, vision_range, is_cleared,
encounter_kind, encounter_profile_id, growth_stage, suppressed_until_step
```
...
- 确认后根据 `enemy_roster_template_id`、`encounter_profile_id`、当前 active map 信息和 player party 构造战斗请求。
...
- `faction_id = hostile`。
- `enemy_roster_template_id`、`encounter_profile_id` 从规则复制。
- `region_tag` 从规则复制。
...
默认 settlement encounter 也属于当前生成行为：如果尚不存在 `encounter_kind=settlement` 的锚点，且存在 `enemy_roster_template_id=wolf_pack` 的 wild rule，则尝试生成一个 `wild_settlement_N`，显示名为“荒狼巢穴”，`encounter_kind=settlement`，`encounter_profile_id=wolf_den`，`vision_range=max(rule.vision_range, 2)`；如果 rule 没有 chunk 坐标，则候选 chunk 为全世界上半区（wolf_pack 时跳过 midpoint 以下/等于下半部分）。
...
- 玩家胜利后：`encounter_kind=single` 时从 `active_world_data.encounter_anchors` 删除该 anchor；`encounter_kind=settlement` 时交给 `WildEncounterGrowthSystem.ApplyBattleVictory` 更新 growth/suppression，不删除。
