...
BAB 是角色命中能力的核心驱动，由 `character_level` + 职业 BAB 档位决定。

#### 三档 BAB 曲线

| 档位 | 公式 | 适用职业组 | L1 | L5 | L10 | L15 | L20 |
|------|------|-----------|----|----|-----|-----|-----|
| Full | +1/级 | 战士/圣骑/野蛮人/游侠 | +1 | +5 | +10 | +15 | +20 |
| 3/4 | +floor(level×3/4) | 牧师/盗贼/武僧/吟游 | +0 | +3 | +7 | +11 | +15 |
| 1/2 | +floor(level/2) | 法师/术士 | +0 | +2 | +5 | +7 | +10 |

#### BAB 与本项目的衔接

- `ProfessionDef` 新增 `bab_progression: StringName`（`full / three_quarter / half`）
- BAB 由 `character_level` 驱动，不由 `profession.rank` 驱动
- 多职业取当前 `is_active` 职业中最优的 BAB 档位
- BAB 作为派生属性注入 `AttributeSnapshot`，字段名 `base_attack_bonus`

#### 迭代攻击（Iterative Attacks）

3.5e 中 Full BAB 达到 +6 后，每差 5 点获得一次额外攻击：
...
以下为"同等级战士 vs 同等级战士"的基准命中率：

| 等级 | 攻击方加值 | 防御方 AC | 需掷 | 命中率 |
|------|-----------|----------|------|--------|
| 1 | +5 (BAB1+Str3+MW1) | 17 (甲5+盾2) | 12+ | 45% |
| 5 | +10 (BAB5+Str4+E1) | 21 (+1甲盾+ring) | 11+ | 50% |
| 10 | +17 (BAB10+Str5+E2) | 27 (+2全套) | 10+ | 55% |
| 15 | +24 (BAB15+Str6+E3) | 33 (+3全套+misc) | 9+ | 60% |
| 20 | +31 (BAB20+Str7+E4) | 38 (+4/5全套) | 7+ | 70% |

### 19.2 跨等级碾压表

| 攻击方等级 | 防御方等级 | 攻击加值 | 目标AC | 需掷 | 命中率 |
|-----------|-----------|---------|--------|------|--------|
| 10 | 1 | +17 | 17 | nat1 miss only | **95%** |
| 1 | 10 | +5 | 27 | 22+ | **不可能** |
| 10 | 5 | +17 | 21 | 4+ | **85%** |
| 5 | 10 | +10 | 27 | 17+ | **20%** |
| 15 | 10 | +24 | 27 | 3+ | **90%** |
...
**这就是你要的压迫感**：L10 打 L1 稳中，L1 打 L10 物理上不可能命中。
