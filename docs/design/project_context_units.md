...
- 技能 JSON 作者工具边界：`SkillContentJsonAuthoringDomain` 只注册当前 stage-1a pilot DTO 与 `layered_barrier` closed payload；schema exporter 从同一个 `SkillJsonDto` 生成 full entry、schema-only `template` control 与递归 partial template view，nullable carrier 的“字段缺席”不等于显式 JSON null。纯 .NET offline CLI linked compile 同一 parser/import contract，输出稳定 rule ID、source label 与 JSON pointer，不启动游戏、不构建 snapshot，也不调用 Resource-bound skill validator。`PendingCastBindingModeKind` 的 plain enum owner 位于 content/skills，Godot interop 只负责它与 `StringName` 的边界映射。
