...
- Timeline/status owner 边界：`BattleTimelineDriver` 真实拥有 timeline phase、current TU、`tu_per_tick`、ready unit、action-threshold 校验/日志、stamina 与 Control objective 分数推进；`BattleUnitActionClockState` 唯一持有 unit action progress、threshold 与 action-rate remainder，并原子累计余数和扣除全部 crossing，`BattleTemporalStatusService` 只决定进度倍率与运行时状态操作，时间状态 id/tag 与 release-effect 闭集判定由 CU-13 的 `TemporalStatusContentRules` 唯一拥有；`BattleUnitCooldownState` 唯一持有 unit cooldown map 与 last-turn TU anchor，并原子执行 anchor 重基准、粒度判定与合法冷却推进；`BattleRuntimeSkillTurnResolver` 真实拥有 current-TU/静滞/turn-start 调度、非法粒度日志、turn timer 与状态周期 tick/duration/turn-start 规则，并按状态应用时的 current TU 初始化 `next_tick_at_tu`，同时在非静滞单位的 status phase 调度 `BattleUnitShieldState` 原子推进/到期清空护盾 duration。step 开始时已处于 `time_stasis` 的单位整步冻结护盾 duration，即使静滞在该 step 末到期也从下一 step 才恢复。`BattleRuntimeModule.MarkAppliedStatusesForTurnTiming(...)` 只保留“先初始化 tick anchor，再通知 Fate”的跨 owner 编排。timeline step 继续按 current TU → Control 区域归属/计分 → 状态 tick/duration → 场地时效 → pending cast → ready collection/sort 推进；unit activation 继续按 trait turn-start → cooldown/turn timer → metrics/contingency/sequential auto-cast → AP/移动点重置 → turn-start status/control 推进。
- 桥接：通用 trait 进战斗为 `IBattleRuntimeCharacterGateway.BuildEffectiveTraitProjectionForEquipmentView(...)` → `BattleUnitFactory` → `BattleUnitEffectiveTraitState`（以 battle-local `EquipmentState` 重算，normal 写只接收 instances 并原子派生 ids；战斗规则不反查 trait/item catalog 或角色装备源），再由 `BattleTraitPassiveProjectionService` 将 trait typed passive 投影到 `BattleUnitSaveModifierState`、`BattleUnitDamageResistanceState` 与 `BattleStatusEffectCollection`，其中 trait 被动状态使用 `trait_passive_status` source layer 以便刷新装备投影时清理重建。伤害抗性由 `BattleUnitState` typed gateway 访问；race/subrace 的后写覆盖、trait 的 stronger-only 覆盖与 strict codec 校验仍由各自调用边界持有，伤害规则以单 tag 查询读取 owner。装备能力进战斗为 `GameContentCatalog.GetEquipmentAbilityBindingDefinitionsTyped/GetTraitDefsTyped` → `BattleRuntimeModule` typed 索引 → `BattleEquipmentAbilityProjectionService` 纯结果 → `BattleUnitEquipmentAbilityProjectionState`；共同 owner 原子持有 sources 与 temporal modifiers，向 equipment runtime/技能入口提供不可变来源视图，并缓存 action/cast 各自按 `ModifierId` ordinal 最小、同 ID 首项优先的 temporal scalar 视图供 timeline/casting 零扫描读取，实际掷骰与属性读取仍留在 `BattleTemporalStatusService`。canonical/strict/detached 继续只包含既有 `equipment_ability_sources` 字段，strict load 把 runtime-only temporal 组件置空；因此正式 encounter roster 启动直接消费 `EncounterRosterBuilder` 返回的 typed unit 列表，不以 canonical payload 作为 world→battle 内部传递通道。AI exact 继续用既有两个 stable key 分别保留两个组件的 raw 语义。角色装备武器的 `range_type` 通过 `WeaponProjection` 进入 `BattleUnitWeaponProjectionState`，由不可变 read view 供装备能力 fact 读取；敌人经 `EncounterRosterBuilder` 投 battle-only source；战斗规则只从 `BattleUnitState` typed gateway 读 effective trait、伤害抗性、装备能力源、时间进度 modifier、武器 range type 与 `creature_type_tags`。装备能力召唤的战斗内单位由 `BattleEquipmentSummonResolver`（`BattleEquipmentAbilityRuntimeService` 持有的 summon 职责拆分类）创建为 battle-only `BattleUnitState`，可从 `SummonUnitsActionPayloadDef` 投影基础技能与天生武器，占用 `BattleState`/grid 并通过 `BattleAiBlackboard` 记录来源单位、装备实例、binding、state key 与过期 TU。
