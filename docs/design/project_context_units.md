...
- 边界：`TraitDef` / `TraitContentRegistry` / `data/configs/traits/*.tres` 是通用 trait 内容定义边界，source scope、effect/stack/charge/roll schema 等固定值由 `TraitContentRules` 校验（trait effect 配置须显式 typed 字段）。跨表校验消费已经投影出的 `SkillDefinition` 索引；raw `SkillDef` 只在资源加载、字段校验、`SkillDefinition.FromResource(...)` 这类明确 Resource-boundary 触达。
...
- 桥接：通用 trait 进战斗为 `IBattleRuntimeCharacterGateway.BuildEffectiveTraitProjectionForEquipmentView(...)` → `BattleUnitFactory` → `BattleUnitState.effective_trait_instances/effective_trait_ids`（以 battle-local `EquipmentState` 重算，战斗规则不反查 trait/item catalog 或角色装备源）。装备能力进战斗为 `GameContentCatalog.GetEquipmentAbilityBindingDefinitionsTyped/GetTraitDefsTyped` → `BattleRuntimeModule` typed 索引 → `BattleEquipmentAbilityProjectionService` → `BattleUnitState.equipment_ability_sources`；敌人经 `EncounterRosterBuilder` 投 battle-only source；战斗规则只从 `BattleUnitState` 读装备能力源与 `creature_type_tags`。
