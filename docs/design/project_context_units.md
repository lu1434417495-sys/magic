...
- 文件：
  ...
  - `scripts/systems/content/skills/SkillResourceProjectionAdapter*.cs`（仅 synthetic/test）
...
- 历史物理拆分（零行为变化，2026-07-19/20）：`SkillContentRegistry` 的 combat profile/effect 校验拆分为 `SkillCombatProfileValidator`（`TypedEffectParamTargets` 映射表随迁）、伤害效果/耐久/路径 AoE 校验拆分为 `SkillDamageEffectValidator`、execute/graded-save/temporal/save-bonus 校验拆分为 `SkillExecuteEffectValidator`（`GradedSaveExecuteParamKeys` 随迁）；这些 Resource validator 自 2026-08-19 起不再属于 production publication gate，仅服务尚未迁移的 synthetic Resource 回归 helper。equipment ability 的 DTO/payload shape 由 strict parser 拒绝，closed vocabulary 由 `EquipmentAbilityImportVocabularyValidator` 负责，binding/reference 业务校验由 `EquipmentAbilityBindingValidator` 负责，ImportModel→Definition 唯一投影归 `EquipmentAbilityDefinitionProjection`。
- 技能等级值图边界：`CombatSkillLevelOverrideImportModel` 以 nullable typed 字段保留“缺席”与显式零值差异，closed 字段在 JSON parser 边界映射为 CLR enum。`SkillJsonImportParser` 对 level key、null、空 override、字符串值和 closed business string fail closed；effect payload 的 parser-only carrier 不得逃逸到 import model。synthetic Resource fixture 如需构造 JSON 无法表示的非法状态，只能经测试专用 `SkillResourceProjectionAdapter` 同步投影。
...
- content JSON 迁移边界：一次性 TRES converter、Resource adapter 与其 regression 在对应域完成 canonical parity 后删除；正式 authoring、runtime 与长期作者工具不再依赖 converter。当前 skills、items、traits、equipment abilities、gear sets、recipes 都从 strict JSON direct-load，迁移清单仅保留需要人工核对的 item icon 旧路径→asset ID 证据。
