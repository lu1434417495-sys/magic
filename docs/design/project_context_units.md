...
- 静态 gameplay/config 内容经各 `*ContentRegistry` 从 code-owned strict JSON 载入、校验并投影为 typed 定义，再由进程级 `ContentSnapshot` 冻结发布；session、catalog、runtime 与 BattleSim 只借用 typed definition 索引。Godot Resource authoring 只保留 engine-asset catalog。
...
- 文件：
  ...
  - `scripts/systems/content/SkillIconAssetCatalogValidator.cs`
  ...
  - `scripts/systems/platform/NativeLeaseScope.cs`
  - `scripts/systems/platform/GodotProjectionLease.cs`
  - `scripts/systems/platform/RuntimePlainPayload.cs`
  - `scripts/systems/platform/GodotObjectLifecycle.cs`
  - `scripts/systems/platform/GameLog.cs`
  - `scripts/systems/platform/GameLogSinks.cs`
...
- 内容快照边界：`ApplicationLifetimeCoordinator` 拥有唯一 `ProcessContentHost`；host 先从 `EngineAssetCatalogBootstrap` 的唯一代码路径加载并锚定 typed engine-asset catalog root、发布 `asset_id -> borrowed typed Resource` 只读索引，再调用 `ContentSnapshotBuilder`。所有 gameplay/config registry 只从 code-owned strict JSON discovery 进入 plain ImportModel/Definition；builder 在同一个同步作用域完成校验与 typed 投影，`ContentIconAssetCatalogValidator` 与 `EnemySpriteAssetCatalogValidator` 随后在同一 publication project 内确认 item/skill icon 和 enemy sprite 的非空 asset ID 已登记且目标为 `Texture2D`，全部成功后才 seal 跨 session 复用的 immutable `ContentSnapshot`。资产校验失败不能发布 snapshot、epoch 或 sealed 状态。`ProcessContentHost` 不拥有 content root map、Resource loader/discovery、adapter 或 authored-path reverse index，只保留 snapshot publication 与 engine-asset owner。`GameSession`、`GameRoot`、`GameContentCatalog`、world/battle runtime 与 BattleSim 不持有 raw registry、JSON DTO/import model 或 authored Resource mirror；production lifecycle 的 legacy debt 必须保持为零。
- engine-asset 解析边界：内容消费者只以 `StringName asset_id` 查询已发布 catalog，代码自有 scene/shader/bootstrap 路径只走分名的 `res://` 入口；item `icon_asset_id`、skill `icon_id` 与 enemy `battle_sprite_asset_id` 不得回退到内容 id、拼接内容路径或经过 authored-path migration seam。`data/configs/` 只有 engine-asset catalog 使用 Godot Resource authoring。
...
- 持久化边界：`SaveRepository` 拥有底层 save 文件 IO，`GameSession` 拥有 active save / schema / meta / index 归并；原子替换成功的 `<save_id>.dat` 是唯一权威提交点，`index.dat` 只是可从合法 payload 重建的派生缓存。payload 提交后 index 写失败只记录稳定 warning、失效 session cache 并等待后续列表读取重建，不得向调用方报告保存失败或触发运行时事务回滚。save/slot-index 的 session cache 与读回结果保持 plain C# graph，写入时才创建 Request-domain `GodotProjectionLease`，每个 nested collection 由同一 lease 显式拥有；`FileAccess` / `DirAccess` 由 Request-domain `NativeLeaseScope` 拥有，并在 remove/rename 前显式关闭文件句柄。`GetVar(false)` 结果必须在 file/Variant 仍存活时立即还原为 plain/typed state，不让 raw Godot payload 逃逸。`world_data` 的 runtime owner 是 `WorldRuntimeData`，只在 save payload 入口/出口投影。子 payload 破坏性 schema 变化时同步升级 owning save version，且只接受当前版本、不做 legacy 兼容迁移。`GameSession.SetPartyState` 以存档往返校验（`SaveSerializer.NormalizePartyState`）作为写入闸门：往返失败返回 `Error.InvalidData`、保留上一份 `_party_state` 并记录 `session.party.normalize_failed`，绝不替换成空队伍再报成功——`RuntimeTransaction` 会因该错误码跳过 `CommitRuntimeState`，避免把清空后的队伍落盘。存档解码的每条拒绝路径都必须带 `SaveDecodeResult.RejectionReason`（`save_version_mismatch` 等），读档失败时由 `session.save.load.decode_rejected` 记录；开发期 SaveVersion bump 造成的弃档因此可与真正的存档损坏区分。索引重建跳过任一存档文件时必须记录 `session.save.index.rebuild_skipped`，不允许静默 `continue`——否则存档文件仍在磁盘上却从读档列表消失且无任何线索；save 目录存在但打不开属于真实 IO 故障，直接抛出而非返回空列表。存档解码链上的 `FromDictionary`（`PartyState` / `PartyMemberState` / `QuestState` / `WorldRuntimeData` / `UnitBaseAttributes` / `UnitReputationState`）一律提供 `out string failureReason` 重载：无参重载保留 null 返回给既有调用点，带 reason 的重载沿解码树逐层拼接失败字段路径，最终由 `SaveDecodeResult.RejectionReason` 带到 `session.save.load.decode_rejected`。新增解码分支必须给出自己的 reason，不允许再出现裸 `return null`。
...
- 世界唯一装备持久化边界：`WorldRuntimeData` 可选持有 `WorldUniqueEquipmentPoolState`，其 `reserve/shop` location 与原始 `EquipmentInstanceState` 一起进入 `unique_equipment_pool`；`GameSession.InitializeNewWorldUniqueEquipmentPool()` 只由新档创建链调用。缺少该可选字段的旧档不回填；save decode 只校验 pool 自身的当前结构，不跨仓库、角色装备或其他位置扫描/去重实例。
...
- 物品内容边界：`ItemContentRegistry` / `RecipeContentRegistry` 只从 strict JSON 进入 plain ImportModel，再投影递归只读的 `ItemDefinition` / `RecipeDefinition`。`ContentSnapshotBuilder` 显式先构建 item、再把 item definition 索引传给 recipe registry。`GameSession`、`GameContentCatalog`、runtime、battle、settlement 与 UI 只借用 definition 索引，不保留 raw registry mirror，也不把 definition 回投为 Godot Dictionary。
- 适合：save schema、序列化、内容接入、全局注册表问题。
- 邻接单元：CU-01、CU-03、CU-04、CU-10、CU-11、CU-13、CU-20、CU-21。

#### Unique World Equipment Pool（世界唯一装备池）

- State/save owner：`WorldRuntimeData` → `WorldUniqueEquipmentPoolState`；根字段 owner：`WorldRuntimeSaveSchema.UniqueEquipmentPool`。
- 新档 mint owner：`GameSession.CreateNewSave(...)` → `GameSession.InitializeNewWorldUniqueEquipmentPool()`；旧档 load 不调用该入口。
- 商店转移 owner：`SettlementShopService` + `GameRuntimeServiceWindowCommandHandler`；`BattleRuntimeLootResolver` 只冻结随机装备 request，随机装备实例选择与转移 owner 是 `GameRuntimeFacade.BattleLootPort` + `GameRuntimeBattleLootCommitService`。
...
- 边界：`WorldContentRegistry` 是三个 world JSON 域的唯一导入/投影入口；它在 seal 前解析 `shared_content_id`、preset/mounted generation ID，检测缺失引用与 generation cycle，并生成完整 immutable definition graph。据点名称池以 `SettlementTierKind` 为唯一键，重复 tier 拒绝发布；野怪规则以 `WorldVerticalBandKind` 分带，procedural 与 guaranteed-starting 路径均按 typed band 选择，不读取数组位置。`WorldMapContentValidator` 的正式入口只校验 typed graph；`GameSession` 只保存 `world_generation_id` 并借用 snapshot 中的 definition，`WorldMapDataContext`、spawn/runtime/facade/UI 不加载或保留 authored DTO、import model、Resource 或路径。旧 `.tres` world authoring 与路径 fallback 已删除。
...
- 文件：
  ...
  - `scripts/systems/world/WorldMapCellData.cs`
  - `scripts/systems/world/VisionSourceData.cs`
- 负责：世界格网、坐标、视野来源、迷雾状态。
- 边界：`WorldMapFogSystem` 以 CLR Dictionary 管理 faction state，`WorldMapFogFactionState` 以 `HashSet<Vector2I>` 持有 visible/explored 坐标；persistent revision 跟踪需要持久化的 fog graph 变化，普通可见性重建不把 fog snapshot 写回 world owner。`fog_states` v2 的 `explored` / `revealed` plain 数组直接保存原生 `Vector2I`，save schema 与 runtime reader 都拒绝字符串或 `{x,y}` Dictionary 坐标。`WorldMapDataContext` 以 owner/map/revision gate 在 save、地图切换和事务边界才调用 `BuildPersistentStatePlain()`，并与 `WorldRuntimeData` 的 fog canonical state/save snapshot 保持递归 plain；事务回滚必须从 root owner 重新加载 active context/fog。Godot Array/Dictionary 只在 fog 的同步 setup/load adapter 或 CU-06 world projection 边界短暂出现，不能缓存回 fog system。
...
- 仓库保存边界：`GameRuntimeWarehouseHandler` 只弱借 `IGameRuntimeWarehousePort`，消费 detached command/window snapshot 并提交 typed intent，不直接读取 `PartyState`、warehouse/item-use service、session、world context 或 catalog。直接加入、丢弃和技能书使用的 service mutation、party stage 与失败 rollback 归 facade port；成功后只把当前 `PartyState` stage 到 `GameSession` 并标记 `party_state` pending dirty，不立即调用完整存档提交，返回标题、runtime dispose、主动保存或世界卸载等 canonical flush 再同步全部 owner 并落盘。facade-private warehouse mutation snapshot 只处理 session staging 失败，原子恢复 runtime/party/world/selection owner，不把磁盘 I/O 失败解释为库存操作失败。
...
- Owner 拆分：主 handler 保留 runtime/modal 可见性查询、命令分发、事务、持久化与 facade 桥接；`SettlementActionPayloadBuilder` 拥有 typed request 与 boundary/dispatch/validation payload 转换，`SettlementActionValidationPolicy` 拥有 service found/enabled 决策；合同板+悬赏板(WIP)在 `GameRuntimeContractBoardCommandHandler.cs`、NPC 任务面板在 `GameRuntimeNpcQuestOfferCommandHandler.cs`、商店/锻造/驿站窗口在 `GameRuntimeServiceWindowCommandHandler.cs`、据点窗口数据构建在 `GameRuntimeSettlementWindowDataBuilder.cs`（主 handler 构造函数统一接线）。
- Recommended reads before changes:
...
- 文件：
  ...
  - `scripts/player/warehouse/ItemPriceRules.cs`
  ...
  - `scripts/systems/content/items/*.cs`
  - `scripts/systems/content/recipes/*.cs`
  - `scripts/systems/content/gear_sets/*.cs`
  - `scripts/systems/content/ContentSnapshot.cs`
...
- 细节文档：
  ...
  - `docs/design/foundations/content_json_authoring.md`
- 负责：共享背包、堆叠/容量、物品与配方定义、装备实例、装备/卸装、物品使用。
- 边界：`WarehouseState` / `EquipmentState` / `EquipmentInstanceState` 家族是 plain C# runtime/save owner。item、weapon profile/dice、trait-roll group、equipment requirement 与 recipe 的 production authoring 只存在于 JSON；strict DTO 经 plain ImportModel/domain validator 和唯一 projector 进入递归只读 `ItemDefinition` / `RecipeDefinition`。133 个 item 已展开旧 31 个 template，production 不执行模板 merge，也没有 Resource adapter、路径 provenance 或 Resource→Definition fallback。`ContentSnapshotBuilder` 先构建 item definition，再通过 `RecipeContentRegistry.Setup(itemDefinitions)` 显式编排 recipe 引用；session、runtime、battle、settlement 与 UI 只借用同一个 definition 索引。item 的非空 `icon_asset_id` 在 snapshot publication 前必须解析为已登记 `Texture2D`，UI 只走 typed asset-ID resolver。物品价格的 basis-point 缩放由 `ItemPriceRules` 唯一实现，以 `long` 完成乘法和 half-up 舍入并饱和到 `int`。物品内容（category / equipment type / 伤害标签 / 固定 trait ids / trait roll groups）跨 trait 规则由 `ItemTraitContentValidator` 校验；装备 trait mint 时机由 `EquipmentRules` / `EquipmentTraitRollService` 拥有。套装不是持久装备类型；`GearSetEvaluationService` 只从当前 `EquipmentState`、只读 item/set definition 计算不同有效成员、阈值属性和阈值 trait，结果不写入装备或存档。
- 唯一实例与背包边界：凤凰十件只在新档初始化时由 `GameSession` mint；进入商店、战利品或共享仓库后仍是同一 `EquipmentInstanceState.instance_id`，购买/出售不得重置耐久或 trait roll。通用 `world_unique_equipment` 内容 tag 由 `WorldUniqueEquipmentContentRules` 识别，`PartyWarehouseService` 对 raw item-id mint、空 instance id 和 force-new id 路径 fail closed，只接受已经具有稳定实例 id 的转移。`WarehouseState` 不承担全世界唯一性扫描，`WorldUniqueEquipmentPoolState` 也不扫描玩家持有位置；系统保证的是正常生成链只创建一套，不拒绝玩家手改存档产生的额外实例。
- 仓库窗口边界：`GameRuntimeWarehouseHandler` 只构建一次 `WarehouseWindowData`（标题/摘要/状态文案 + 借用的 `WarehouseWindowSnapshot`），窗口直接消费 typed snapshot 条目，headless plain 快照由同一 DTO 单向投影；presentation 文案不下沉进 `WarehouseWindowSnapshot` 领域快照。
...
- 文件：
  ...
  - `scripts/player/progression/ContingencyContractRules.cs`
...
- 边界：`PartyState.member_states` owner 是 `PartyMemberStateCollection`；任务所属阶段由 `PartyState` 内部的 `QuestJournalState` 唯一拥有，active、claimable、failed 与 rewarded id 的迁移必须经该聚合原子完成，任务查询只返回 detached `QuestState`，不能凭外部引用改写 canonical journal。`PartyMemberState` 持有 `ContingencyMatrixSetupState`（只承载战斗外 setup / 充能 / 消耗回写事实，不承载战斗内 release queue / hook）。contingency trigger/timing/target-resolver 的闭集映射与精确字段集合由 `ContingencyContractRules` 唯一声明，runtime state decoder 与 authoring definition projection 共同消费，不能分别维护字符串分支。其余成长、任务、奖励状态都是 plain C# runtime/save DTO；`PendingCharacterReward` / `PendingCharacterRewardEntry` 与其 payload codec 同归 `scripts/player/progression/` 的 Party save graph owner，奖励生成与应用逻辑仍留在 progression service。`PartyState.BuildSaveSnapshotPlain()` 是 Party save graph 的 canonical 序列化源，Godot Dictionary 边界投影也从该 plain snapshot 构建，避免并行 schema 源。角色 trait 与装备 roll trait 的持久实例边界是 `TraitInstanceState` / `TraitInstanceCollection`：成员实例只承载 `character` source、装备实例只承载 `equipment_roll` source，反序列化按严格字段集与 source kind 校验。
...
- 文件：
  ...
  - `scripts/player/progression/gear_sets/*.cs`
  - `scripts/systems/content/traits/*.cs`
  - `scripts/systems/content/equipment_abilities/*.cs`
  - `scripts/systems/content/gear_sets/*.cs`
  - `scripts/systems/content/generation/*.cs`
  - `scripts/player/progression/QuestContentValidator.cs`
  - `scripts/systems/progression/*ContentValidator.cs`
  - `scripts/systems/content/ContentSnapshot.cs`
  - `scripts/systems/content/ContentSnapshotBuilder.cs`
  - `scripts/systems/content/SkillIconAssetCatalogValidator.cs`
  - `scripts/systems/content/ItemIconAssetCatalogValidator.cs`
  - `scripts/systems/content/skills/BattleAttackRollModifierSpec.cs`
  - `scripts/systems/content/skills/CombatSkillAttackDefenseMode.cs`
  - `scripts/systems/content/skills/PendingCastBindingModeKind.cs`
  - `scripts/systems/content/skills/SkillTypedLevelValueModels.cs`
  - `scripts/systems/content/skills/SkillJsonImportContracts.cs`
  - `scripts/systems/content/skills/SkillJsonImportParser.cs`
  - `scripts/systems/content/skills/SkillDiagnosticFixtureProjection*.cs`（仅 diagnostic/test）
  - `scripts/systems/content/skills/SkillCanonicalJsonSchema*.cs`
  - `scripts/systems/content/skills/SkillImportCanonicalJson.cs`
  - `scripts/systems/content/skills/SkillContentJsonAuthoringDomain.cs`
  - `scripts/systems/content/skills/generation/*.cs`
  - `scripts/player/progression/CombatRangedWeaponReactionDef.cs`
  - `scripts/player/progression/SkillDefinition.cs`
  - `scripts/player/progression/definitions/CombatEffectPayloadDefinition.cs`
  - `scripts/player/progression/CombatChainDamageDefinition.cs`
  - `scripts/systems/content/skills/SkillDefinitionProjector.cs`
  - `scripts/systems/content/skills/SkillDefinition*Validator.cs`
  ...
  - `data/configs/json/equipment_abilities/*.json`
  - `data/configs/json/gear_sets/*.json`
  - `data/configs/json/recipes/*.json`
...
- 内容图标边界：`SkillDefinition.IconId` 与 `ItemDefinition.IconAssetId` 只表示 engine-asset catalog ID；空值明确表示无图标。process snapshot 发布前由 `ContentIconAssetCatalogValidator` 校验所有非空 ID 已登记为 `Texture2D`，不把资源路径或 Godot wrapper 带入 definition graph。
- 边界：技能、items、traits、equipment abilities、gear sets、recipes、身份/成长、任务、信仰、屏障、特殊技能 profile、contingency template 与 `gameplay_configuration` 的 production authoring 均从 code-owned JSON 经 strict DTO、plain import model、domain validator 与唯一 projector 发布 immutable Definition；JSON 跨域关系只保存内容 ID，不保存 Resource path、类型名或 authored Resource。`GameplayConfigurationDefinition` 集中拥有成就、据点商店、新游戏队伍/起始武器标签规则及生成 BattleSim 基准角色；`ProgressionContentRegistry`、`GameSession`、`SettlementShopService` 与生成门禁不得另存 seed、固定 ID 或按 ID 前缀推断行为。任务失败后的重启规则由 quest closed import kind（`terminal/restartable`）唯一拥有，`QuestDefinition` 只向上层投影 `CanRestartAfterFailure` 语义事实，不能用 `is_repeatable` 代替。contingency template 投影/充能公式归 `ContingencyContentRules`；技能的特殊运行时分派由 closed `runtime_behavior` 投影为 `SkillRuntimeBehaviorKind`，不能按 `skill_id` 文本识别；时间状态 id/tag/release-effect 判定归 `TemporalStatusContentRules`。各 JSON registry 只存在于 `ContentSnapshotBuilder` 的同步构建作用域；跨表校验完成后 process host seal 一个共享 snapshot。`BarrierSkillContentValidator` 在 seal 前用 projected definition 索引双向校验 `layered_barrier` 的 profile 引用与屏障层 breaker skill 引用。session/runtime/UI 只借用同一 epoch 的 immutable Definition graph。只供通用运行时内部结算引用、不得进入任何学习路径的技能使用 `learn_source = internal`。
- gameplay configuration 组合边界：`ContentSnapshotBuilder` 单点导入 `gameplay_configuration`，先暴露其诊断，再把成就 Definition 注入 progression registry。`GameplayConfigurationDefinition.BattleSkillRoles` 是基础攻击角色的唯一配置源；enemy registry、encounter builder、battle runtime、AI、mastery、summon 与生成门禁只消费该 snapshot 投影，不以技能 ID 字面量分派。contingency 的 charge material item 引用由 `ContingencyTemplateCrossDomainValidator` 在 snapshot seal 前校验。
- 技能加载/校验/投影边界：`SkillContentRegistry` 只从 `data/configs/json/skills` 导入 family JSON，负责 import/definition 索引和错误编排。`SkillJsonImportParser` 是 production source 到 `SkillImportModel` 的唯一入口；业务规则唯一从 `SkillImportModelValidator` 进入 Definition validator 组，Definition 字段映射唯一归 `SkillDefinitionProjector`。正式 session/catalog/runtime 只读 process `ContentSnapshot.Skills`。`SkillDiagnosticFixtureProjection` 只把测试专用 `RefCounted` fixture 同步映射到相同 import model，再复用同一 normalizer 与 Definition validator；它不属于 authoring、加载或 publication gate。
...
- 施法变体边界：附加配置由 strict import model 经唯一 `SkillDefinitionProjector` 映射为 `CombatCastVariantDefinition` 的不可变 typed 值；地面范围服务的 state/read-view 入口共享该定义，不另存字符串键参数包。字段契约见 `docs/design/battle/skill_runtime.md`。
...
- Shield 内容边界：护盾家族、属性调整值与共享/逐目标投骰分别由 `CombatEffectDef.shield_family`、`shield_attribute_modifier_id`、`shield_roll_per_target` 投影到 immutable `CombatEffectDefinition`；属性只允许六项基础属性的 modifier id，旧 `params.shield_family`、非 shield effect 携带护盾字段以及无骰池的逐目标投骰均由 `SkillDefinitionCombatProfileValidator` 拒绝。护盾数值、替换、preview 与 AI 边际收益归 CU-16。
- 地形移动接触内容边界：移动拦停使用 closed `terrain_contact_mode = interrupt_movement_on_failed_save`，有效触发数、接地要求、格内起步重判、同源实例上限与替换策略均由 `CombatEffectDef` typed 字段投影到 immutable `CombatEffectDefinition`；完整范围、空地、通行与垂直于目标向量的 line 布置由 `CombatSkillDef` typed 字段声明。内容校验拒绝未知模式、缺失豁免、非正触发数、混入伤害/状态 payload 或不一致的实例策略，不在 `params`、描述或技能 id 中编码。
- 索钩登高内容边界：地面登高位移使用 closed `forced_move_mode = grapple_ascent`；水平上限继续由 `forced_move_distance = 1` 表达，垂直上限必须走 `CombatEffectDef.grapple_max_height_gain` → `CombatEffectDefinition.GrappleMaxHeightGain`。逐级高度变化使用互斥的 effect `min_skill_level/max_skill_level` 窗口，不把高度塞进 `params`、射程或技能 id 分支。
...
- 历史物理拆分（零行为变化，2026-07-19/20）：原 registry 的 combat profile/effect 校验拆分为现行 `SkillDefinitionCombatProfileValidator`、`SkillDefinitionDamageEffectValidator` 与 `SkillDefinitionExecuteEffectValidator`；这些 validator 现只接受 plain import/definition graph。equipment ability 的 DTO/payload shape 由 strict parser 拒绝，closed vocabulary 由 `EquipmentAbilityImportVocabularyValidator` 负责，binding/reference 业务校验由 `EquipmentAbilityBindingValidator` 负责，ImportModel→Definition 唯一投影归 `EquipmentAbilityDefinitionProjection`。
- 技能等级值图边界：`CombatSkillLevelOverrideImportModel` 以 nullable typed 字段保留“缺席”与显式零值差异，closed 字段在 JSON parser 边界映射为 CLR enum。`SkillJsonImportParser` 对 level key、null、空 override、字符串值和 closed business string fail closed；effect payload 的 parser-only carrier 不得逃逸到 import model。若 diagnostic fixture 需要构造 JSON 无法表示的非法状态，只能经测试专用 `SkillDiagnosticFixtureProjection` 同步投影到同一 import model/normalizer；不得恢复 Resource loader、TRES adapter 或第二套业务映射。
- 技能 JSON 作者工具边界：`SkillContentJsonAuthoringDomain.CreateImportDescriptor(...)` 是 document load、file-local template merge、strict parser、identity normalize 与 domain-local validation 的唯一组合入口；schema exporter 从完整 `SkillJsonDto` 生成 full entry、schema-only `template` control 与递归 partial template view。阶段 2 完成后转换器和 `.tres` source 已删除，family JSON 是唯一 production authoring truth。
- CombatSkill 边界：`CombatSkillDef.attack_resolution_mode`（authoring）与 `SkillDefinition.AttackResolutionModeKind`（runtime DTO）唯一由 `BattleSkillResolutionRules` 解释为 direct effect / fate attack / force-hit-no-crit。`CombatSkillDef.attack_defense_mode` 经 `CombatSkillAttackDefenseMode` 封闭投影为 `normal/touch/flat_footed`，只声明该技能攻击检定所对抗的 AC 组成，不从投射物种类、职业、标签或技能 id 推断。`CombatSkillDef.projectile_kind` / `CombatCastVariantDef.projectile_kind_override` 由 `CombatProjectileContentRules` 校验并投影；技能级合法值为 `none/nonmagical/magical/current_weapon`，变体空值表示继承且可用显式 `none` 关闭。装备必中主动技能应配 `attack_resolution_mode = &"direct_effect"`，再由 combat effect 的 target/status requirement 与 `BattleDamageResolver` 决定是否生效，不在装备能力 service、技能 id、状态 params 里硬编码“必中”。`CombatWindupDef` / `CombatWindupDefinition` 只声明蓄力每挡费用、技能等级挡位上限、基础武器骰倍率与每挡武器骰增量；`CombatDirectionalPiercingDef` / `CombatDirectionalPiercingDefinition` 只声明逐级基础倍率、成功命中衰减下限、动态体力参数与允许高度差；`CombatApproachAttackDef` / `CombatApproachAttackDefinition` 只声明相对起始格的最大路径高度差，最大推进距离继续使用等级化 `range_value`，目标枚举使用 `current_weapon_plus_configured`。直线路径、绝对高度、阻挡、最短推进、正式武器攻击与 AI 最终位置评分归 CU-15/CU-16；力量/体质换算、蓄力挡位合法性、结算 snapshot 和打断语义同样归 CU-15/CU-16。
- CombatSkill 目标类型边界：`CombatSkillDef.excluded_target_creature_type_tags` 进入 import model 后由 `SkillDefinitionCombatProfileValidator` 校验非空且唯一，再由唯一 projector 投影为 immutable `CombatSkillDefinition.ExcludedTargetCreatureTypeTags`；目标合法性、canonical preview 与 AI 候选共同消费该排除集合，不能用单个 effect 的正向 tag 门禁或技能 id 特例替代。随机书技能候选以实际起始等级和一槽费用排除 MP 消耗超过 40 的内容，选中耗蓝技能后 `RandomStartingSkillResourceSupportService` 将随机法力池抬到至少可支付一次该技能。
- 主动施法反应内容边界：`CombatSpellReactionDef` 进入 import model 后经 `SkillDefinitionCombatProfileValidator` 校验并由唯一 projector 投影为 `CombatSpellReactionDefinition`，只拥有精确待机状态、触发投送类别、反应技能、所需武器族、维持豁免公式及逐级命中/DC 曲线；`SkillImportModelValidator.ValidateBatchMessages(...)` 还校验反应技能引用存在且是包含武器骰伤害的主动 unit skill。状态生命周期、触发时序、标准攻击结算、法术中断和 AI 收益归 CU-15/CU-16，不按具体弓手技能 id 分支。
- 远程武器受击反应内容边界：`CombatRangedWeaponReactionDef` 进入 import model 后经 `SkillDefinitionCombatProfileValidator` 校验并由唯一 projector 投影为 `CombatRangedWeaponReactionDefinition`，只拥有精确待机状态、允许武器族、反应伤害标签、独立攻击防御模式、逐级命中曲线、命中/未命中触发开关、单次层数消耗和暴击许可；法珠数量与持续时间继续由逐级唯一 active status effect 声明。主武器骰快照、触发时序、状态消费、正式反击、preview/HUD 与 AI 边际估值归 CU-15/CU-16，不按法师技能 id 分支。
- 装备能力内容边界：`EquipmentAbilityContentJsonAuthoringDomain` 只从 expanded JSON 导入 strict DTO；`EquipmentAbilityImportGraphMapper` 映射 plain ImportModel，`EquipmentAbilityImportVocabularyValidator` 复用 `EquipmentAbilityClosedVocabulary` / `EquipmentAbilityPayloadKindCatalog` 做 domain 闭集校验，`EquipmentAbilityDefinitionProjection` 是进入 immutable runtime Definition 的唯一投影。正式规模为 56 packs/173 bindings；有效词表为 3 condition + 27 action kind、35 fact ID、10 trigger、10 timing。trait、skill、windup skill 与外部 status catalog 是生产构建必填依赖；status 仍由系统声明、技能 effect、trait passive status 与全部 pack 创建型 action 组成两阶段目录，再校验 condition/fact/action 引用。召唤、附近单位、追击、时间进度、认知、减伤、伤害转治疗、状态栈、随机分支、临时边与成长状态都必须走对应 plain payload ImportModel→Definition，不从技能/装备名字推导。`grant_skill`、`on_battle_end`/`after_battle` ghost、全部 authoring Resource/TRES adapter/converter 与 Resource fallback 均已删除；battle/runtime 只接受 Definition/spec。
- 装备能力状态/aura/fatal/替换 ABI：binding 的 `activation_status_id` 只引用已声明状态，并把该状态投影为独立稳定能力来源；`mitigation_auras` 声明 aura id、格半径、阵营、damage tag 与 mitigation tier；fatal intercept 的 `success_actions` 只允许 `apply_status` / `trigger_skill`，其中创建状态也进入两阶段声明目录；`add_damage_dice` 的 `replacement_group_id/replacement_priority` 声明附伤 action bundle 的互斥组与优先级。四类字段都由 `EquipmentAbilityBindingValidator` / `EquipmentAbilityPayloadValidators` fail closed，并经 `EquipmentAbilityDefinitionProjection` 投影，不从状态名、技能名或装备名推断。
- 属性内容规则边界：`AttributeContentRules` 是 armor/shield/dodge/deflection/natural-armor 五种 AC component id、typed kind、双向映射、只读顺序与 membership 的唯一 owner；`AttributeModifierContentRules` 唯一拥有 flat/percent modifier mode 双向映射，authored Resource、typed definition、属性服务与装备服务共同消费，不由 `AttributeModifier` Resource 反向向 runtime 提供 converter。装备能力 authoring 校验和属性、world、battle 消费者共同依赖这些 content-definition 规则，不在 runtime service 或 validator 内复制白名单。
- 装备能力状态/后置动作边界：状态锁定（如 counterattack/guard/dodge bonus）、强制位移免疫等由 `ApplyStatusActionPayloadDefinition` typed 字段进入 runtime status；`after_skill` / `after_damage` 的直接伤害、治疗、清状态、按 fact 治疗、状态栈消费等继续消费 typed action Definition。有时限 target mark 自己持有剩余 TU 并随目标状态时间线推进；同一 `mirror_status_id` 只聚合展示，不合并多个来源时钟。到期通过 `on_target_mark_expired/after_status_expired` 分发一次反应；授予技能和击杀来源条件消费 shared fact query，不从授予技能 id 或武器 id 推导特例。
- 装备能力来源生命周期边界：`BattleEquipmentAbilityProjectionService` 纯计算 sources、binding 级 temporal modifiers 与 cognition ceiling modifiers，不原地改写单位；`BattleUnitFactory.RefreshEquipmentProjection(...)` 由 `BattleUnitEquipmentAbilityProjectionState` 深拷贝并一次提交三个组件，计算或提交失败时旧投影保持完整。认知上限 modifier 必须使用封闭 `BattleCognitionKind`，按来源装备 instance/binding 随完整投影建立和移除，不把派生有效认知写进持久状态。提交后才由 `BattleEquipmentAbilityRuntimeService`（经其持有的 `BattleEquipmentTargetMarkResolver`）按来源单位、装备实例和 binding 清理已经失去投影来源且声明 `remove_on_source_missing` 的 typed target mark；这类来源移除只重建或删除镜像状态，不分发自然到期反应。同一刷新边界还由 `BattleEquipmentStatusActionResolver` 清除声明 `remove_on_source_deactivated` 且 typed provenance（source kind + effective source key + binding）精确匹配失效来源的 status，provenance 是 `BattleStatusEffectState` 上的 battle-local 字段，不进状态字典 codec。正式换装直接消费刷新结果；装备耐久摧毁则由 `BattleDamageResolver.ResolveEffects/ResolveAttackEffects` 在完整效果结算后统一触发刷新，因此普通技能、冲锋/连击、ground effect、内部技能与装备立即攻击共享同一清理边界；可用的 `BattleEventBatch` 同步记录受影响的来源/目标单位。
- 装备能力 fact 来源隔离边界：`status_stacks` 可通过 `EquipmentAbilityFactQueryDefinition.RequireSourceUnitMatch` 要求目标状态的 `source_unit_id` 与当前装备能力来源单位一致；不匹配时返回 0。需要来源私有的命中、追加骰、消费或击杀条件时复用该 fact，不按具体状态或武器 id 增加运行时分支。
- 装备投射类别边界：`BattleEffectCategoryResolver` 只从 typed `projectile_kind` 与当前 cast variant override 派生 `projectile` + `magical_projectile/nonmagical_projectile`；这些派生类别不得直接写入 `delivery_categories/effect_categories`，也不得从职业、magic tag、射程、伤害、豁免或技能 id 推断。`current_weapon` 仅在本次 effect 含武器伤害且当前运行态与正式物品定义均确认主手为 ranged 时派生非魔法投射；显式 `magical` 不会再补成非魔法投射。可能随远程武器命中穿越屏障的毒素、石化等附加类别仍由 reaction 的 `projected_effect_categories` 显式声明，`EquipmentAbilityContentRegistry` 在加载期校验它覆盖可从 typed damage/save payload 推导出的类别；运行时不从具体武器 id 兜底推导。
...
- 文件：
  ...
  - `scripts/systems/progression/CharacterProgressionCapabilities.cs`
  - `scripts/player/progression/AttributeContentRules.cs`
  - `scripts/player/progression/ActionCadenceContentRules.cs`
...
- 文件：
  ...
  - `scripts/systems/battle/terrain/*.cs`
  ...
  - `scripts/systems/game_runtime/IGameRuntimeBattleLootCommitPort.cs`
...
- Typed 模拟开战边界：`BattleStartUnitRoster` 只接收调用方刚创建且不再保留的 mutable `BattleUnitState`，复制列表容器后由 `BattleRuntimeModule` 在 start 内一次性消费；空列表仍作为显式提供并进入既有开战单位校验，重复消费或同一阵营同时出现在 context canonical payload 与 typed roster 中会被拒绝。普通 `BattleSimRunner` 的 start context 只保留地形、出生点和选项，双方单位经该 typed 通道交接，runtime 不复制 unit graph，也不把它们重新 strict parse。`BattleSimFormalCombatFixture` 由 `BattleSimFormalRuntimeStartInput` 绑定 caller-owned context lease 与同批构建的 enemy-only roster：敌方保留 factory 已生成的 runtime-only projection，友方继续由 character gateway/party active roster 构造；context 必须完全省略 `enemy_units`，不能形成同侧双重真相。
...
- 反击编排边界：`BattleAttackActionCoordinator` 唯一拥有 reaction root batch、root/action ID、logical-action stack、depth/work guard 与最外层 drain；`BattleEffectExecutionContextService` 唯一拥有 effect-origin stack。`BattleDamageResolver` 只发布 immutable `BattleAttackResolutionFact`，`BattleCounterattackSystem` 按 `(ActionId, attacker, defender)` 去重并 FIFO 排空，`BattleCounterattackQueryService`/`BattleCounterattackRules` 在 drain 时复核 capability、单位、控制、反应预算和正式即时攻击 availability。counterattack origin 禁止直接再次触发反击，但保留 contingency/equipment 同步递归；command、timeline、AutoCast、repeat、charge、ground 与 equipment immediate 全部复用当前 root 的同一 `BattleEventBatch`。`BattleImmediateWeaponAttackService` 与 `BattleWeaponAttackOutcomeCommitter` 是 counterattack/equipment immediate 以及标准武器 outcome 的共享 typed owner；反击 definition availability 与正式施法共同消费 CU-16 的 `BattleSkillWeaponRequirementRules`，门槛失败发生在 attempt cost/chance RNG 前。preview 链不查询或投影反击 capability/readiness/risk，真实反击只由正式提交后的攻击事实触发。生产内容当前不自动安装 capability。
- Terrain/Fate capability 边界：`BattleTerrainEffectSystem` 只弱借 `IBattleTerrainEffectRuntime`，通过该端口读取 state/grid/damage resolver 并提交 changed facts、状态时钟、日志、击倒与贡献；`FateRuntimeModule` / `MisfortuneGuidanceService` 只借 `IMisfortuneGuidanceBattleQuery` 查询 calamity snapshot 与 reason。两个 domain owner 都不持有或接收 `BattleRuntimeModule` concrete composition root；显式端口实现集中在 `BattleRuntimeModule.DomainPorts.cs`，调用顺序仍保持同步。
- 地形移动接触运行时边界：`BattleGroundSkillValidationService` 对 preview/commit 共用完整面积、占用、footprint 与 layered barrier 校验；`BattleTerrainEffectSystem` 唯一拥有 timed field 的命令内去重、豁免、有效触发递减、同源替换与整组移除。普通 Move 失败仍按原选定路径全额支付并锁定；`BattleMovementService` 的技能步进、`BattleChargeResolver` 的冲锋和 `BattleSpecialSkillResolver` 的非 blink/jump 强制位移均在每步提交后调用同一接触 owner，格内起步按 authored 开关重判。飞行豁免只由接地开关决定，传送/交换/生成/复活不接入；来源倒下不清 timed field。canonical preview 通过 typed `BattleTerrainContactPreviewData` 暴露 DC、次数、持续时间和接触条件，熟练度只在失败实际拦停时由 terrain runtime port 提交。
- 装备移动轨迹边界：`EquipmentMovementTrailDef` 投影离开格、持续 TU、接触伤害与 usage；普通 Move 和装备冲锋都由正式移动提交生成 terrain trail。命令级 replacement 只保留最高优先级轨迹，避免同次冲锋重复留下普通与强化轨迹；接触伤害携带 `BattleEffectOrigin.EquipmentAbility()`，不会回流触发只接受外部伤害的装备反应。
- 战利品提交边界：`BattleRuntimeLootResolver` 对 `random_equipment` 只输出包含 `item_id / quantity / drop_luck` 的 typed request，不在战斗结算阶段预生成实例。`GameRuntimeBattleLootCommitService` 只弱借 `IGameRuntimeBattleLootCommitPort`，拥有掉落合并、灾厄碎片章节上限、随机装备请求提交、fatal/non-fatal 判定；facade port 在同时拥有 world unique-pool、party warehouse、session 与 equipment-drop service 的事务边界决定转移唯一原实例或生成普通装备，并拥有 opaque warehouse+fate+unique-pool checkpoint 的捕获与恢复。单条非法掉落只恢复 entry checkpoint 并记录丢弃，致命失败恢复整批 checkpoint；外围 battle finalization transaction 继续负责角色/世界 staging 与 flush 失败的完整回滚。
- Timeline/status owner 边界：`BattleTimelineDriver` 真实拥有 timeline phase、current TU、`tu_per_tick`、ready unit、action-threshold 校验/日志、stamina 与 Control objective 分数推进；`BattleStaminaRecoveryRules` 唯一提供 stamina tick 数、体质/恢复百分比与休息倍率换算，timeline 提交与 AI 只读未来投影共同消费，不再各写一套公式；`BattleUnitActionClockState` 唯一持有 unit action progress、threshold 与 action-rate remainder，并原子累计余数和扣除全部 crossing，`BattleTemporalStatusService` 只决定进度倍率与运行时状态操作，时间状态 id/tag 与 release-effect 闭集判定由 CU-13 的 `TemporalStatusContentRules` 唯一拥有；`BattleUnitCooldownState` 唯一持有 unit cooldown map 与 last-turn TU anchor，并原子执行 anchor 重基准、粒度判定与合法冷却推进；`BattleRuntimeSkillTurnResolver` 真实拥有 current-TU/静滞/turn-start 调度、非法粒度日志、turn timer 与状态周期 tick/duration/turn-start 规则，并按状态应用时的 current TU 初始化 `next_tick_at_tu`，同时在非静滞单位的 status phase 调度 `BattleUnitShieldState` 原子推进/到期清空护盾 duration。step 开始时已处于 `time_stasis` 的单位整步冻结护盾 duration，即使静滞在该 step 末到期也从下一 step 才恢复。`BattleRuntimeModule.MarkAppliedStatusesForTurnTiming(...)` 只保留“先初始化 tick anchor，再通知 Fate”的跨 owner 编排。timeline step 继续按 current TU → Control 区域归属/计分 → 状态 tick/duration → 场地时效 → pending cast → ready collection/sort 推进；unit activation 继续按 trait turn-start → cooldown/turn timer → metrics/contingency/sequential auto-cast → AP/移动点重置 → turn-start status/control 推进。
- Source-scoped 状态聚合边界：状态 state 保存各来源独立的层数、时长与 tick anchor，turn resolver 分别推进和结算；聚合层只供存在性、驱散、HUD 与通用查询。canonical preview 与 AI 使用同一纯合并语义计算候选来源的边际变化，来源倒下不会清除既有贡献，驱散聚合状态会一次移除全部来源。主动技能的默认高伤骰熟练度由 `BattleSkillMasteryService` 统一消费 damage result 的 4/5 理论最大值或暴击事实，并要求目标有实际 HP 伤害或护盾吸收；范围目标分别计量，状态周期伤害不再次入账。
- 蓄力 pending 边界：`BattleCommand.windup_tier` 只表达确认前的本次选择；确认后由 `BattleCastingTimeService` 把挡位、属性、总 TU、动态体力、武器骰倍率和武器签名冻结到 battle-only `BattleWindupSnapshot`。`BattleTimelineDriver` 对所有 pending 冻结 action progress，但只把普通读条单位排除在 stamina recovery 之外，因此蓄力单位在非静滞时继续按常规非休息规则恢复体力，完成 tick 亦相同。蓄力不可手动取消；本地硬控、施术者位移、武器变化或目标失效打断并启动完整冷却，HP 损失本身不触发维持检定。目标移动不打断，完成时由 canonical target validation 忽略已清零 AP 但重验其余目标规则，失败记为落空。
- 主动后撤边界：`BattleCommand.source_retreat_direction` 只表达玩家或 AI 对本次 unit skill 选择的精确单位正交方向；`BattleSourceRetreatRules` 以攻击前目标坐标、typed 距离、移动锁定状态、grid step 与 layered barrier 边界构造 preview/execution 共用的直线路径，物理阻挡只缩短位移而不拒绝攻击。`BattleMovementService` 提交实际路径而不扣移动力或记录第二次 action，仍应用经过地格的 terrain contact、changed coords 与 position-changed；手动选择由 `GameRuntimeBattleSelection` 分成目标/方向两阶段，AI unit-skill evaluator 枚举远离目标的方向后逐个进入 canonical preview。需要读条、蓄力、variant、special/random-chain 或自动选向的调用路径不支持该效果。
- 空中牵引边界：`CombatEffectDef.forced_move_mode = airborne_pull`、`forced_move_distance` 与 `forced_move_max_target_body_size` 经 immutable effect definition 投影；单次 `BattleCommand.forced_move_destination_coord` 只保存玩家或 AI 明确选择的最终落点。`BattleAirbornePullRules` 是 mutable/read-view 共用真相，统一检查目标状态、体型、强制位移免疫、曼哈顿距离、向施法者靠近、完整 footprint 空落点与 layered barrier；它有意忽略中间单位和普通高度差。正式 execution 直接迁移到落点，只在那里结算一次 terrain contact，再以通用 `forced_move_applied` 触发后置状态效果；canonical preview 通过 `BattleForcedMovePreviewData` 投影同一计划。`GameRuntimeBattleSelection` 只提供“目标后一次落点”流程；AI 枚举并评分目标-落点组合，评分事实归 CU-20，运行时不得按技能 id 自动牵引。
- 地面强风边界：`wind_push` 的距离、目标体型上限和豁免经 `CombatEffectDefinition` 投影；`BattleGroundRelocationService` 只收集风区内目标并按风向由远到近交给通用 `BattleSpecialSkillResolver`，范围外占位只能阻挡。`BattleWindPushRules` 用 detached state 生成逐目标失败分支落点、豁免概率和阻断原因，`BattleSkillPreviewService` / HUD / `BattleAiScoreService.ForcedMove` 共同消费，AI 按失败概率折算位移、接战、地形、高度和暴露价值。涉及该模式时，推荐同时读取 `CombatEffectDef` / `SkillDefinition` / `SkillDefinitionCombatProfileValidator`、`BattleWindPushRules`、`BattleGroundEffectService` / `BattleGroundRelocationService`、`BattleSpecialSkillResolver`、`BattleSkillPreviewService`、`BattleForcedMovePreviewData` / `BattlePreviewProjection`、`BattleHudAdapter`、`BattleAiGroundSkillActionEvaluator` / `BattleAiScoreService.ForcedMove` 与技能专项回归。
- 踏步攻击边界：`CombatSkillDefinition.ApproachAttack` 标识需要先推进再攻击的 single-unit 武器技能；`BattleApproachAttackRules` 以当前武器射程、等级化推进距离、正交目标、完整 footprint、grid step、layered barrier 与起始 anchor 高度构造 preview/execution 共用的最短直线路径。路径中每一步的全部占用格都按 profile 与起始高度做绝对差比较，`maximum_path_height_delta_from_origin = 0` 因而拒绝任何上坡、下坡或多格单位局部异高。合法性在付费前校验；付费后由 `BattleMovementService` 逐步提交且不扣普通移动力，terrain/barrier/位置变化造成的动态中断会取消攻击但不退款，成功到位后重新校验目标、武器与正式攻击距离，再进入标准武器攻击链。`BattlePreview.source_advance_path` 和 `resolved_anchor_coord` 向 HUD/AI 暴露同一计划；AI 强制走 canonical preview，并以既有伤害、资源与最终位置风险评分，不按技能 id 分支。
- 穿身攻击边界：`CombatSkillDefinition.LineThroughAttack` 标识以直线末端敌人为主目标的 typed 武器技能；`BattleLineThroughAttackRules` 用配置射程、最大允许武器攻击距离、正交关系、完整 footprint、阻挡边/layered barrier、途中单位阵营与敌后空落点构造 preview/execution 共用的有序计划。途中敌人各独立走标准武器攻击，未命中不停止；等级曲线只用整数武器骰组、非负攻击检定加值与成功命中 cap 增长终点的一次主攻击，途中结果不记录技能熟练度。终点命中才由 `BattleMovementService` 重新验证冻结路径和落点并直接提交穿越位移。`BattlePreview.target_unit_ids/source_advance_path/resolved_anchor_coord/hit_preview.Stages` 向 HUD 与 AI 暴露同一目标顺序、落点和 capped-success DP；AI 按阶段概率估值且不按技能 id 分支。涉及该 profile 时，推荐同时读取 `CombatLineThroughAttackDef/Definition/ContentRules`、`BattleLineThroughAttackRules`、`BattleSkillExecutionOrchestrator.LineThroughAttack`、`BattleSkillPreviewService`、`BattleAiScoreService.LineThroughAttack` 与对应技能回归。
- 单位换位边界：`CombatEffectDef.effect_type = position_swap` 经 `BattleEffectKind.PositionSwap` 投影并进入共享 unit execution allow-list；`BattlePositionSwapRules` 是 preview/commit 共用的目标、双方 footprint、占位、静滞、敌方强制位移免疫与 layered-barrier boundary owner，成功时原子交换两方 anchor，不走路径和地形接触。敌方换位使用 effect 自带的 caster-spell 意志豁免，友军自愿换位不豁免。`BattlePositionSwapPreviewData` 同时向 HUD 和 AI 暴露双方落点及成功概率，`BattleAiScoreService` 按目标威胁差、施法者新位置风险和豁免失败率评分。涉及该效果时，推荐同时读取 `CombatEffectDef` / `CombatSkillDef` / `SkillDefinition`、`BattlePositionSwapRules`、`BattleSkillTargetValidationService`、`BattleSpecialSkillResolver`、`BattleSkillPreviewService`、`BattlePreview` / `BattlePreviewProjection`、`BattleAiSkillAffordanceClassifier` / `BattleAiScoreService` 与技能专项回归。
- 索钩登高运行时边界：`BattleGridService.CanGrappleAscent(...)` 是 mutable/read-view 共用的落点真相，要求锚点正交相邻、目标支撑面比当前占用面至少高2层且不超过 effect 上限、落点可占用，并拒绝显式阻挡移动/占位的 edge feature；普通高度步行限制不参与该特例。`BattleGroundSkillValidationService`、`BattleGroundRelocationService` 与 `GameRuntimeBattleSelection` 按单位真实技能等级选取唯一 active effect，preview 与执行共用该规则；提交仍走地面强制位移、屏障边界与 changed-coord 链，不消耗移动力。
- 主动施法反应边界：`BattleSkillExecutionOrchestrator` 只在手动/AI 主动法术已提交 AP、资源与冷却后、应用法术效果前读取状态来源技能的 `CombatSpellReactionDefinition`，按 action progress 与稳定单位顺序执行当前武器仍合法且标准攻击目标校验通过的反应。反应射击复用正式基础攻击、命中、伤害、装备能力、死亡和日志链；只有实际 HP 损失才进入正式体质维持豁免，失败或击倒施法者会中断本次法术且不退款。未触发的后序待机状态保留；自动施法、Contingency、装备触发和 direct auto-effect 不调用该触发点。`BattleRuntimeSkillTurnResolver` 在反应者下一行动开始时清除仍未触发的待机状态，`BattleAiScoreService` 只按当前武器范围内已知主动法术威胁估值。
- 远程武器受击反应边界：`BattleDamageResolver` 在每个独立远程武器攻击检定前冻结实际主武器基础骰与本次 main W 倍率，在原攻击命中或未命中的全部正式结算完成后，经 `IBattleRangedWeaponAttackReactionSink` 同步通知 `BattleSkillExecutionOrchestrator`。orchestrator 只消费由状态来源技能投影出的 `CombatRangedWeaponReactionDefinition`，原子扣除配置层数后建立独立、禁止暴击的攻击检定，并把冻结骰池转换为配置伤害标签；不复制属性/固定加值、暴击骰、bonus W、装备附伤或其他段，也不重检原弹道距离与视线。动态伤害仍经过正式护盾、抗性、死亡和日志链，反应自身不含武器骰标记，因而不递归触发。`BattleRangedWeaponReactionPreviewBuilder` / HUD 公开公式，`BattleAiScoreService.RangedWeaponReaction` 只按持续窗口内可用弓弩威胁、法珠 cap 和现存法珠计算边际期望伤害。
- 桥接：通用 trait 进战斗为 `IBattleRuntimeCharacterGateway.BuildEffectiveTraitProjectionForEquipmentView(...)` → `BattleUnitFactory` → `BattleUnitEffectiveTraitState`（以 battle-local `EquipmentState` 重算，normal 写只接收 instances 并原子派生 ids；战斗规则不反查 trait/item catalog 或角色装备源），再由 `BattleTraitPassiveProjectionService` 将 trait typed passive 投影到 `BattleUnitSaveModifierState`、`BattleUnitDamageResistanceState` 与 `BattleStatusEffectCollection`，其中 trait 被动状态使用 `trait_passive_status` source layer 以便刷新装备投影时清理重建。套装阈值 trait 走同一 bridge，但保留 `gear_set_threshold` source、threshold-scoped effective key 和真实 usage-anchor 装备实例；换装刷新会从 battle-local equipment view 同步重算套装阈值、属性和能力来源。伤害抗性由 `BattleUnitState` typed gateway 访问；race/subrace 的后写覆盖、trait 的 stronger-only 覆盖与 strict codec 校验仍由各自调用边界持有，伤害规则以单 tag 查询读取 owner。装备能力进战斗为 `GameContentCatalog.GetEquipmentAbilityBindingDefinitionsTyped/GetTraitDefsTyped` → `BattleRuntimeModule` typed 索引 → `BattleEquipmentAbilityProjectionService` 纯结果 → `BattleUnitEquipmentAbilityProjectionState`；projection 会先以本次换装后完整有效 trait 集合复核 binding 的 `required_effective_trait_ids`，缺任一前置时不生成该 source 或 modifier，满足时仍保留原 trait/装备来源作为 usage owner。共同 owner 原子持有 sources、temporal modifiers 与 cognition ceiling modifiers，向 equipment runtime/技能入口提供不可变来源视图，并缓存 action/cast 各自按 `ModifierId` ordinal 最小、同 ID 首项优先的 temporal scalar 视图供 timeline/casting 零扫描读取，实际掷骰与属性读取仍留在 `BattleTemporalStatusService`。canonical/strict/detached 包含持久的 `cognition_kind` 与既有 `equipment_ability_sources` 字段，strict load 必须收到已知认知值且把 runtime-only temporal/cognition modifier 组件置空；不提供缺字段、未知字符串或旧 payload fallback。因此正式 encounter roster 启动直接消费 `EncounterRosterBuilder` 返回的 typed unit 列表，不以 canonical payload 作为 world→battle 内部传递通道。AI exact 用三个 stable key 分别保留三个装备投影组件的 raw 语义，并用 `cognition_kind` 覆盖基础认知。角色装备武器的 `range_type` 通过 `WeaponProjection` 进入 `BattleUnitWeaponProjectionState`，由不可变 read view 供装备能力 fact 读取；敌人经 `EncounterRosterBuilder` 投 battle-only source；战斗规则只从 `BattleUnitState` typed gateway 读 effective trait、伤害抗性、装备能力源、时间进度 modifier、有效认知、武器 range type 与 `creature_type_tags`。装备能力召唤的战斗内单位由 `BattleEquipmentSummonResolver`（`BattleEquipmentAbilityRuntimeService` 持有的 summon 职责拆分类）创建为 battle-only `BattleUnitState`，从 `SummonUnitsActionPayloadDefinition` 投影基础认知、基础技能与天生武器，占用 `BattleState`/grid 并通过 `BattleAiBlackboard` 记录来源单位、装备实例、binding、state key 与过期 TU。
- 装备能力运行时动作边界：`BattleEquipmentAbilityRuntimeService` 解析 before-hit / after-hit / after-skill / after-kill / after-damage / after-attack-check / after-status-expired 的 typed action payload 并写入 `BattleEventBatch` / `BattleState`，包括强制暴击来源、内部 `SkillDefinition` 结算、直接伤害、治疗、按 fact 治疗、清状态/target mark、状态栈消费、授予技能、召唤单位、召唤物消费、临时边特征、持久装备 counter 修改、持久状态同步与立即武器追击；其中召唤/召唤物消费职责拆分为 `BattleEquipmentSummonResolver`、target mark 生命周期职责拆分为 `BattleEquipmentTargetMarkResolver`、条件/fact 求值（condition group、compare_fact、nearby 计数、typed fact 读取）拆分为 `BattleEquipmentAbilityConditionEvaluator`、反应动作执行拆分为 `BattleEquipmentStatusActionResolver`（apply/clear status、状态栈消费）、`BattleEquipmentSkillTriggerActionResolver`（trigger_skill 内部技能）、`BattleEquipmentAreaActionResolver`（schedule area/terrain/edge feature）与 `BattleEquipmentDirectEffectActionResolver`（伤害/治疗/AP/耐久/add_damage_dice）、能力状态机拆分为 `BattleEquipmentAbilityStateResolver`（modify_ability_state、derived state 同步、once-scope/per-battle charge、persistent counter、state schema 查询）、攻击修正收集拆分为 `BattleEquipmentAttackModifierResolver`（attack roll/defense 修正、强制暴击、投射类别、bonus damage dice 收集、damage roll mode、固定减伤、loot 倍率），全部由主服务持有并在 `Setup` 相互接线（evaluator 与 executor 经主服务注入所需 resolver 引用），主服务保留事件入口、`ResolveActions` 编排、roll gate 与对外 internal 委托入口；有时限 target mark 的剩余 TU 存在 `BattleEquipmentTargetMarkState`，由 `BattleRuntimeSkillTurnResolver` 与目标状态时钟同步推进。`BattleAttackCheckPolicyService` 把强制暴击及来源装备实例/binding/action 写入本次 `AttackCheckInput`，未命中不升级；预览只有在 `CritLocked`/force-hit-no-crit 均未生效时才公开“命中后必定暴击”，命中后的击杀归因沿该来源进入 on-kill；`BattleDamageResolver` 在真实武器攻击检定提交后分发 after-attack-check，并把声明合并的内部技能结果并回父攻击；`BattleSkillExecutionOrchestrator` 负责装备技能 entry revalidation、usage commit，并向装备能力 runtime 提交通用目标结果摘要和 `BattleKillProvenance`；`BattleCommandPreviewService` 只做只读 preview/entry 校验。装备授予技能入口取 binding 固定 `skill_level` 与角色已学同名技能等级的较高值；来源绑定武器追加伤害通过伤害事件携带来源技能 id，只有角色已学该技能时才向该技能熟练度写入，不按具体武器分支。
- 状态激活与 fatal-success 运行时边界：`BattleEquipmentAbilityRuntimeService.CollectActiveBindings(...)` 把单位当前 `activation_status_id` 状态投影为 `battle_status:<unit_id>:<binding_id>` 来源，不依赖最初施加状态的装备仍然存在；状态到期/移除后来源自然消失。fatal 候选恢复成功后，主 service 才按声明顺序委托 `BattleEquipmentStatusActionResolver` / `BattleEquipmentSkillTriggerActionResolver` 执行 `success_actions`，并沿同一 `BattleState`、save context 与 `BattleEventBatch` 结算；preview 不执行这些动作。persistent fatal usage 由 `EquipmentAbilityUsageRuntime` 写回来源装备实例，`per_world_month` 的 period index 直接消费 `WorldTimeSystem.StepToMonth(world_step)`。
- 武器面板 overlay 边界：binding 级 `weapon_profile_overlays` 是 projection-only 改写，不作为 dispatcher action；`EquipmentWeaponProfileOverlayService` 是唯一合成点，玩家侧由 `BattleUnitFactory._apply_member_weapon_projection(...)`、敌方侧由 `EncounterRosterBuilder.ApplyEnemyWeaponProjection(...)` 在取得 base `WeaponProjection` 后调用，按 priority → 装备槽位顺序 → binding id → overlay id 稳定排序在 fresh base 副本上合成 range/dice/damage tag/grip 改写，再由 owner 单次 `ApplyWeaponProjectionTyped(...)` normalize 写回，换装/来源失效/重复刷新均从 base 重算，不叠加。overlay 的 `condition_group` 只接受 source 侧 `has_equipment_tag` 这类 projection-safe 条件，其余 condition kind 由 validator 以 `EQA_OVERLAY_CONDITION_NOT_PROJECTION_SAFE` fail-closed；非法 mode/grip/tag/clamp/dice 组合分别由 `EQA_OVERLAY_*` 诊断码拒绝。preview、range、AI threat range 只读合成后的 `BattleUnitState.weapon_*`，不各自重算 overlay。
- Reaction budget 边界：`BattleUnitReactionState` 唯一拥有 capacity、remaining charge、recharge interval 与未来 `NextRechargeAtTu`；`BattleUnitCounterattackCapabilityState` 唯一拥有 battle-local capability instances。`BattleReactionBudgetRules.InitializeUnitForAdmission` 在单位写入 `BattleState` 前初始化两个 owner；召唤 preview 在其克隆 state 上调用同一规则；clone、strict codec、canonical/detached 与 AI mutation-exact 保留 presence/raw 语义，但两者不进入持久存档。恢复粒度只消费 `BattleTimeRules.TuGranularity`（运行时与离线 JSON 校验共享的纯规则）；`time_stasis` 整步平移未来 recharge anchor，不追赶补 charge。
...
- 装备能力 AP 恢复边界：`ModifyActionPointsActionPayloadDefinition.Mode = restore_current_action_points_capped` 可在 after-kill 等通用 action 阶段恢复指定单位的当前 AP，并以该单位 `attribute_snapshot[action_points]` 为正常上限；已达到或超过上限时不增加也不反向降低。触发次数限制若需要必须由内容显式声明，运行时不默认附加 once-per-turn。
...
- 有序单位目标槽位边界：`CombatSkillDef.unit_target_resolution_mode = ordered_slots` 经 `CombatUnitTargetResolutionContentRules` 与 immutable `CombatSkillDefinition` 投影后，由 `BattleSkillTargetValidationService`、`BattleSkillPreviewService`、`BattleSkillExecutionOrchestrator.OrderedUnitSlots`、`BattleRuntimeSkillTurnResolver`、`GameRuntimeBattleSelection` 和 `BattleAiMultiUnitSkillEvaluator` 共同消费。重复单位 id 与手动顺序不得聚合；每槽独立走屏障/效果链，死亡后的既定槽位消散且不重选；法力和体力按实际槽位数线性定价，preview/commit/HUD/AI 共享 `BattleTargetSlotCostRules`。`BattleAiScoreService` 对该模式保留重复目标并逐槽累计收益，不能在 effect plan 后按单位 id 去重。`ProgressionService`、`RandomStartingSkillResourceSupportService`、`EncounterRosterBuilder` 与 `GameSession.CharacterCreation` 也必须通过同一规则读取最低一槽费用，不能把基础费用为零误判为不消耗资源。涉及该模式时，推荐同时读取 `CombatUnitTargetSlotRules.cs`、上述 runtime/AI/UI/progression owners、`tests/battle_runtime/skills/run_mage_arcane_missile_regression.cs` 与 `tests/battle_runtime/ai/run_enemy_multi_unit_skill_command_regression.cs`。
- 屏障与装备桥接边界：`BattleEquipmentAbilityRuntimeService` 只有在本次 effect 含武器伤害、能力来源匹配当前主手、weapon projection read view 的 range type 与正式 `ItemDefinition` 都确认为 `ranged` 时，才把 `current_weapon` 的 `projectile/nonmagical_projectile` 与 `on_hit/after_hit` 的 `projected_effect_categories` 合入 `BattleBarrierService` 的 unit effect 分类；显式 `none/nonmagical/magical` 由 `BattleEffectCategoryResolver` 直接解释，不接受手写派生类别。近战武器在读取任何装备附加投射类别之前即被硬排除，非当前主手、非武器伤害与 terrain effect 也一律不继承；选中 cast variant 沿 preview、commit、ground clip、repeat attack 与 chain damage 传入同一分类链。单位技能预览对手动排序/重复目标在一次 command 内共享 detached barrier preview session，以本地破层模拟保持逐槽顺序且不写真实 store；随机链继续保留真实抽样候选池，并单独记录经屏障过滤的 impact candidates，breaker 首击被挡后的候选只有在同一随机链仍存在可达后续命中时才进入 impact 集合。AI 的 unit/multi/random-chain fast preview 只在场上存在 layered barrier 时委托 canonical `PreviewCommand`，不得在 evaluator 内复制色层或装备类别规则；候选池与候选组上限只统计 canonical preview 后仍可影响目标的候选，不能让被屏障完全阻挡的前排目标耗尽上限。
...
- AI definition/runtime 边界：CU-20 的 enemy brain/action 与 score profile production authoring 只来自 JSON strict import graph；进入 runtime 的正式输入是 `EnemyAiBrainDefinition`、具体 `*ActionDefinition` 与 `BattleAiScoreProfileDefinition`。`BattleAiActionAssembler` 从 definition graph 构建 managed `BattleAiRuntimeActionPlan`，各 `BattleAi*ActionEvaluator` 拥有实际决策算法；测试 fixture 只构造 strict DTO/import model 或 immutable Definition，不保留 test-only Resource authoring mirror、duplicate/instance-id 路径或第二套 projector。一次 AI 选择的 decision lifetime 以 `BattleAiDecisionResult` 为交付边界：结果只交出 deep-copied decision/command/score/trace，capture 的 `finally` 立即 clear context，之后不保留 state、plan、score profile 或 nested Godot collection alias；mutation guard 使用 typed `BattleAiMutationSnapshot` 覆盖 unit/status/cell/terrain/barrier/blackboard、target mark、temporary edge 与 allocator 等完整权威面，在决策边界比较后只报告差异并失败退出，不承担 gameplay 状态恢复。技能与 barrier profile definition index 都防御性复制并只暴露只读视图，guard 按 canonical key、成员与引用 identity 校验；`SkillDefinition` 及其 combat/variant/effect 嵌套 definition 必须防御性冻结公开构造输入，不能把调用方 mutable collection 或 modifier alias 留进 runtime graph。`BattleAiSkillAffordanceClassifier` 按每个效果经 `BattleTargetTeamRules` 解析后的正式目标阵营分别聚合 hostile/support affordance；混合技能保留 `mixed`，profile 级 `any` 或第一个效果不能遮蔽其余分支。装备授予技能经同一个 `BattleSkillAvailabilityService` 进入 action plan、查询、正式预览和评分；plan fingerprint 以 closed `EnemyAiActionKind` 的稳定 content ID 表示 action 形状，并纳入稳定装备技能入口形状，来源换装会使下次决策重建计划，HP/usage 等动态 `IsSelectable` 变化只在决策时重查。action plan generation 在 clear/rebind/dispose 时显式关闭。
- Shield runtime/preview/AI 边界：`BattleShieldPreviewRules` 唯一计算 typed 属性调整值、动态骰面、护盾区间/期望值、家族以及相对现有护盾的期望净增量；`BattleShieldService` 正式执行共享同一规则并按 definition 选择单次效果共享投骰或逐目标独立投骰，不能按技能 id 分支。canonical preview 与 HUD 投影该只读摘要；纯 shield 技能归 support，AI 把每个合法友军目标的期望净护盾写入 typed score input，复用 `heal_weight` 评分且不计泛化 control/status 收益。
- 装备耐久 preview/AI 边界：`BattleEquipmentDurabilityResolver` 从 typed effect、当前 `EquipmentState` 与 `BattleSaveResolver` 统一计算非随机候选、稀有度豁免加值、预期耐久损失和摧毁概率；canonical preview/HUD 只投影 `BattleEquipmentDurabilityPreviewData`，无匹配装备时保留合法技能并显示零收益。AI 把两项期望事实写入 `BattleAiScoreInput`、fingerprint、decision clone 与 trace，分别复用 damage/status 权重评分；不能按技能 id 或“目标有装备”增加候选过滤。
- 豁免失败随机状态 runtime/preview/AI 边界：`BattleWeightedStatusOutcomeRules` 唯一拥有正权重求和、正式 roll 选择和 basis-point 预览分配；`BattleDamageResolver` 只在真实豁免失败后消费一次 combat RNG 并把选中的 typed 子效果交给通用状态链，成功豁免不抽取。canonical damage/skill preview 与 HUD 同时展示伤害分支、各结果的条件概率及实际施加概率，不能抽样或修改目标；AI 将豁免失败概率作为期望控制 basis points 写入 score input、fingerprint、decision clone 与 trace，不把随机池计作必然控制，也不按技能 id 评分。
- 状态解除/后继 runtime 边界：`BattleStatusEffectState` 保存 typed 行动跳过、正伤害解除、解除后 successor 与正常行动消费事实；`BattleUnitState.EraseStatusEffect` 统一执行解除和 successor 合并，`ClearStatusEffects` 仅用于整体清空且不生成后继。`BattleDamageResolver` 只在实际 HP 伤害或正护盾吸收后触发解除，0 伤害不触发，并将解除 id 投影到 execution 与 canonical damage preview；`BattleRuntimeSkillTurnResolver` 跳过声明状态的 activation，`BattleTimelineDriver` 只在正常 activation 结束后消费 successor。AI 对带豁免的普通控制把边际控制乘正式失败概率，现有免疫使收益与有效目标数归零。
- 地面拦停 AI 边界：`BattleAiGroundSkillActionEvaluator` 仍枚举通用地格候选并逐个调用 canonical preview；垂直 line 方向与正式目标收集共用 `BattleTargetCollectionService.ResolveAreaDirection(...)`。`BattleAiScoreService` 从 typed terrain-contact definition 和 preview 地格计算路线威胁数、当回合可达数及有效触发数收益，飞行过滤服从接地开关；这些事实写入 `BattleAiScoreInput`、fingerprint、decision clone 与 trace，不按 `archer_tripwire_arrow` 或其他技能 id 分支。
- AI 地面转位目标边界：`UseGroundRepositionSkillAction.positioning_mode` 只接受 `escape/high_ground` 并投影进 immutable definition；默认 `escape` 保持既有安全距离与生存收益门槛，`high_ground` 不伪装成逃生动作，只枚举 `grapple_ascent` 的四个正交相邻候选、先过 typed 格子规则与 canonical preview，再按真实高差和 `high_ground_weight` 增加位置收益。候选不得按技能 id 判断登高规则。
...
- AI 挑衅评分边界：`BattleAiScoreService.Taunt` 只对挑衅到期前会行动、且届时真能施放的 attack-check 候选估算 `on_hit_damage * (p - p²)`。只读 projection 在 detached clone 上推进冷却、下一回合 AP、共享 stamina recovery 与状态 duration，再通过 `IBattleAiScoreContext.skill_cast_block_reason_callback` 复用 runtime canonical 门禁；`BattleAiScoreContextAdapter` 与正式 decision context 都必须绑定同一 callback。`direct_effect` 与所有 `BattleSkillResolutionRules.IsForceHitNoCritSkill(...)` 认定的攻击不受劣势影响，不能领取挑衅保护分。
- AI 性能与生命周期边界：production `BattleAiService` 默认关闭全状态 mutation snapshot；`FullSnapshotDiagnostic` 只作为显式正确性诊断 lane，并由真实 scorer/action 回归覆盖。`BattleMovementQueryService` 的 pure topology/path cache 可在同一 battle epoch 和 state/grid/delegate owner 身份不变时跨 decision 复用；epoch 或 owner 变化立即丢弃，`DisposeRuntime()` 只结束当前 battle cache，`Dispose()` 是不可逆的终止边界。AI 的 layered-barrier 评分从 decision context 借用 immutable `BarrierProfileDefinition` 索引，并结合 `BattleBarrierStore`、单位位置、现存层数与剩余 TU 生成 detached tactical projection；评分不按具体技能 id 分支，decision clear 后不保留 profile/state 引用。单位 `coord/body_size/body_size_category/footprint_size/occupied_coords` 的唯一存储归 plain C# `BattleUnitGeometryState`，anchor + body → footprint → occupied 的局部不变量由 owner 原子维护；cell occupant 拓扑、落点事务与 movement geometry revision 仍归 `BattleGridService` / `BattleState`。`BattleUnitState` setter 与 admission 是正式写入口；攻击、邻接、AI、grid、UI 等读路径只消费不可变 geometry read view，不执行 refresh/修复。clone、canonical 与 load 对 owner 缺失或不一致投影 fail-fast，mutation exact 单独保留 raw 诊断状态。
- BattleUnitState activation/action-clock/rest 边界：单次 activation 的行动、移动、锁定移动点授权与施法耗尽归 `BattleUnitTurnState`，timeline 只编排 activation 开始的整体 reset 与结束时的 casting clear；movement、metrics、special-skill 与 casting runtime 在各自提交点写入 turn owner。实际行动通过 `BattleUnitState.CommitActionTakenThisTurnTyped()` 跨 turn/rest owner 原子记录已行动并清除 resting；metrics 的非 wait、正 AP 行动与成功建立 pending cast 的 casting 提交点复用该网关，后者仍以真实 `apCost = 0` 记录 metrics。action progress、threshold 与 action-rate remainder 归 `BattleUnitActionClockState`，canonical/detached 继续投影原 progress/threshold flat key，runtime remainder 只进入 clone 与 AI exact。跨 activation 保留的 resting fact 独立归 `BattleUnitRestState`：timeline 在未行动回合结束时标记，stamina recovery 只读取该事实；原 `is_resting` codec/stable key 不变。casting remainder 仍是独立生命周期。
- BattleUnitState movement-tag 边界：地形移动能力的有序标签归 plain C# `BattleUnitMovementTagState`，不与 geometry、vision/proficiency/save tags 或 movement-query cache 合并；factory、summon 与 simulation 通过 typed gateway 写入，grid、charge、movement query 与 `BattleTerrainRules` 通过专用只读 view 消费。normal 写保持空值过滤、首次去重与顺序，strict codec、clone、canonical/detached 和 mutation exact 保留各自旧语义，原 `movement_tags` flat/stable key、位置与数组类型不变。当前生产写都发生在 `BattleState.SetUnit(...)` 前；未来若允许入场后动态修改，必须由 state-level gateway 同步推进 movement geometry revision。
- BattleUnitState vision/proficiency 边界：race/subrace 投影的两组有序标签归 plain C# `BattleUnitVisionProficiencyState`，共享刷新 owner 但保持独立组件；`RaceTraitResolver` 按 race → subrace 在临时集合归一化后原子替换，升格抑制路径同时清空。save tags、抗性、save bonus 与 effective trait 不属于该 owner。生产调用只走 typed gateway；strict codec、clone、canonical/detached 和 mutation exact 继续保留原两个 flat/stable key、逐组件 null/raw 语义、顺序与数组类型。
- BattleUnitState save-modifier 边界：单位静态的三组 save tag、ability bonus map 与 save-tag bonus map（`BonusByTag`）归 plain C# `BattleUnitSaveModifierState`；passive refresh 原子清空五组件，race/subrace 与 effective trait 继续按原先顺序追加，敌人模板只初始化 tag。trait 的 `save_tag_bonus_entries` 由 `BattleTraitPassiveProjectionService` 在 effective traits 刷新时跨 trait 聚合（同 tag 的 `add` 贡献 checked 求和、`highest` 贡献取最高正值、两类并存时相加），以原子 replace 写入 `BonusByTag`。`BattleSaveResolver` 总加值为 ability bonus +  unit `BonusByTag` + transient status bonus（status 仍按既有 Math.Max 语义），execute/preview/AI 共用同一 read view。`BattleSaveContentRules` 是 save tag 唯一 closed-domain owner，canonical `dragon_frightful_presence` 归为 control tag。AI effect signature 继续只覆盖原有三组 tag。damage resistance、effective trait 源事实以及 `BattleStatusEffectState` 上的动态 save tag/bonus 均不属于该 owner。strict codec、clone、canonical/detached 和 mutation exact 保留原五个 flat/stable key、相对顺序、逐组件 null/raw 及 malformed bonus map 回落为空的既有语义。
- BattleUnitState damage-resistance 边界：单位静态 damage tag → mitigation tier 映射归 plain C# `BattleUnitDamageResistanceState`，不与 save modifier、effective trait source、status effect 或 equipment projection 合并。normal owner 写只过滤空 key/value并允许后写覆盖；race/subrace 的覆盖顺序、trait 的 stronger-only 策略与 strict codec 的 tier 校验分别留在调用边界。伤害规则通过单 tag typed query 读取，不复制或遍历整表；clone、canonical/detached 与 mutation exact 继续保留原 `damage_resistances` flat/stable key及各自 null 归一/raw presence 语义，不提供旧 public map facade。
- BattleUnitState effective-trait 边界：battle-local effective instances 与其派生 trait ids 归 plain C# `BattleUnitEffectiveTraitState`，不与下游 save modifier、damage resistance、trait passive status、equipment ability source 或 charge counter 合并。normal replace 防御性深拷贝 instances、保留实例顺序与重复项，并从中生成 ordinal 排序且去重的 ids；生产消费者只读 immutable scalar view 或 `HasEffectiveTrait(...)`，不会取得 mutable entry/roll value。strict codec 继续校验 entry schema、effective key 唯一及 ids 集合等价，并 raw 安装以保留 payload ids 顺序；gameplay clone、canonical 与 detached 始终忽略 raw ids 并从 instances 重派生；mutation exact 独立保留 owner presence、instances/ids 的 null、顺序、重复、非法 nested sentinel 与 mismatch。原两个 flat/stable key、相对顺序和类型不变，不提供旧 public 字段 facade。
- BattleUnitState creature-type 边界：已 materialize 的有序生物分类归 plain C# `BattleUnitCreatureTypeState`，不与 movement/save/effective-trait 投影合并；encounter 与 summon 通过 typed gateway 写入，equipment/skill/damage rules 通过 readonly view 或 `HasCreatureTypeTag(...)` 读取，执行期不回查 enemy template、identity 或 item/trait catalog。`CombatEffectDef.required_target_creature_type_tag` 投影为 effect-local 单标签目标门禁，`BattleEffectTargetRequirementRules` 统一供地面技能 preview、正式执行与 AI 有效目标计数读取，不能只在表现层过滤。normal 写过滤空值、首次去重并保序，strict codec、gameplay clone 与 mutation exact 分别保留既有校验、null 归一和 raw 诊断语义；原 `creature_type_tags` codec/stable key、顺序与类型不变。
- BattleUnitState cognition 边界：基础认知由 `BattleUnitState` 以封闭 `BattleCognitionKind` 独立持有，顺序固定为 `mindless < instinctive < sapient`；`BattleCognitionRules` 只读基础认知、`BattleStatusSemanticTable` 提供的状态认知上限和 `BattleUnitEquipmentAbilityProjectionState` 的来源绑定认知上限并取最低值。认知不与 INT、creature tag 或 AI brain 合并，也不自行改变阵营、控制权或行动选择。`madness` 只把上限压到 `instinctive`；`taunted` 在有效认知低于 `sapient` 时保留状态和倒计时但暂停攻击劣势与 AI 强制目标，恢复后在原到期点前继续生效。`CombatEffectDef.required_target_min_cognition` 与 `BattleEffectTargetRequirementRules` 统一供 preview、execution 和 AI 消费。`cognition_kind` 是 canonical/strict/detached/clone/mutation 必填字段，strict load 对缺失或未知值 fail closed，不提供旧 payload 兼容。
- BattleUnitState known-skill 边界：有序 active ids、level map 与 lock-hit bonus map 归 `BattleUnitKnownSkillState`，三组件保持独立；active 首项仍表达 main skill，level missing 与显式 `0` 不合并。生产消费者只走 typed 单项 gateway 或零副本 struct view；canonical/detached 与 AI mutation snapshot 从借用只读组合视图直接填充最终容器，只有显式 mutation-exact seam 创建拥有型 raw 副本。旧三个 flat/stable key 与 AI unit snapshot 的 active/level 范围不变；mutation diagnostic 用内部 sentinel 区分 owner 缺失和 owner 存在但三个组件均为 null。
- BattleUnitState current-resource 边界：current hp/mp/stamina/aura/ap/move-points、hp 绑定的 alive fact 与 stamina recovery progress 归 `BattleUnitCombatResourceState` 原子持有。normal HP 写同步 alive，但默认态仍允许 `hp = 0 / alive = true`；Timeline 决定恢复规则输入，owner 原子推进 stamina/余数，resting 由独立 `BattleUnitRestState` 提供。canonical/detached、strict codec、clone 与 mutation exact 保持 raw 语义，strict 只额外拒绝负 move-points；旧八个 flat/stable key 的名称、顺序与类型不变。
- BattleUnitState combat-resource capability 边界：已解锁资源 id 的有序集合独立归 `BattleUnitCombatResourceUnlockState`，不与 current-resource owner 合并。normal 写过滤空值、未知值和重复项并在末尾补齐默认 `hp` / `stamina`；生产读取走单项 gateway 或零副本 struct view，canonical/detached、strict codec、clone 与 mutation exact 继续保留各自原有的默认同步、校验和 raw 诊断语义。旧 `unlocked_combat_resource_ids` flat/stable key 不变。
- BattleUnitState weapon-projection 边界：profile/item/type/range/family、grip/attack range、单手/双手骰、versatile/双手使用与物理伤害标签十二个既有字段，加上 battle-only 的装备实例 id 与 heavy 属性，归 `BattleUnitWeaponProjectionState` 原子持有。normal apply 复制 mutable dice 并规范化，canonical/detached/clone 继续先规范化 live unit，strict codec 安装已验证 raw values，mutation exact 保留 owner presence、nullable `StringName` 与 null/present/invalid dice 差异；生产规则只读不可变 value view。旧十二个 codec flat key不变，新增两项只进入运行时/AI stable projection，不加入存档 schema，也不提供旧字段兼容 facade。
- BattleUnitState reaction/counterattack 边界：`BattleUnitReactionState` 与 `BattleUnitCounterattackCapabilityState` 分别原子持有 TU 反应预算和 battle-local capability；两者统一使用 explicit owner presence，并进入 clone、strict codec、canonical/detached 与 AI mutation-exact，但不进入持久存档。`BattleAttackActionContext` / `BattleAttackResolutionFact` 是 core immutable 攻击契约，delivery kind 由 `BattleAttackDeliveryRules` 基于正式 effect 与冻结的 weapon projection 分类；`BattleCounterattackRules` 是 trigger、actor pair 与 readiness 的纯规则 owner。
...
- BattleSim override 边界：`BattleSimContentProvider` 与 `BattleSimFormalCombatFixture` 只借用 process snapshot 的 typed definitions；formal fixture 为每次 roster/runtime 复制自身需要的 skill、profession、achievement、item、trait 与 identity 索引，不重建 `ProgressionContentRegistry` / `ItemContentRegistry`。两个 formal benchmark 在 `BattleRuntimeModule.setup(...)` 同步注入 snapshot traits 与 equipment-ability bindings，runtime 自行复制 catalog 后再构建 typed hostile roster，使正式 factory 具备生成 equipment/temporal projection 的内容条件；默认 loadout 不伪造 temporal modifier。formal roster 的建卡属性、技能配置、核心技能与装备占位均由 CLR DTO/List 承载；只有同步调用遗留 `CharacterCreationService` payload API 时创建 Request-domain projection lease，并在该成员建卡返回前关闭。battle character gateway 的成就结果是 `IReadOnlyList<StringName>`，正式 `CharacterManagementModule` 仅在 Godot-facing 公共包装处投影 typed Array。`BattleSimOverrideApplier` 复制 definition 索引，并以新的 `SkillDefinition` / `EnemyAiBrainDefinition` / `EnemyAiActionDefinition` / `BattleAiScoreProfileDefinition` 值表达本次模拟 patch。override 结果只属于该次 simulation，不改写 process snapshot，也不对 authored Resource 调用 `Duplicate` / `Set`。
- 战斗规则边界：伤害上下文正式链是 `DamageResolutionContext`，写回前 hook 边界是 `DamageApplicationProjection` / `IBattleDamageApplicationHook`（hook / release suppression 由战斗 runtime 的 contingency owner 管理，不入 damage payload / AI 评分 / save schema）。`BattleUnitState` 状态效果正式源是 `BattleStatusEffectCollection`，状态上的一次性攻击优势、攻击/豁免加值与 `armor_class_bonus_per_stack` 由 `BattleStatusEffectState` typed 字段声明；真实攻击检定消耗和状态 AC 汇总归 `BattleDamageResolver` / `BattleHitResolver`，真实豁免消耗归 `BattleSaveResolver`，preview/probability 不消费。持续状态的 typed `upkeep_*` 配置和 `upkeep_elapsed_tu` 快照归 `BattleStatusEffectState`，定时扣费、硬控/断供终止、终止惩罚及终止冷却归 `BattleRuntimeSkillTurnResolver`，近战连击层的命中生成归 `BattleDamageResolver`、按层攻击检定归 `BattleHitResolver`；execute effect 的 `soul_fracture_duration_tu = 0` 明确表示不施加灵魂裂隙，正数才生成该状态；typed unit save modifier真相源是 `BattleUnitSaveModifierState`，damage mitigation 真相源是 `BattleUnitDamageResistanceState`；技能的 `requires_weapon` 效果要求 `equipped` 武器投影，或要求技能显式 `allows_natural_weapon` 且当前投影为 `natural`；若技能同时带 `melee` 标签，则 `BattleRangeService` 进一步要求武器投影的 `weapon_range_type == melee`，不再用射程阈值猜测近远程。`required_weapon_families` 用于宽泛武器家族门禁，`required_weapon_type_ids` 精确匹配战斗武器投影的 `weapon_profile_type_id`，两者均不替代近战类型判断；`vault_behind_target` 的落点与两段 edge/barrier 合法性唯一归 `BattleVaultBehindTargetRules`，`BattleSkillTargetValidationService` 在付费前校验并投影最终落点，`BattleSpecialSkillResolver` 只在命中后提交使用者移动，AI 对此效果强制走 canonical preview；layered barrier 真相源是 `BattleBarrierStore` / `BattleBarrierInstanceState`，`BattleBarrierService` 同时拥有单位边界穿越、投射效果显式起点检查、投射地面效果的逐格 allowed/blocked 裁剪、当前层破解提交、只读指定锚点预览判定与 special profile 豁免；barrier changed-coord 提交直接消费 CLR `IEnumerable<Vector2I>` typed overload，不创建临时 Godot Array，效果服务也不得按单位锚点或地形格再次复制屏障规则。临时边特征真相源是 `BattleTemporaryEdgeFeatureState`，由 `BattleEdgeService` 叠加进 runtime edge face，移动/占位/寻路仍消费统一 edge face；authored 格上边特征（墙/门/闸门）已于 2026-08-16 移除，游戏不再有任何 LOS 阻断机制。`BattleStatusSemanticTable` 拥有通用状态语义（如 `paralyzed` 的行动、移动与 pending cast 阻断），不在具体武器能力中硬编码。`BattleFateEventBus` 事件 surface 是 typed `BattleFateEventPayload`，misfortune 直接触发用 `MisfortuneTriggerRequest`。fixed schema 名称（combat resource id / damage tag / mitigation tier / target-team filter / save tag/ability / forced-move mode 等）由 enum/typed utility 解析；伤害 `bonus_condition` 闭集由 `BattleDamageBonusConditionKind` / `BattleTypedNames` 唯一投影，条件语义统一归 `BattleDamageBonusConditionRules`，正式伤害与 AI 估值共同消费该 owner，不各自维护字符串分支。
- 伤害来源上下文边界：`BattleDamageOriginKind` closed domain 归 `BattleDamageOriginContentRules`；`DamageResolutionContext` 默认携带 fail-closed 的 `Unknown` 并在不可变 clone 链中显式保留。`ResolveProducerOrigin(...)` 只把同一单位的显式 `MainDirectEffect` 规范化为 `SelfDamage`，不从 skill、item、binding id 或调用深度猜测来源。
- 蓄力规则边界：`BattleWindupRules` 唯一拥有自然挡位上限、力量 TU 阶梯、动态体力和最终 W 倍率换算；`BattleSkillPreviewService` 与正式开始共同消费同一 quote，正式完成只使用开始时冻结的 `BattleWindupSnapshot`。武器固定加值、装备附伤和反应不随 W 倍率复制；蓄力完成仍作为一次标准武器攻击经过命中、重击、护甲、格挡、装备 on-hit 与反击链。AI 为每个合法挡位建立独立 canonical-preview 候选，并显式计入动态体力与延迟代价；Contingency、装备即时攻击与 `trigger_skill` 自动路径不支持蓄力。
- 伤害应用生命周期边界：`DamageApplicationInput` / `DamageApplicationProjection` / hook context/result 与 `BattleDamagePreviewResult` 是 plain typed value，shield/hp/fatal preview、hook 和业务结果不长期保存 Godot Dictionary。仍要求 Dictionary 的同步 adapter 在入口立即归一化；caller 调用 `DamageApplicationInput.ToDictionaryLease()`，由该方法从 `BuildSnapshotPlain()` 创建 Request-domain projection lease，并在调用返回后关闭。preview/result payload 直接从 plain damage event 构造，不缓存 wrapper 到 `BattleDamageResolver`、event batch 或下一次效果结算。普通致死在 fatal trait 与 Last Stand 之后、唯一 `MarkDead` 之前调用 `IBattleFatalInterceptArbiter`；候选按显式 order 稳定排序，受 death protection priority、per-battle/per-world usage 和 roll gate 约束。preview 使用含 units/cells/environment/world step 的当前规则所需 `BattleDetachedPreviewState` 子集，按候选输出 reach/success/contribution 概率、期望恢复和动作摘要，不推进正式 RNG/usage；objective、barrier、target mark、temporary edge 与 backpack/report 等尚未进入该 detached owner。saving throw 成功/失败分支分别推演后再按概率合并，不能以平均伤害抹去仅一条分支的致死候选。正式伤害与致死仲裁全部完成且目标仍存活后，defender 侧 `OnDamageTakenFinalized` reaction 才读取最终生命；typed fact 包含 raw/final damage、damage tag、伤害前 HP/百分比、equipment-generated 与 self-damage 来源标记。finalized preview 的确定动作只写 detached state，条件动作仅报告概率，无法安全投影的动作返回 typed unsupported reason。
- 多段致死预览边界：`BattleDamagePreviewWorkingSet` 保存 continuation frontier；每个分支在正式 consume-on-attempt / consume-on-success 时点只改自己的 detached usage，成功分支恢复并执行受支持 success/finalized 动作，失败或死亡分支只要未来仍可能拦截就继续进入下一段。完整 unit detached snapshot、per-battle charge 与 world step 构成当前等价签名。frontier 当前无硬上限；本地概率 finalized 状态动作只汇总 conditional action、不延续状态分裂。正式凤凰内容使用 branch-local 确定动作，已由连续致死与披风二段回归覆盖；未来扩展必须同时补 bounded/truncated 结果。
- 致死成功动作边界：equipment fatal arbiter 在候选恢复生命且确认目标存活后才执行受限的 `apply_status` / `trigger_skill` success action，然后返回首个成功候选；这些内部技能继续走 canonical effect、范围、减伤与死亡规则，但不会取得只面向攻击主直接伤害段的装备附伤。preview 只对 100% 且属于受支持安全子集的成功动作应用 detached 投影，概率动作保持 conditional，ground/mixed skill 与递归超过一层的内部技能明确 unsupported，不污染确定的 `target_preview_after`。
- 伤害段边界：`BattleDamageResolver` 负责把 `CombatEffectDefinition.ExtraDamageSegments` 结算为同一次 damage effect 下的额外 `DamageEventResult`；额外段复用该 effect 的 save result 与目标倍率规则，但不继承武器骰或装备追加骰。`CombatDamageSegmentDef.double_dice_on_critical` 默认 false；显式 true 时仅为该段自身骰池追加一组暴击骰，flat bonus 不重复。目标分类倍率只通过 `BattleUnitState.HasCreatureTypeTag(...)` 读取 `BattleUnitCreatureTypeState`，不回查敌人模板或物品/trait catalog。
- 装备伤害骰边界：装备能力 `add_damage_dice` 与状态来源绑定的武器额外骰由 `BattleDamageResolver` 统一结算；来源绑定骰只在状态 `source_unit_id` 匹配攻击者且本次 effect 实际包含武器伤害时触发，并入 bonus damage dice 以复用暴击额外骰路径；装备能力 `AddDamageDiceActionPayloadDefinition.RequireWeaponDamage` 默认 true 以保持既有武器内容语义，显式 false 才允许徒手或法术攻击的主直接段触发。装备能力骰数可由通用 fact（如装备能力状态 fact）按 JSON authoring 公式放大；`subtract=true` 的骰只扣减匹配主伤害标签的本次基础伤害，不生成负数额外伤害段。`CombatEffectDef.weapon_dice_multiplier` 只复制基础物理武器骰，不复制 flat bonus、装备追加骰或非物理附伤；`bonus_weapon_dice_multiplier` 在条件成立时生成独立伤害事件并继承本次重击；`bonus_damage_separate_event` 可把固定条件骰拆成独立伤害事件并继承重击。多次常规武器攻击仍逐击查询并结算装备附伤。
- 装备附伤替换边界：`BattleEquipmentAbilityBonusDamageReplacementRules` 在 damage-query 与 after-hit result 两条链上按 action bundle 仲裁 `replacement_group_id`；同组最高 `replacement_priority` 获胜，同优先级按 binding/action ordinal 稳定决胜，不同组与未分组结果继续叠加。winner 的全部骰项作为整体保留，来源失效后下一次收集自动回落到仍有效的低优先级候选。
- 桥接：装备能力的命中检定加值、优势、防御组件调整、命中后强制暴击与召唤物数量/距离修正由 `BattleEquipmentAttackModifierResolver` 从 `BattleUnitEquipmentAbilityProjectionState` 的不可变 source read view、`BattleUnitState.attribute_snapshot`、typed target mark 和 battle-only 召唤单位 blackboard 收集，经 `IBattleEquipmentAttackCheckQuery` 注入 `BattleAttackCheckPolicyService`，再由 `BattleHitResolver` 生成本次 `AttackCheckInput`；技能自身的 `attack_defense_mode` 也只在该 canonical check 内合并到同一防御调整，因此 execution、preview、repeat attack 与 AI 共用规则。bonus dice、damage roll mode 与固定减伤经 `IBattleEquipmentDamageQuery` 注入 `BattleDamageResolver`，真实 after-attack-check/after-hit/hit-received/damage-applied 与耐久投影刷新经 `IBattleEquipmentCombatReactionSink` 同步提交。三个端口不提供 state getter，状态只来自 `BattleAttackCheckPolicyContext` / `DamageResolutionContext` / reaction context；反应 context/result plain DTO 归属 battle core，嵌套伤害仍保留当前同步调用栈。命中检定加值与强制暴击可用 `require_weapon_damage` 限定只作用于含武器伤害的攻击定义，也可通过 `attribute_modifier_id` 从使用者属性快照读取动态调整值。忽略 AC component 等规则只调整本次目标 AC，不改写目标 `attribute_snapshot[armor_class]`。召唤单位是否为 summoned 的规则只读 `BattleAiBlackboard` / 状态效果，不从武器内容表反查。
- 装备减伤光环桥接：`BattleEquipmentAttackModifierResolver` 经 `IBattleEquipmentDamageQuery.CollectMitigationAuras(...)` 从当前 `BattleState` 动态查询存活来源、目标阵营和 footprint 格距；`BattleDamageResolver` 把结果作为 `equipment_ability_aura` source 合入现有 immune/half/double/bypass 规则，不复制第二套火焰或距离特例。
- 装备条件减伤 tier 桥接：`IBattleEquipmentDamageQuery.CollectMitigationTiers(...)` 以贯通 SkillId/SaveTag/EffectCategories/DamageTag/DamageOriginKind 的 typed context 收集 defender 侧 `grant_mitigation_tier` action；`BattleDamageResolver.ResolveMitigationTierResult()` 在最终 tier 选择前把结果作为 `equipment_ability_mitigation_tier` source 与 status/aura/unit resistance 一起聚合，保持 immune 优先、half/double 抵消与多份 half 单次结算；报告 source id 保留 label 或 binding/action provenance。DamageOriginKind 的 closed domain 归 `BattleDamageOriginContentRules`，fail-closed，Unknown 不参与条件匹配。
- 每次主直接伤害结算加骰桥接：`IBattleEquipmentDamageQuery.CollectBonusDamageDiceForEffect(...)` 以 `BattleEquipmentAbilityDirectDamageContext`（含 PrimaryDamageTag/DamageOriginKind/SourceEffectOrdinal/攻击检定与暴击事实）在 canonical resolver 每次准备结算主 Damage effect 时纯读取查询一次；origin 只在显式 `main_direct_effect` 时触发（timeline/upkeep、地形、反射、自伤、equipment bonus、direct reaction、trigger-skill 均排除），miss 段不触发、豁免型主直接伤害不依赖攻击检定，repeat/chain 逐段重进 resolver 自然逐段触发且无跨段去重；`AddDamageDiceActionPayloadDefinition.DamageTypeMode`（closed：`explicit`/`inherit_primary`）决定显式 tag 或继承主 effect canonical tag，继承骰进入同段倍率/save/mitigation 且不被暴击复制；extra_damage_segments 与 equipment bonus 追加段不重复查询不递归。execute/preview/AI 共用同一 `ResolveDamageOutcome`/preview working-set 路径，回归为 `tests/battle_runtime/runtime/run_equipment_direct_effect_bonus_dice_regression.cs`。
- 通用 attack-hit reaction 桥接：typed trigger `on_attack_hit`（仅允许 `after_hit` timing）在真实攻击检定成功后、shield/HP commit 之后、kill/nested reactions 之前由 `BattleDamageResolver.ResolveAttackEffects` 经 `IBattleEquipmentCombatReactionSink.ResolveAttackHit` 每目标同步提交一次，不要求 weapon damage；miss 与无攻击检定的豁免型伤害不触发，context 携带 `SkillId`/`DamageOriginKind` provenance 与 event batch，action 复用 after-hit `ResolveActions` 编排；preview/AI 不挂载该 sink 因而不消费 charge。旧 `on_hit` 保持 weapon-hit 语义不扩宽。回归为 `tests/battle_runtime/runtime/run_equipment_attack_hit_reaction_regression.cs`。
- 目标过滤边界：死者单位默认不可被 unit skill 选中；只有 ally/self 且 effect 列表显式包含 `HealFatal` 才由 `BattleSkillTargetValidationService` 放开 dead target，普通 `Heal` 不推断复活语义。effect 级 `exclude_source` 同样在 target validation 阶段生效：若对自身所有可作用效果都排除 source，则付费前拒绝命令，不为具体装备技能 id 建特例。orchestrator 主文件的技能预览链（`BattleSkillPreviewService`）、目标校验（`BattleSkillTargetValidationService`）、链式伤害（`BattleChainDamageService`）与随机链抽样（`BattleRandomChainSkillService`）已物理拆分为四个弱借用 `BattleRuntimeModule` 的 service；连锁 service 在主目标结算前调用 `BattleChainDamageRules` 冻结逐跳路线，preview/execution 共用坐标对与 barrier 中止语义，AI 只消费 canonical preview。orchestrator 在 `Setup` 中接线，在 `DisposeRuntime` 中同时断开 runtime、owner 与 sibling borrower，对外 internal 入口由 orchestrator 保留窄委托。
- 治疗抑制与类型过滤边界：`BattleSkillCreatureTypeTargetRules` 在 mutable/read-only unit target validation 中消费 immutable 排除 tag，并在支付资源前拒绝；`BattleStatusModifierRules` 对独立状态取最低治疗倍率，同一 aggregate refresh 状态只刷新自身。`BattleSkillPreviewService` 仅把本次治疗抑制状态投影为 `BattleStatusContributionPreviewData`，HUD 不枚举其他减疗；`BattleAiScoreService.HealingSuppression` 按治疗能力、可恢复生命、命中率和逐状态剩余 TU 形成 `estimated_*_healing_denied` 边际事实，不复用普通 status/control 数量分。涉及该链时推荐同时读取 `CombatSkillDef` / `SkillDefinition` / `SkillDefinitionCombatProfileValidator`、`BattleSkillCreatureTypeTargetRules` / `BattleSkillTargetValidationService`、`BattleStatusSemanticTable` / `BattleStatusModifierRules`、`BattleSkillPreviewService` / `BattleStatusContributionPreviewData` / `BattlePreviewProjection`、`BattleAiScoreService.HealingSuppression` / `BattleAiScoreInput` 与 `tests/battle_runtime/skills/run_mage_bone_chill_regression.cs`。
...
- 文件：
  - `scripts/enemies/definitions/WildEncounterRoster*.cs`
...
- 文件：
  ...
  - `scripts/systems/battle/presentation/BattleBoardRenderSnapshot.cs`
...
- 文件：
  ...
  - `scripts/systems/platform/TrueRandomSeedService.cs`
- 细节文档：
  - `docs/design/platform/godotsharp_lifecycle.md`
  - `docs/design/platform/code_coverage.md`
  - `tests/shared/TestWorldGenerationDefinitionFactory.cs`
  - `tests/shared/TestSkillDefinitionProjection.cs`
  - `tests/equipment/*`
  - `tests/warehouse/*`
  - `tests/battle_runtime/**/*`
  - `tests/battle_runtime/skills/run_mage_arcane_orbit_regression.cs`
  - `tests/progression/**/*`
  - `tests/runtime/**/*`
  ...
  - `tests/runtime/validation/run_item_icon_asset_catalog_validator_regression.cs`
  - `tests/runtime/validation/run_content_json_schema_export_regression.cs`
  - `tests/runtime/validation/run_content_json_offline_validation_regression.cs`
  - `tests/runtime/validation/run_*_json_content_regression.cs`
  - `tests/runtime/validation/run_equipment_closure_generation_*_regression.cs`
  - `tests/text_runtime/**/*`
  - `tests/world_map/**/*`
  - `scripts/dev_tools/*.cs`
  - `scripts/tools/run_content_json_schema_export.cs`
  - `scripts/tools/run_equipment_closure_generation_catalog_export.cs`
  - `scripts/systems/content/generation/*.cs`
  - `tools/*.py`
  - `tools/*.gd`
  - `tools/content_json_validation/**/*`
  - `tools/architecture/**`
...
- content JSON 离线校验边界：`tools/content_json_validation` 是不引用 Godot 的独立 .NET 进程，通过 linked compile 复用 `scripts/systems/content/json` 的纯 CLR document/template/import contract；host reader 只改变单文件或目录的稳定 `.json` 枚举范围，随后仍走同一条 document load → file-local template merge → strict DTO parse → domain-local validation 链。每个域必须在 `ContentJsonOfflineValidationCatalog` 显式注册 source-generated `JsonTypeInfo<TDto>` parse、typed import normalize 与 domain validator delegate；工具输出同一 `ContentJsonDiagnostic` provenance 的 v1 JSON/NDJSON，不构建 `ContentSnapshot`、不做跨域校验，也不通过反射发现 DTO。
- content JSON 终态边界：一次性 TRES converter、Resource adapter、content Resource loader/discovery、反向路径索引与 migration inventory 已删除；全部 gameplay/config domains 从 strict JSON direct-load。只有 engine-asset catalog 及其 typed entry 保留 Godot Resource authoring。
...
- 边界：`TestHarness.Finish(...)` 只冻结断言并生成 `TestResult`；C# runner 统一继承 `LifecycleTestSceneTree`，先由 `TestResourceOwnership.Close()` 关闭当前测试显式拥有的 pathless GodotObject fixture，再由 `TestExitCoordinator` 把结果提交给 `ApplicationLifetimeCoordinator`，owner teardown、production finalizer barrier 与最终退出均由同一 shutdown pipeline 负责。`LifecycleMeasurementBarrier` 只服务同进程 soak 的周期量测。外层 `run_regression_suite.py --lifecycle-correctness` 强制 strict/trace，并把 GodotSharp fatal marker 或 shutdown report 的非零 `legacy_debt` 判为失败。确定性 lifecycle soak 单进程执行 110 周期，记录逐周期 owner/engine-asset/lease 完整向量、activity 增量与 managed/private memory 统计。业务 fixture 优先用 immutable Definition/CLR builder；正式 skill 从 JSON catalog 读取，刻意非法的 validator fixture 通过 `SkillDiagnosticFixtureProjection` 汇入同一 DTO/import-model normalize 边界，不写盘或加载 `.tres`。
...
- content-icon catalog 回归边界：正式 process snapshot 分别锁定 skill/item 非空图标 occurrence 与唯一 asset ID 数，并用真实 resolver 逐项验证 `Texture2D`；unknown 与已登记 wrong-type ID 必须经 `ProcessContentHost` 实际 publication preflight 失败，证明 rollback 后没有 snapshot、epoch 或 sealed partial publish。HUD/warehouse 回归同时覆盖登记 ID、空 ID 表现与 unknown/path-like 非空 ID 无路径或默认图标回退。
- 覆盖率边界：`run_coverage_suite.py` 先还原本地 `dotnet-coverage` 工具并构建项目，再以唯一 session 静态插桩 Godot 实际加载的 `magic.dll`，启动 collector server 后复用 `run_regression_suite.py --jobs` 并发执行，最后无论成功或失败都先 shutdown collector、再 uninstrument 恢复 DLL。Cobertura 分母只包含 `magic.dll` 中非 `tests/`、非 `tools/`、非 `.godot/` 的 production source；空报告、缺失 production sequence point 或测试源码泄漏进入分母均 fail closed。Microsoft Cobertura 未提供 branch counters 时只报告真实 line coverage，并明确把 branch coverage 标为 unavailable。
- CI 分组边界：`run_regression_suite.py` 在正式过滤后将排序清单按 shard index / count 轮流分配，各组互斥且并集完整；`.github/workflows/ci.yml` 用四个隔离 job 执行同一 strict suite。必需检查 `Build and Headless Smoke` 只在四组全部成功、四份 Cobertura 合并并通过 production 分母校验后成功；分组不增加 simulation、benchmark 或 E2E。
...
- Windows 导出 smoke 边界：受版本控制的 `export_presets.cfg` 显式包含没有 `.import` sidecar 的 JSON 非资源文件；`tests/export` 只从真实 Windows Desktop 导出 EXE/PCK 启动，工作目录与源码树隔离，并验证 production typed engine-asset catalog、`ui.item.icon.default`、包内 items/traits/equipment abilities/gear sets/recipes 五个阶段 4 JSON 域的真实复读计数，以及 world 三个 JSON 域的 registry rebuild、preset/root/mounted/shared 稳定 ID 闭包和预期失败退出码。官方 release template 不开放 editor-only `--script`/scene override，因此 runner 只在临时导出目录生成 `override.cfg`，把 main scene 与两个 autoload 指向 test-only owner，不修改 production project/lifecycle；导出二进制与 override 只存在于系统临时目录，不进入版本控制。
...
- 文件：
  ...
  - `scripts/systems/battle/sim/BattleSimAnalysisOutput.cs`
...
- 敌方内容边界：production discovery 由 `EnemyContentJsonDomains` 固定拥有三个 JSON 目录；`EnemyContentRegistry` 分别运行 strict DTO parser、plain import validator 与 `EnemyContentDefinitionProjector`，直接发布 `EnemyTemplateDefinition`、`EnemyAiBrainDefinition`、具体 `*ActionDefinition`、generation/transition definitions 与 `WildEncounterRosterDefinition`。template 的武器、派生 HP 与攻击加值统一归 plain typed `EnemyTemplateProjectionRules`，projector 直接取得 immutable weapon/derived facts，不构造 Godot Dictionary、Variant 或任何 authoring Resource 中转。`EnemyContentRegistry.ValidateProjectedGraph(...)` 复用 production 的 Definition 图校验，供纯 DTO/Definition fixture 验证跨域引用；`TestEnemyDefinitionFactory` 与 plain `TestEnemyTemplateDefinitionBuilder` 只构造 immutable Definition，后者委托同一个 projection-rule owner，不形成第二套 projector。三个域不存在 seed、目录扫描配置、`.tres` source、Resource adapter 或 test-only Resource 旁路。`ContentSnapshot` 冻结这些索引后，session、catalog、world、battle 与 headless runtime 只借用同一 definition graph，不持有 JSON DTO/import model 或 session 级 registry mirror。
...
- BattleSim profile 边界：`BattleSimProfileContentRegistry` 从 `battle_sim_profiles` strict JSON domain 导入 plain model，`BattleSimProfileDefinitionProjector` 唯一投影 `BattleSimProfileDefinition` / typed override patch；enemy brain 与 BattleSim 的 `BattleAiScoreProfileJsonDto` 统一复用 `EnemyContentDefinitionProjector.ProjectScoreProfile(...)` 的显式字段投影，authoring projection 不通过反射或 `TryWithScalar(...)` 静默跳过字段，后者只用于 simulation-local typed override。正式 profile 随 process snapshot 发布，simulation runtime 与 report 只传递 definition。具体 patch 的 typed copy-on-write 规则归 CU-16。AI score tuner 的标量路径与搜索边界由 `score_weight_space.json` 统一拥有，Python `score_weight_catalog.py` 与 C# `BattleSimTuningParameterCatalog` 都只把该结构化目录投影为各自的 typed spec，不扫描另一端源码来推断契约。
...
- 装备闭包生成门禁边界：`EquipmentClosureGenerationCatalog` 只导出五个阶段 4 schema domain 名与已发布 skill/profession/item/trait/equipment pack/binding/gear-set/recipe/Texture2D asset ID 列表，不导出 `res://` 或 `.tres`。候选 items、traits、equipment abilities、gear sets、recipes 依次通过 strict schema、domain validator、与 process snapshot 合并后的 cross-domain contract、真实 `EquipmentClosureGenerationBattleSimGate`；cross-domain 层复用正式 item-trait、skill-book 与 gear-set business validator，并保留字段 JSON pointer。BattleSim 以 immutable combined definition 覆盖 provider，默认最多按 ordinal 抽 8 件候选装备、12 个固定 seed，逐件与同类型正式基线比较完成样本、胜率差和每局伤害比；不会重发布 snapshot。该模拟属于 opt-in generation gate，不进入 routine regression。
...
- 文件：
  ...
  - `scripts/systems/game_runtime/headless/GameTextSnapshotRenderer.cs`
...
### 只改世界唯一装备池、商店刷新或随机掉落转移

- 必带：CU-02、CU-06、CU-10、CU-19
- 实现必读：`docs/design/progression/equipment_sets.md` 的“凤凰唯一获取链”与 CU-02 的 Unique World Equipment Pool read set
- 按需补：CU-08（商店展示）、CU-15（战利品提交）、CU-21（headless runtime）
