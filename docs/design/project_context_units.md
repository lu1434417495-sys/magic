...
- Save owner 约束：`SaveRepository` 现在拥有底层 save payload 文件 IO，包括 save path 构建、压缩 payload 读写、原子替换/重命名/删除和 save 目录确保；`GameSession` 继续拥有 active save、schema/meta 组装、save index 归并和运行时 pending/dirty 状态，不要把新的底层文件写入细节继续内联回 `GameSession`。
- 生命周期约束：`GameSession.Dispose()` 是 session/root/content registry 的 owning teardown，必须释放 `GameRoot` / `GameContentCatalog` 绑定、内容 registry、log/runtime 辅助资源并 `Free()` native node；内容 registry 自己必须通过 `Dispose(bool)` 清空 typed/resource 缓存，保证直接 `Dispose()`、`using`/`IDisposable` 和 Godot 基类释放路径都不把 `RefCounted` 资源留到进程退出；底层文件替换/清理入口 `FileIOCoordinator` 与 save payload serializer `SaveSerializer` 都是 plain C# utility/service，不参与 Godot object 生命周期，也不要恢复 `GlobalClass` / `RefCounted`。
...
- 负责：共享背包、堆叠/容量、物品与配方定义、装备实例、装备/卸装、物品使用；`ItemDef` 的 item category / equipment type / weapon physical damage tag、`EquipmentRules` 的装备槽位、`EquipmentInstanceState.RarityTier` 都由 enum/typed 转换拥有，C# caller 不要恢复 GD helper 函数或合法值 HashSet；`WeaponDamageDiceDef.ValidateDice` 属于程序集内部校验入口，不要作为 public Godot-facing helper 复用；`PartyWarehouseService` / `PartyEquipmentService` / `PartyItemUseService` 的正式 setup 现在优先走 typed item/skill catalog，`PartyWarehouseService` 是 plain C# inventory service，runtime/progression/equipment owner 只通过 `IDisposable` 清引用，不要恢复 `RefCounted` 或 GodotObject validity/dispose 路径；`PartyItemUseService` 的 `GDictionary` setup 边界只做严格 `StringName` key 解码，不要恢复 string-key fallback；`RecipeContentRegistry` 正式 setup 只消费 typed item catalog，不要恢复 `GDictionary -> ItemDef.item_id` value-index helper；forge/settlement 服务调用 `PartyWarehouseService.Preview/CommitBatchSwapEntriesTyped` 时应传 `{ item_id = StringName }` 这类正式 batch entry 字典，不要用裸 item id `Variant` 作为批量输入。
...
- 生命周期约束：`BattleRuntimeModule` 只 owns 自己创建的默认 `BattleTerrainGenerator`；`setup(...)` 注入或测试直接替换的自定义 generator 仍由 caller 释放。module 在替换 generator 或 dispose 时必须释放自己创建且仍持有的旧默认实例，但不得释放 caller-owned generator，避免 battle sim runner / 测试 fixture 复用已 disposed 的 `RefCounted`。
...
- 补充约束：`BattleUnitState` 的技能等级、锁定命中加值、伤害抗性、冷却与次数状态应由 typed map owner 承载，`BattleState` 的地形列、runtime edge face 和 layered barrier payload cache 也应通过 `BattleState` owner API 读写；规则、AI、地形和屏障服务不要恢复直接持有或写入这些 Godot dictionary 字段，只有 save/schema、terrain generation、HUD/test payload 断言等边界显式投影。
