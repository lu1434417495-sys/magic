...
- 负责：启动入口、世界预设入口、存档选择、显示设置、建卡入口；`CharacterCreationWindow` 的 rolled attributes 是窗口内部 typed UI state，最终 `character_confirmed` 信号才投影为 Godot payload，不要恢复 `_rolled_attributes` Godot dictionary 作为跨阶段业务态；`WorldPresetRegistry` / `DisplaySettingsService` 的固定值和预设查询只作为内部 typed/service 边界，不要恢复 public GD helper、公开常量或把 preset info 当 Godot dictionary 业务态回读。
...
- 负责：active save、slot meta、save payload、save index、内容注册表、全局会话边界；`GameSession` 现在持有统一 `GameRoot`，`GameContentCatalog` 是正式内容类型的组合根读入口，并持有自己的 typed 内容快照缓存（覆盖 runtime skill definition / trait / profession / achievement / quest / item / recipe / enemy template / enemy brain / wild roster / progression identity catalog / battle special profile snapshot）：`GameSession` 在刷新 progression / item / recipe / enemy / battle special profile 内容后显式调用 catalog 重建，catalog getter 返回自己缓存的只读视图而非每次转发回 session 重建 typed index，重建会自增 catalog revision 供下游做有效性校验；catalog 的 typed 字典读侧是防御性只读视图（`ReadOnlyDictionary` 包装），下游即便 downcast 也拿不到内部可变 `Dictionary`、不能改写 catalog 快照，要修改正式内容应回到 `GameSession` 内容缓存再触发重建；catalog 生命周期绑定 owning `GameRoot`，root dispose 时会解绑并使 catalog 失效（清空 typed 快照并自增 revision），任何仍持有旧 catalog 引用的下游此后读到的是空内容而非 stale 快照；runtime 装配应优先从 catalog 读取 typed 内容表，再由各领域 owner 持有自己的运行期状态；save index cache 的 session 真相源是 plain `List<Dictionary<string, object>>` 与 typed file signature，`ListSaveSlots()` / `PeekSaveSlots()` 只在 Godot API 边界投影 `Array<Dictionary>`，不要恢复 `_save_index_entries_cache` / `_save_index_cache_signature` 这类长期 Godot payload cache；content validation 的 session 真相源是 typed `ContentValidationSnapshotData`，`GetContentValidationSnapshot()` 只作为 Godot payload 投影边界，不要恢复 `_content_validation_snapshot` 长期缓存或让调用方依赖同一个 dictionary 实例被原地刷新；skill 内容长期迁移方向是运行期只消费 catalog/session 的 plain C# `SkillDefinition` 快照，`GameSession` / `GameContentCatalog` 不再持有或暴露 `SkillDef` Resource 快照，`SkillDef` / `CombatSkillDef` Resource 只保留为 `.tres` 导入、内容校验和明确 Resource-boundary 补丁边界；trait 内容正式读侧走 `ProgressionContentRegistry.GetTraitDefsTyped()` / `GameSession.GetTraitDefsTyped()` / `GameContentCatalog.GetTraitDefsTyped()`，其中 `ProgressionContentRegistry.GetTraitDefsTyped()` 是 progression trait content source，`GameSession.GetTraitDefsTyped()` 是 session 正式 getter，`GameContentCatalog.GetTraitDefsTyped()` 是 catalog typed trait snapshot，不要把 trait content 回投成 `GDictionary` 业务态再读；quest 内容正式读侧走 typed `get_quest_def(StringName)` / `get_quest_defs_typed()`，runtime caller 不要再 duplicate quest payload、回读 `Variant`、`QuestDef.from_dict()` 或 string-key fallback；identity 内容正式读侧现在只通过 `ProgressionContentRegistry.GetIdentityCatalogTyped()` / `GameSession.get_progression_identity_catalog_typed()` 进入 `ProgressionIdentityCatalogData`，registry/content seed 是允许把 Godot 字典资源投成 typed catalog 的边界，但 typed index 只投正式 `StringName` key，runtime caller 不要把 identity catalog 再投影成 bundle/dictionary 业务态后回读；`GameSession.GetSkillDefinitionsTyped()` / `GameContentCatalog.GetSkillDefinitionsTyped()` 是正式 skill runtime 入口，`ProgressionContentRegistry` / `SkillContentRegistry` 不再暴露 `GetSkillDefsTyped()`，raw `SkillDef` bucket 只允许通过 validation snapshot、resource load/projection、或 process-shutdown finalizer drain 专用方法触达；`GameSession.GetProfessionDefsTyped()` / `GetAchievementDefsTyped()` / `GetQuestDefsTyped()` / `GetItemDefsTyped()` / `GetRecipeDefsTyped()` / enemy typed getter 的正式真相源是 session typed index，`_profession_defs` / `_item_defs` 等 Godot dictionary 只保留为 content/test/validation 边界投影，不要在 runtime getter 里从这些 payload 字典回扫重建索引，也不要假设共享 `GDictionary` 变更会自动代表 formal runtime 输入；`BattleSpecialProfileRegistry.Rebuild(IReadOnlyDictionary<StringName, SkillDefinition>)` 的边界只从 DTO skill catalog 建 profile 授权索引，manifest 的 `owning_skill_ids -> profile_id` runtime 映射必须由当前 typed skill catalog 上的 `SpecialResolutionProfileId` 授权，不要再按 `SkillDef.skill_id` value 或 manifest 自身 owner 列表恢复 string-key-only skill_defs；battle special profile 的 authoring snapshot 可以携带 `profile_resource` 供 manifest/validation 测试读取，runtime setup 必须改走 `GetBattleSpecialProfileRegistryRuntimeSnapshot()` 的 plain payload 与 `IBattleSpecialProfileView` typed DTO 视图，不要把 `profile_resource` 放进 `RuntimePayloadStore` 或在 resolver 中从 payload 反查 Resource；`GameContentCatalog` 内部只长期保存 runtime special profile plain snapshot，authoring snapshot 若携带 `profile_resource` 必须保持 static content 边界。
...
- 生命周期约束：`GD.Load` / `.tres` borrowed content 与 registry build/merge/template/patch 产生的 derived static content 都是 process-lifetime static content，由 `GodotContentOwnership` 强持有整个 Resource graph；normal/test finalizer drain 只 retain static keep-alive store，不 suppress / clear borrowed 或 derived static wrappers 后 forced GC，process-exit 才允许 shutdown-only static suppress。`GodotTypedResourceGraphWalker` 是 static content owner graph 的统一 walker，必须覆盖 project Godot wrapper field、auto-property backing field、`Godot.Collections.Array` / `Dictionary` / `Variant` wrapper；新增 content wrapper 字段时不要在 consumer getter 里补生命周期逻辑。`SkillCatalog` 缓存的 skill-level `SkillEffectiveCombatDefinition` 是 plain C# 派生视图，不持有 `SkillDef` / `CombatSkillDef` wrapper；AI / runtime scoring 只校验 ownership，不在消费点 suppress/dispose 或按类型做全局兜底。
...
- Battle loot rollback 约束：`GameRuntimeBattleLootCommitService` 的仓库提交回滚只保存 typed `WarehouseState` 副本和 `Dictionary<StringName, bool>` fate flag snapshot，恢复时走 `PartyState.ApplyFateRunFlagsTyped(...)`，不要再把 fate flags 经由 `Godot.Collections.Dictionary` / `RuntimePayloadCopy` 作为事务真相源。
...
- World runtime data 约束：`WorldMapDataContext` 内部 root/active 世界状态应通过 `WorldRuntimeData` 持有，settlement state、submap stack、mounted submap、encounter anchors、world events、fog/world npc 等运行时读写先进入 typed owner，再通过 `WorldMapDataProjection.Project(...)` 写回 save/window/proxy 边界；`root_world_data` / `active_world_data` 只作为 Godot/save/proxy 边界投影保留，不应在业务逻辑新增字典真相源，也不要让 `BindRootWorldData(...)` 传入的字典成为后续原地同步对象。`RuntimeTransactionRollbackState` 的 world rollback snapshot 也应保存 `WorldRuntimeData`，不要恢复按 key 手写捕获/回投的 nested dictionary snapshot；`Capture(runtime, scopes)` 按事务 scope 裁剪捕获，world scope 捕获走 `WorldRuntimeData.DuplicateState()` typed 浅拷（不可变元素共享引用、可变 `EncounterAnchorData` 逐字段拷、fog 走 plain clone），不要退回整图 ToDict/FromDict 捕获——party-only mutation（如 contingency setup save/charge/clear）必须传入 transaction 以跳过整图 world ToDict/FromDict round-trip，`Restore` 对"标记了但未捕获"的 scope 报 GameLog.Error 并跳过，不要把 `Capture(runtime)` 无 scope 全量捕获重新引入 party-only 命令路径。
- Runtime payload 约束：runtime/save/window 边界确实需要 `Godot.Collections.Dictionary` / `Array` projection 时，正式运行态应先进入 typed DTO / plain C# snapshot，再通过 `RuntimePlainPayload` / `RuntimePayloadCopy` 这类投影边界短暂生成 Godot wrapper；plain list owner 应保持 C# dictionary entries，不要恢复 `RuntimePayloadStore`、`GameRuntimePayloadEntry`、`RuntimePayloadList` 或 Godot wrapper 真相源；发现 Godot Object/Resource payload 必须在入口抛出而不是静默转 string，调用方应显式投影成 id/path/plain DTO 或改走 typed view；确实仍需长期保存 wrapper 的历史边界必须立即 `RuntimeStateLifecycle.MarkValueGraphFinalizerless(...)` 登记为 runtime value graph。不要在 command handler、rollback、writeback、HUD adapter、snapshot builder 或 world context 中裸 `Duplicate(true)` 后长期保存 wrapper。
...
- 负责：据点服务窗口、商店窗口、人物信息展示；`SettlementWindow` 的服务 entry / member availability / service payload cache 应在窗口内部保持 typed/plain C# 结构，Godot `Dictionary` 只在 `ShowSettlement(...)` 输入、modal payload 构建和 action signal 边界出现，不要恢复 `_services` Godot array 或长期 duplicated service `GDictionary` cache；`ShopWindow` 的 entry payload cache 同样应保持 plain C# dictionary，确认提交时才投影 Godot payload。
...
- 负责：队伍编成、转职选择、奖励弹窗、角色摘要展示、触发术 setup 窗口；`PromotionChoiceWindow` 的 prompt/selection 字典只属于 UI 投影与 Godot signal 边界，窗口内部 choice 列表应保持 typed/plain `PromotionChoiceEntry` 状态并在确认时才投影 selection payload，提交后必须在 scene/runtime adapter 处转换为 `PromotionSelectionData`，不要让 selection 字典继续进入 runtime/progression 规则链；`ContingencySetupWindow` 只渲染当前 setup 状态并通过 save/charge/clear 信号提交意图，不能直接改 `PartyMemberState`。
...
- 补充约束：当前战斗可用技能入口的正式查询面是 `BattleSkillAvailabilityService` / `BattleSkillAvailabilityView`。HUD slot、手动 slot 选择、selected skill sync、preview/issue access gate 都应按 `skill_entry_id` 解析当前 entry；`selected_skill_id` 只作为 catalog/display 辅助，不能作为 stale entry 的 fallback。`known_active_skill_ids` 仍只表达长期已知主动技能，不要写入装备授予技能；装备授予技能必须从 `BattleUnitState.equipment_ability_sources` 反查当前 battle runtime 的 equipment ability binding index，再由 `binding.granted_actions` 生成 `EquipmentSkill` entry；带 `usage_period_kind` / `max_uses_per_period` 的装备技能入口必须使用 battle environment snapshot 的 `WorldStep` 读取 `EquipmentInstanceState.ability_usage_periods` 判定可用性，成功执行后由 battle runtime 提交到 battle-local equipment view，战后 local writeback 再复制回成员装备状态，不要把次数放进 `known_active_skill_ids`、全局字典或技能定义本身。
- 补充约束：装备能力施加的装备专属状态语义应由 `ApplyStatusActionPayloadDef` / `ScheduleAreaEffectActionPayloadDef` 的 typed 字段配置并投影到 `ApplyStatusActionPayloadDefinition` / `ScheduleAreaEffectActionPayloadDefinition`，再经 `BattleEquipmentAbilityRuntimeService`、`BattleDelayedAreaEffectSystem`、`BattleTerrainEffectSystem` 写入 `BattleStatusEffectState`；`BattleStatusSemanticTable` 只保留通用战斗状态语义，不要把单件装备的 stack/display/debuff/dispellable/timeline damage 规则重新硬编码进去。新增装备状态时必须同时检查 `EquipmentAbilityContentRegistry` schema、对应 `data/configs/equipment_abilities/*.tres`、`BattleStatusEffectState`、`BattleStatusSemanticTable.MergeStatus(...)` 与该武器的 `tests/battle_runtime/runtime/run_*_weapon_ability_regression.cs`。
...
- 补充约束：`BattleContingencySystem` 属于 battle runtime orchestration，负责从 persistent setup 初始化战斗内状态、收集 hook fact、经 `ContingencyTargetResolverService` 解析目标并排队/执行 `AutoCastRequest`；consumed 状态的单一真相是 `BattleUnitState.MarkContingencySetupConsumed` 印章（sidecar 的 `IsSetupConsumedForMember` 经 owner unit 解析，不要恢复 sidecar 内平行 consumed 字典）；battle finalization 只在结算提交成功后通过 character gateway 写回 consumed setup，失败时必须随 finalization rollback 回到战前 party/setup 状态；consumed setup 的写回契约是 `IBattleRuntimeCharacterGateway.ValidateContingencyConsumedSetups/CommitContingencyConsumedSetups`（`CharacterBattleWritebackService` 单一校验源），`BattleRuntimeModule.EndBattle` 先对全部存活单位跑只读 Validate 再逐一 Commit，不要恢复 `is CharacterManagementModule` 向下转型或"非具体模块即静默成功"的 fallback。
...
- 补充约束：`BattleDamagePreviewRangeService` 的正式输入只接受 `CombatEffectDefinition` / `BattleUnitReadView` 或当前源单位，不再保留 `CombatEffectDef` overload；资源层 `CombatEffectDef` 必须在加载/测试 Resource-boundary 先投影成 DTO，伤害预览测试也应直接构造 `CombatEffectDefinition`，不要为了 preview range 恢复 Resource effect 业务态。
- 生命周期约束：`BattleTerrainEffectSystem`、`BattleDelayedAreaEffectSystem`、`BattleRatingSystem`、`BattleUnitFactory`、`BattleChargeResolver`、`BattleRepeatAttackResolver`、`BattleSkillMasteryService`、`BattleRuntimeSkillTurnResolver`、`BattleShieldService`、`BattleEquipmentAbilityRuntimeService`、`BattleSkillExecutionOrchestrator`、`FateRuntimeModule`、`MisfortuneService` 与 dev/profiling helper `AiTraceRecorder` 是 plain C# runtime helper，`BattleRuntimeModule` 只需通过普通 `DisposeRuntime()` / `IDisposable` 清 runtime weak ref、event subscription 与缓存，不要恢复成 `RefCounted` 或纳入 GodotObject owned-dispose 路径；`BattleFateEventBus` 仍声明 Godot Signal，可继续保留 `RefCounted`。
...
- 补充约束：装备能力源进入战斗的正式桥接是 `GameContentCatalog.GetEquipmentAbilityBindingDefinitionsTyped()` / `GetTraitDefsTyped()` -> `BattleRuntimeModule` typed 内容索引 -> `BattleEquipmentAbilityProjectionService` -> `BattleUnitState.equipment_ability_sources`。玩家单位由 `BattleUnitFactory` 在创建、`RefreshBattleUnit`、`RefreshEquipmentProjection` 时基于 battle-local `EquipmentState` 与 `effective_trait_instances` 重算，不得回查角色持久装备；敌人单位由 `EncounterRosterBuilder` 在模板入场时把 `attack_equipment_item_id` 投影为 battle-only source。建成后的战斗规则只能从 `BattleUnitState` 读取装备能力源和 `creature_type_tags`，不得从敌人模板、仓库或角色状态二次读取。
...
- 补充约束：`MeteorSwarmProfile` / `.tres` 只在 `BattleSpecialProfileRegistry` authoring/content 边界加载、校验和投影；runtime/resolver 只能通过 `IBattleSpecialProfileView` 读取 plain C# `MeteorSwarmProfileData` / `MeteorSwarmImpactComponentData` / `MeteorSwarmTerrainProfileData`，不要从 `RuntimePayloadStore`、`GDictionary` snapshot 或 `GD.Load` 反查 `MeteorSwarmProfile` Resource，也不要在 profile 缺失时 fallback 到常量继续生成退化 plan。
- 补充约束：`MeteorSwarmCastContext`、`MeteorSwarmTargetPlan`、`MeteorSwarmCommitResult`、`MeteorSwarmTargetOutcome` 这组 meteor DTO 也应保持 plain C# 数据载体；context/plan 的正式 skill 标识是 `skill_id`，不要为 runtime 内部 typed 传递再次挂回 `SkillDef` / `CombatCastVariantDef` / `MeteorSwarmProfile` / `RefCounted` / `GlobalClass`。
...
- 补充约束：`BattleCommand` 是 plain C# runtime command DTO，目标单位、目标格、装备占用槽位等正式状态由 internal typed `List<T>` backing 承载，public Godot Array 属性只作为 UI/test/Godot 边界投影；命令根不参与 GodotObject lifecycle，不要恢复 `RefCounted`、`RuntimeStateLifecycle.MarkFinalizerless(...)` partial constructor 或对命令根调用 GodotObject disposal。AI 行动 helper 内部的候选坐标、screening contact destinations、distance-band destinations 等临时集合应保持 `List<Vector2I>` / `IReadOnlyList<Vector2I>`，只在 UI/Godot API/payload 边界投影为 Godot Array，不要为内部排序或 foreach 临时创建 `Godot.Collections.Array<Vector2I>`。`BattlePreview` 也是 plain C# runtime preview DTO，log/target/random-chain 集合由 typed backing list 承载；`AttackPreviewData` 是 plain C# hit-preview DTO，HUD/snapshot/hover 的 hit-preview public payload 只能投影为 `GDictionary`，不要把 live `AttackPreviewData` object 放进 Godot payload；`save_branch_preview` 是 typed `BattleSaveBranchPreviewData` backing state，与 `damage_preview` 同级由 runtime/AI/HUD 边界投影，不要把 Godot Dictionary 当成正式 preview 状态，也不要恢复 preview 构造期 lifecycle 注册。`BattleState` 的 report entries 与 promotion queue live owner 是 plain C# dictionary list，public `report_entries` / `promotion_queue` 只作为 Godot/test/snapshot 边界投影；`BattleEventBatch` / `BattleCommonSkillOutcome` 都是 plain C# runtime result DTO，changed ids/coords/log/progression delta 使用 typed backing，report entries 的内部 owner 是 plain C# dictionary list，只在 public `report_entries` / `ReportEntriesTyped` / log payload 边界投影 Godot Dictionary；battle start failure 的 live owner 是 typed `BattleStartFailureSnapshot`，只在 report/analysis 边界投影，不要恢复 `_last_start_failure` 的 `RuntimePayloadStore` 真相源；不要恢复 battle state/batch/outcome 根的 `RefCounted` / GodotObject disposal 或 `RuntimePayloadList` report/promotion 真相源。
- 补充约束：`BattleUnitState` 的技能等级、锁定命中加值、伤害抗性、冷却与次数状态应由 typed map owner 承载，`BattleState` 的地形列和 layered barrier payload cache 应通过 `BattleState` owner API 读写；`BattleState` / `BattleUnitState` 是 plain C# battle state，不要恢复 lifecycle partial constructor 或构造期 `RuntimeStateLifecycle.MarkValueGraphFinalizerless(this, ...)`；`BattleUnitState` 只保存 `battle_sprite_texture_path`，不要把 `EnemyTemplateDef.battle_sprite_texture` / `Texture2D` content resource 放进 live battle state，渲染层按 path 加载并缓存 texture；runtime edge face cache 的 live owner 是 `Dictionary<Vector3I, BattleEdgeFaceState>`，`BattleEdgeFaceState` 是 plain C# runtime DTO，不参与 GodotObject 生命周期，不要恢复 `RuntimePayloadStore` / Godot Dictionary `Variant.Object` edge cache。`BattleCellState.timed_terrain_effects` 的 live owner 是 `List<BattleTerrainEffectState>`，`BattleTerrainEffectState` 是 plain C# terrain DTO，不参与 GodotObject 生命周期，residual `@params` 的 owner 内部也是 plain C# dictionary，只有 `ToDictionary()` / `FromDictionary()` save/schema 边界投影为 Godot Array/Dictionary payload；layered barrier runtime 真相源是 `BattleBarrierStore` / `BattleBarrierInstanceState`，`BattleBarrierService` 和 `BattleAiMutationGuard` 应直接读写 typed barrier entry / snapshot，规则、AI、地形和屏障服务不要恢复直接持有或写入这些 Godot dictionary 字段，只有 save/schema、terrain generation、HUD/test payload 断言等边界显式投影。
- 生命周期约束：battle runtime/save state 中仍保留的 Godot payload wrapper 只能在 owning 构建/导入边界通过 `RuntimeStateLifecycle.MarkValueGraphFinalizerless(...)` 注册为 runtime state graph；runtime `new` / `Duplicate(true)` / `DuplicateForRuntime(...)` Resource graph 必须进入当前 `GodotTransientResourceScope`，例如 damage/effect resolver、skill execution 和 terrain factory。`BattleAiRuntimeActionPlan` 是 AI authored/generated runtime action plan owner：authored `.tres` action 作为 borrowed Resource entry 复用，generated `use_unit_skill` / `move_to_range` / `use_random_chain_skill` / `use_multi_unit_skill` / `move_to_multi_unit_skill_position` / `use_charge` / `use_charge_path_aoe` / `use_ground_skill` 使用 plain runtime entry（CLR `List<StringName>` / typed spec / typed candidate request）并经 typed evaluator/candidate request 评估，不再创建 generated action Resource / collection wrapper。consumer、score、preview、hook 读取点只能 assert/log ownership，不做 suppress、dispose、free 或 unknown wrapper 类型兜底。
- 补充约束：`BattleUnitState` 的状态效果正式源是 `BattleStatusEffectCollection`，读写只走 `GetStatusEffect()` / `GetStatusEffectsTyped()` / `GetSortedStatusEffectIdsTyped()` / `SetStatusEffect()` / `EraseStatusEffect()`；`status_effects` 只能作为 `ToDictionary()` / `FromDictionary()` save/schema payload key 出现，不要恢复 public/live `GDictionary status_effects` 或从投影字典回扫运行时状态。`BattleStatusEffectState` 是 plain C# battle DTO，不参与 GodotObject 生命周期；其 runtime 语义字段必须保持显式 typed 字段，source-bound 攻击检定修正这类状态侧命中修正由显式字段进入 `BattleAttackCheckPolicyService`，不要把它藏回 residual params 或装备能力临时状态；residual `@params` 的 owner 内部也是 plain C# dictionary，只在 `ToDictionary()` / `FromDictionary()` 边界投影 `GDictionary`；`BattleStatusEffectParams` 只在 content/save params 边界解析并剥离 residual save payload，residual owner 同样是 plain C# dictionary，不要让规则、AI、被动或特殊技能 resolver 直接读取 status `@params` 来决定效果。
...
- 补充约束：`BattleUnitState.equipment_ability_sources` 是战斗内装备能力来源的正式 typed state，live owner 必须是 `List<BattleEquipmentAbilitySourceState>`；`BattleEquipmentAbilitySourceState` 是 plain C# DTO，不参与 GodotObject 生命周期，`AbilityIds` 保存当前 V1 的 binding id 列表。玩家 source 必须携带持久 `SourceEquipmentInstanceId` 供战后写回，敌人 `EnemyBattleOnlyEquipment` source 必须保持空 instance id 且不能持久化到战斗外。`creature_type_tags` 是战斗单位上的分类事实，装备能力条件、预览、AI 或执行链需要生物分类时只能读取该字段。`BattleEquipmentAbilityRuntimeService` 是装备能力在战斗规则侧生效的 typed owner，攻击修正、命中后反应、击杀触发、战利品倍率修正、状态施加、装备耐久提交、延迟地形效果排程和条件事实查询都应从 unit source 与绑定定义进入该服务；装备授予主动技能不由该 service 注入 `known_active_skill_ids`，而由 `BattleSkillAvailabilityService` 在查询可用技能时从同一 source/binding 链即时生成 `EquipmentSkill` entry。延迟区域地形效果由 `BattleDelayedAreaEffectSystem` 承接时间线排程，落地后继续进入 `BattleTerrainEffectSystem` / `BattleTerrainEffectState` 的 typed terrain owner，不得让规则层回查 item/trait catalog 或角色持久装备。命中后额外伤害骰必须保持 typed result，并由 `BattleDamageResolver` 按 `damage_tags` / `damage_type` 分组为主伤害同 tag bonus 或独立附加 `DamageEventResult`，不要把 fire/acid/poison 等配置化伤害混入主物理伤害事件；装备能力配置产生的当前轮临时状态应写入 `BattleUnitState` typed per-turn charge helpers，并以装备实例 id（敌人 battle-only 装备退回 item id）+ `state_key` 作为 owner key，让同一装备的多个 trait binding 可以通过配置协作，但不能跨装备实例串状态；`once_scope=turn` 这类装备能力次数门槛应使用 source instance / binding / reaction / action 组成 key，不要在 ability service 内新增临时字典状态。schema 校验/clone/save roundtrip 由 `BattleUnitState` 拥有，只有 `ToDictionary()` / `FromDictionary()` 边界投影为 payload。
...
- 负责：battle HUD、棋盘绘制、单位/prop 渲染、相机、overlay、hover 展示；`BattleHudAdapter` 是 plain C# HUD projection helper，只消费 runtime 提供的 `BattlePreview` 投影 HUD/hover payload，不拥有技能命中、伤害、射程或目标合法性计算，也不要恢复 `RefCounted` / `GlobalClass`；`BattleUiTheme` 是 plain static C# theme helper，`BattleBoardRenderProfile` 是 plain C# render-profile 配置对象；`BattleBoardController` 是 `BattleBoard2D` 持有的 plain C# board helper，source id / tileset / texture / target-hit-badge cache 的 owner 应保持 CLR `Dictionary` / `List`，Godot `Dictionary` / `Array` 只作为 render profile source spec、panel 输入或 Godot API 边界，controller `Clear()` / `BattleBoard2D._ExitTree()` 必须清 typed cache、断开 TileMapLayer.TileSet 并 drain runtime scope；`BattleMapPanel` 的 battle equipment snapshot / backpack entry cache 只保存 plain CLR dictionary/list，Godot `Dictionary` 只在 snapshot 应用和 UI/command 局部投影边界出现；`BattleMapPanel` 动态创建的 viewport/board/equipment overlay 节点由场景树释放，panel `_ExitTree()` 只断信号、清 runtime/context 引用、suppress C# wrapper finalizer 并清字段，不手动释放 scene-owned child nodes。
...
- 文件：
  ...
  - `tools/*.py`
  - `tools/*.gd`
- 负责：headless 回归、contract 验证、fixture、截图/签名辅助；`TestHarness.Finish(...)` 是测试退出前的集中生命周期闸口，应在返回退出码前执行 `TestResourceOwnership.Drain()` 和 `GodotSharpCleanup.CollectPendingFinalizers()`，保持 idempotent，并确保 `SceneTree.Quit(_test.Finish(...))` 这类调用先 drain 再 quit；普通 test runner / finally 不应直接调用 `GodotSharpCleanup.CollectPendingFinalizers()` 或 `GodotObjectLifecycle.CollectPendingFinalizers()`；测试 helper 中的 `GodotSharpCleanup.DisposeGodotObject(GodotObject)` 只能 register/suppress runtime value graph，不承担 native dispose 语义，功能测试不要为了生命周期手工拆 Resource 子图；功能测试需要读取正式 `SkillDef` `.tres` 时应通过 `TestSkillDefinitionProjection.LoadSkillDefinition(...)` 在加载边界登记 borrowed content 并立即投影为 `SkillDefinition`，普通 runtime fixture 不要继续把 `SkillDef` 传入服务；`AiTraceRecorder` 内部事件、调用栈和函数统计是真正的 plain CLR list/dictionary owner，`GetEvents()` / `GetFuncStats()` / trace dump 只在输出边界投影 Godot payload；normal/test forced GC 只处理 known owners（static content retain-only、runtime state known graph、owned transient scope drain/test quarantine），不要在普通测试或 forced GC drain 里新增未知 wrapper 全局 suppress，也不要 suppress 或清空 process-lifetime borrowed/derived static content keep-alive store。
...
- 文件：
  ...
  - `data/configs/enemies/rosters/*.tres`
- 负责：敌方模板、AI brain/state/action、generation slot、transition rule、wild encounter roster 内容；`EnemyTemplateDef.base_attribute_overrides` / `attribute_overrides` / `skill_level_map` 的正式 key 是 `StringName`，schema 校验、属性投影和 `GetSkillLevelTyped()` 不再从 string key 恢复；`BuildBrainIndex` / `BuildItemDefIndex` / `WildEncounterRosterDef.BuildKnownTemplateIdSet()` 也只从正式 `StringName` key 物化 typed index，不要用 value 自带 id 或 string key 补索引；敌方内容校验需要 skill catalog 时应消费加载边界已经投影好的 `SkillDefinition` index，生产 `EnemyTemplateDef` 不再提供 `SkillDef` resource dictionary 到 DTO 的投影 helper。
