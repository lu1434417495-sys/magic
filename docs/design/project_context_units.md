...
- 桥接：装备能力的命中检定加值、优势与防御组件调整由 `BattleEquipmentAbilityRuntimeService` 从 `BattleUnitState.equipment_ability_sources` 收集，经 `BattleAttackCheckPolicyService` 汇总后交给 `BattleHitResolver` 生成本次 `AttackCheckInput`；忽略 AC component 等规则只调整本次目标 AC，不改写目标 `attribute_snapshot[armor_class]`。
