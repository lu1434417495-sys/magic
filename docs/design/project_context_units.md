...
- 补充约束：当前战斗可用技能入口的正式查询面是 `BattleSkillAvailabilityService` / `BattleSkillAvailabilityView`。HUD slot、手动 slot 选择、selected skill sync、preview/issue access gate 都应按 `skill_entry_id` 解析当前 entry；`selected_skill_id` 只作为 catalog/display 辅助，不能作为 stale entry 的 fallback。`known_active_skill_ids` 仍只表达长期已知主动技能，不要写入装备授予技能。
...
- 补充约束：`BattleUnitState.equipment_ability_sources` 是战斗内装备能力来源的正式 typed state，live owner 必须是 `List<BattleEquipmentAbilitySourceState>`；`BattleEquipmentAbilitySourceState` 是 plain C# DTO，不参与 GodotObject 生命周期，`AbilityIds` 保存当前 V1 的 binding id 列表。玩家 source 必须携带持久 `SourceEquipmentInstanceId` 供战后写回，敌人 `EnemyBattleOnlyEquipment` source 必须保持空 instance id 且不能持久化到战斗外。`creature_type_tags` 是战斗单位上的分类事实，装备能力条件、预览、AI 或执行链需要生物分类时只能读取该字段。schema 校验/clone/save roundtrip 由 `BattleUnitState` 拥有，只有 `ToDictionary()` / `FromDictionary()` 边界投影为 payload。
