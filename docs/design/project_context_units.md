...
- 边界：`WarehouseState` / `EquipmentState` / `EquipmentInstanceState` 家族是 plain C# runtime/save owner。`ItemDef` / `RecipeDef`、weapon profile/dice、trait-roll group 与 equipment requirement 是 `.tres` authoring schema；`ItemContentRegistry` / `RecipeContentRegistry` 仅在 `ContentSnapshotBuilder` 的同步构建作用域内借用 raw Resource，并把模板合并、技能书生成、武器属性与装备需求投影为递归只读 `ItemDefinition` / `RecipeDefinition`。process snapshot seal 后，session、runtime、battle、settlement 与 UI 只借用同一个 definition 索引，不创建 registry mirror、Resource duplicate 或 Godot Dictionary 回投；无效 nested authoring 节点按索引化内容路径立即失败。物品价格的 basis-point 缩放由 content-definition 层的 `ItemPriceRules` 唯一实现，以 `long` 完成乘法和 half-up 舍入并将超出 `int` 返回契约的结果饱和；authored `ItemDef` 与 typed `ItemDefinition` 的既有入口只委托该规则。物品内容（category / equipment type / 伤害标签 / 固定 trait ids / trait roll groups）跨 trait 规则由 `ItemTraitContentValidator` 校验；装备 trait mint 时机由 `EquipmentRules` / `EquipmentTraitRollService` 拥有——仓库分配稳定装备 instance id 后才生成 `equipment_roll` trait，不在 transient drop 阶段或已入库实例上重 roll。套装不是持久装备类型；`GearSetEvaluationService` 只从当前 `EquipmentState`、只读 item/set definition 计算不同有效成员、阈值属性和阈值 trait，破损件不计数、重复副本不重复计数，结果不写入装备或存档。
...
- 历史物理拆分（零行为变化，2026-07-19/20）：`SkillContentRegistry` 的 combat profile/effect 校验拆分为 `SkillCombatProfileValidator`（`TypedEffectParamTargets` 映射表随迁）、伤害效果/耐久/路径 AoE 校验拆分为 `SkillDamageEffectValidator`、execute/graded-save/temporal/save-bonus 校验拆分为 `SkillExecuteEffectValidator`（`GradedSaveExecuteParamKeys` 随迁）；这些 Resource validator 自 2026-08-19 起不再属于 production publication gate，仅服务尚未迁移的 synthetic Resource 回归 helper。`EquipmentAbilityContentRegistry` 的逐 payload 校验仍由 `EquipmentAbilityPayloadValidators`、binding/reaction 级校验由 `EquipmentAbilityBindingValidator`、Resource→Definition 投影与枚举解析由 `EquipmentAbilityDefinitionProjection` 拥有。
...
- 装备能力内容边界：`EquipmentAbilityContentRegistry` 在加载期把 authored pack/binding/payload Resource 投影为 plain definitions；trait、skill 与外部 status catalog 是生产构建必填依赖，空目录仍执行 membership 校验，缺失目录直接阻止 snapshot。damage tag / equipment slot / cognition kind 直接消费各自 closed typed rule；status 由 `StatusContentRules` 的系统声明、技能 effect、trait passive status 与全部装备 pack 的创建型 action 组成两阶段声明目录，再校验 condition/fact/action 引用，battle 语义表只消费内容声明并附加运行时语义。battle/runtime 不接受 raw `ItemDef`。召唤、消费召唤物、按召唤物数量/距离产生攻击检定修正必须走 `SummonUnitsActionPayloadDef` / `ConsumeSummonedUnitsActionPayloadDef` / `SummonedUnitAttackRollModifierActionPayloadDef` 与 `summoned_unit_count` fact，召唤动作必须显式声明基础认知；附近生物/友军计数走 `nearby_unit_count` / `nearby_ally_count` fact；击杀后的额外武器攻击走 `ImmediateWeaponAttackActionPayloadDef`；命中后升级为暴击走 `CriticalHitOverrideActionPayloadDef`；不进入技能可用性列表的装备内部结算走 `TriggerSkillActionPayloadDef`，仍引用正式 `SkillDef` 并复用技能效果、豁免、范围和死亡规则；这两个自动动作不能引用需要手动选择挡位的蓄力技能，加载期引用校验与 runtime executor 都必须拒绝。AP 变动与下一行动回合 AP 清零走 `ModifyActionPointsActionPayloadDef`；装备期间影响行动/读条进度倍率的被动走 binding 级 `EquipmentTemporalProgressModifierDef`，装备期间限制有效认知的被动走 binding 级 `EquipmentCognitionCeilingModifierDef`；装备触发的固定伤害减免走 `DamageReductionActionPayloadDef`；按实际伤害等 fact 计算治疗走 `HealFromFactActionPayloadDef`；状态栈消费走 `ConsumeStatusStacksActionPayloadDef`；随机分支走 `EquipmentOutcomeTableDef` / `EquipmentOutcomeEntryDef`，仍由分支内 typed action payload 表达效果；持久成长计数由 `ModifyAbilityStateActionPayloadDef` 写入，计数同步出的保存状态由 `EquipmentAbilityStateSchemaDef` 的同步字段声明；临时战场边特征/裂隙走 `ApplyEdgeFeatureActionPayloadDef` 的 from/to selector、`duration_tu`、edge feature 字段与 `max_active_edges`；武器近/远程分类条件读 `weapon_range_type` fact。资源只声明 binding/state/source/target 关系，不在技能 id 或武器 id 文本中推导召唤物、附近单位、追击目标、时间进度、认知、伤害减免、伤害转治疗、状态栈消费、随机分支、临时边或成长状态键。
...
- 装备伤害骰边界：装备能力 `add_damage_dice` 与状态来源绑定的武器额外骰由 `BattleDamageResolver` 统一结算；来源绑定骰只在状态 `source_unit_id` 匹配攻击者且本次 effect 实际包含武器伤害时触发，并入 bonus damage dice 以复用暴击额外骰路径；装备能力 `AddDamageDiceActionPayloadDef.require_weapon_damage` 默认 true 以保持既有武器内容语义，显式 false 才允许徒手或法术攻击的主直接段触发。装备能力骰数可由通用 fact（如装备能力状态 fact）按 authoring 公式放大，具体武器的成长键仍只在 `.tres`；`subtract=true` 的骰只扣减匹配主伤害标签的本次基础伤害，不生成负数额外伤害段。`CombatEffectDef.weapon_dice_multiplier` 只复制基础物理武器骰，不复制 flat bonus、装备追加骰或非物理附伤；`bonus_weapon_dice_multiplier` 在条件成立时生成独立伤害事件并继承本次重击；`bonus_damage_separate_event` 可把固定条件骰拆成独立伤害事件并继承重击。多次常规武器攻击仍逐击查询并结算装备附伤。
...
- 细节文档：
  ...
  - `tests/runtime/validation/run_skill_tres_to_json_converter_regression.cs`
  - `tests/tooling/test_skill_tres_to_json_converter.py`
  - `tests/text_runtime/**/*`
  - `tests/world_map/**/*`
  - `scripts/dev_tools/*.cs`
  - `scripts/tools/run_content_json_schema_export.cs`
  - `scripts/tools/run_skill_tres_to_json_converter.cs`
  - `scripts/systems/content/json/ContentJsonTransactionalDirectoryPublisher.cs`
  - `tools/*.py`
  - `tools/*.gd`
...
- skill tres-to-JSON 工具边界：`run_skill_tres_to_json_converter` 是 skill-specific headless 入口，不提供 domain/input discovery；默认写入仓库外的唯一持久系统临时目录，`--output-dir` 只接受与工程目录不重叠的 host path，`--in-place` 才能写固定 `data/configs/json/skills`。七项全域先在内存完成 Resource adapter 与 canonical writer，随后 `ContentJsonTransactionalDirectoryPublisher` 才写同卷 sibling staging；新目标单次 move，既有精确七文件 generation 经 backup/promote/失败回滚发布。该边界只承诺可恢复的 all-or-nothing publication，不宣称 Windows crash-atomic；focused regression 才从真实 staging 输出经共享 descriptor 复读并逐字段比较，production converter 不把 JSON round-trip 变成第三段逻辑。
...
- skill-icon catalog 回归边界：正式 process snapshot 锁定非空技能图标 occurrence 与唯一 asset ID 数，并用真实 resolver 逐项验证 `Texture2D`；unknown 与已登记 wrong-type ID 必须经 `ProcessContentHost` 实际 publication preflight 失败，证明 rollback 后没有 snapshot、epoch 或 sealed partial publish。HUD 回归同时覆盖登记 ID、空 ID 短名 glyph 与 unknown 非空 ID 无路径/技能/默认图标回退。
...
- 生成技能 BattleSim 门禁边界：`scripts/systems/battle/sim/generation` 是 content 层 `ISkillGenerationBattleSimulationGate` 的唯一 tooling adapter；`BattleSimContentProvider` 可借用同一 snapshot 的敌人、屏障与 profile，同时用只读 combined skill index 覆盖技能视图，不修改或重新发布 process snapshot。每个可采样的 enemy-targeted damage candidate 与同 surface benchmark 使用相同单位 id、属性、武器、AI brain、地图和 20 个固定 seed，各自进入现有 `BattleSimRunner`；只有双方至少各 20 个 completed samples 且 candidate 至少被尝试 3 次才进入强度判定。比较口径固定为 ally win-rate delta basis points 与 ally damage-per-completed-run ratio basis points；高离群阈值为 `+3500 bp / 22500 bp`，低离群阈值为 `-3500 bp / 4500 bp`，任一越界均产生定位到 candidate `combat_profile` 的稳定诊断。该 runner 只由 opt-in `/simulation/` 回归和生成验证工具调用，不进入 routine regression。
