...
- 技能等级值图边界：`CombatSkillLevelOverrideImportModel` 以 19 个 nullable typed 字段保留“缺席”与显式零值差异，四个 closed 字段在 JSON/Resource 投影边界显式映射为 CLR enum；`SkillDescriptionVariables` 防御复制并按 Ordinal 稳定排序字符串键值。`CombatSkillImportModel.LevelOverrides`、`SkillImportModel.LevelDescriptionConfigs` 与对应 `SkillDefinition` 属性只暴露按等级排序的递归只读 typed 图，不提供旧 `Dictionary<string, object>` 重载。`SkillJsonImportParser.Parse(...)` 是 internal DTO 到 import model 的同步封闭入口，level key、null、空 override、字符串值和 closed business string 在该入口 fail closed；effect payload 的 parser-only `object`/`JsonElement` carrier 不得逃逸到 import model。`SkillTresImportAdapter.TryAdapt(...)` 是同步 borrowed `SkillDef` → 同一 plain import model 的唯一 Godot 边界，只搬运并封闭 Resource/Variant/legacy `params` 形状，不执行仍由 Resource validator 拥有的业务数值规则，也不让 Resource、Godot collection、路径或 UID 逃逸。
- 技能 JSON 作者工具边界：`SkillContentJsonAuthoringDomain.CreateImportDescriptor(...)` 是 document load、file-local template merge、strict parser、identity normalize 与 domain-local validation 的唯一组合入口，Godot focused 与纯 .NET offline CLI 复用同一契约；schema exporter 从完整 `SkillJsonDto` 生成 full entry、schema-only `template` control 与递归 partial template view，nullable carrier 的“字段缺席”不等于显式 JSON null。`SkillTresToJsonConverter` 只为 stage-1a 明确的七个 prismatic ward pilot 按固定顺序执行 borrowed `SkillDef` → `SkillTresImportAdapter` → `SkillCanonicalJsonSchema.EntrySchema` / shared canonical writer，并生成一文件一 full entry、空 templates、固定 family 的 document envelope；生产转换不调用 JSON importer 或复制第三套字段投影。纯 .NET offline CLI 输出稳定 rule ID、source label 与 JSON pointer，不启动游戏、不构建 snapshot，也不调用 Resource-bound skill validator。`PendingCastBindingModeKind` 的 plain enum owner 位于 content/skills，Godot interop 只负责它与 `StringName` 的边界映射。
...
- CombatSkill 目标类型边界：`CombatSkillDef.excluded_target_creature_type_tags` 由 `SkillCombatProfileValidator` 校验非空且唯一，再投影为 immutable `CombatSkillDefinition.ExcludedTargetCreatureTypeTags`；目标合法性、canonical preview 与 AI 候选共同消费该排除集合，不能用单个 effect 的正向 tag 门禁或技能 id 特例替代。随机书技能候选以实际起始等级和一槽费用排除 MP 消耗超过 40 的内容，选中耗蓝技能后 `RandomStartingSkillResourceSupportService` 将随机法力池抬到至少可支付一次该技能。
- 主动施法反应内容边界：`CombatSpellReactionDef` 经 `SkillCombatProfileValidator` 校验后投影为 `CombatSpellReactionDefinition`，只拥有精确待机状态、触发投送类别、反应技能、所需武器族、维持豁免公式及逐级命中/DC 曲线；`SkillContentRegistry` 还校验反应技能引用存在且是包含武器骰伤害的主动 unit skill。状态生命周期、触发时序、标准攻击结算、法术中断和 AI 收益归 CU-15/CU-16，不按具体弓手技能 id 分支。
- 远程武器受击反应内容边界：`CombatRangedWeaponReactionDef` 经 `SkillCombatProfileValidator` 校验后投影为 `CombatRangedWeaponReactionDefinition`，只拥有精确待机状态、允许武器族、反应伤害标签、独立攻击防御模式、逐级命中曲线、命中/未命中触发开关、单次层数消耗和暴击许可；法珠数量与持续时间继续由逐级唯一 active status effect 声明。主武器骰快照、触发时序、状态消费、正式反击、preview/HUD 与 AI 边际估值归 CU-15/CU-16，不按法师技能 id 分支。
...
- BattleUnitState save-modifier 边界：单位静态的三组 save tag 与 ability bonus map 归 plain C# `BattleUnitSaveModifierState`；passive refresh 原子清空四组件，race/subrace 与 effective trait 继续按原先顺序追加，敌人模板只初始化 tag。`BattleSaveResolver` 通过 readonly view/单项 bonus gateway 消费，AI effect signature 继续只覆盖原有三组 tag。damage resistance、effective trait 源事实以及 `BattleStatusEffectState` 上的动态 save tag/bonus 均不属于该 owner。strict codec、clone、canonical/detached 和 mutation exact 保留原四个 flat/stable key、相对顺序、逐组件 null/raw 及 malformed bonus map 回落为空的既有语义。
