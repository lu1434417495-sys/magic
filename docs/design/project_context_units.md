...
- Runtime payload 约束：runtime/save/window 边界确实需要 `Godot.Collections.Dictionary` / `Array` projection 时，应通过 `RuntimePayloadStore` / `RuntimePayloadList` / `GameRuntimePayloadEntry` / `RuntimePayloadCopy` 或立即 `RuntimeStateLifecycle.MarkValueGraphFinalizerless(...)` 登记为 runtime value graph；不要在 command handler、rollback、writeback、HUD adapter、snapshot builder 或 world context 中裸 `Duplicate(true)` 后长期保存 wrapper。
...
- 补充约束：`MeteorSwarmCastContext`、`MeteorSwarmTargetPlan`、`MeteorSwarmCommitResult`、`MeteorSwarmTargetOutcome` 这组 meteor DTO 也应保持 plain C# 数据载体；context/plan 的正式 skill 标识是 `skill_id`，不要为 runtime 内部 typed 传递再次挂回 `SkillDef` / `CombatCastVariantDef` / `RefCounted` / `GlobalClass`。
...
- 补充约束：`BattleUnitState` 的状态效果正式源是 `BattleStatusEffectCollection`，读写只走 `GetStatusEffect()` / `GetStatusEffectsTyped()` / `GetSortedStatusEffectIdsTyped()` / `SetStatusEffect()` / `EraseStatusEffect()`；`status_effects` 只能作为 `ToDictionary()` / `FromDictionary()` save/schema payload key 出现，不要恢复 public/live `GDictionary status_effects` 或从投影字典回扫运行时状态。`BattleStatusEffectState` 是 plain C# battle DTO，不参与 GodotObject 生命周期；其 runtime 语义字段必须保持显式 typed 字段，`BattleStatusEffectParams` 只在 content/save params 边界解析并剥离 residual save payload，不要让规则、AI、被动或特殊技能 resolver 直接读取 status `@params` 来决定效果。
...
- 文件：
  ...
  - `tools/*.py`
  - `tools/*.gd`
- 负责：headless 回归、contract 验证、fixture、截图/签名辅助；`TestHarness.Finish(...)` 是测试退出前的集中生命周期闸口，应在返回退出码前执行 `TestResourceOwnership.Drain()` 和 `GodotSharpCleanup.CollectPendingFinalizers()`，保持 idempotent，并确保 `SceneTree.Quit(_test.Finish(...))` 这类调用先 drain 再 quit；普通 test runner / finally 不应直接调用 `GodotSharpCleanup.CollectPendingFinalizers()` 或 `GodotObjectLifecycle.CollectPendingFinalizers()`；测试 helper 中的 `GodotSharpCleanup.DisposeGodotObject(GodotObject)` 只能 register/suppress runtime value graph，不承担 native dispose 语义，功能测试不要为了生命周期手工拆 Resource 子图；normal/test forced GC 只处理 known owners（borrowed/derived static suppress+retain、runtime state known graph、owned transient scope drain/test quarantine），不要在普通测试或 forced GC drain 里新增未知 wrapper 全局 suppress，也不要清空 process-lifetime borrowed/derived static content keep-alive store。
