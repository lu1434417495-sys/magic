...
Codex 应自主执行可验证切片，不需要用户逐个确认。只有涉及 save/schema 兼容、UI 暂缓范围、高风险大改、或必须触碰 unrelated dirty changes 时，才先说明风险并等待确认。

## 执行目标

整体目标是把项目运行时核心收敛为 C# typed 形态。

每个迁移 goal 都应尽量同时清理同一切片内的：
