...
- 实施锚点：替换当前 orchestrator 中 special profile 执行与预览两条路径的 `meteor_swarm` 硬编码分发；文档不记录易腐化的行号，但实现必须保证 execute / preview 走同一个 router 契约。
...
MVP 方法语义在二期启动前必须冻结为契约：`flush_buffers()` 只应用已验证的 wish delta 和 transaction buffer；`rollback_mvp_delta()` 只撤销本次 wish delta，不负责跨事件回滚、外部副作用撤销或 reaction history restore。二期只能通过新增二期专用方法或专用 transaction 类型扩展 reaction rollback，不能改写这些 MVP 方法的既有语义。
...
本轮只处理“新方案可行性最终确认（Kimi）”新增的确认与提醒，不重新打开上一轮已经裁决过的 residual 风险。

| Kimi 新意见 | 处理 | 已更新方案 / 不采纳原因 |
|---|---|---|
| 对 5 项 residual 风险处理质量的认可 | 不采纳新增修改 | 这是对上一轮裁决的确认，不是新的方案缺陷。`copy_spell` 顺序、sanctuary hook、router 注册面、AP 豁免和 MVP transaction 边界已经在正文与上一轮裁决表中覆盖。 |
| 二期前冻结 transaction 接口契约 | 采纳 | 已更新提交路径与二期进入条件：MVP 的 `flush_buffers()` 只应用已验证 wish delta 和 buffer，`rollback_mvp_delta()` 只撤销本次 wish delta；二期 reaction rollback 只能通过新增二期专用方法或专用 transaction 类型扩展，不改写 MVP 方法语义。未采纳在 MVP 中新增空 `pre_snapshot` / `rollback()` 占位接口。 |
| 编码前确认 `BattleUnitState` 序列化白名单 | 采纳 | 已核对当前 `BattleUnitState` 使用 `TO_DICT_FIELDS` 与 exact schema 校验，并在计数字段方案中明确 `per_battle_cast_counts` 必须保持战斗瞬态，不进入 `to_dict()` / `from_dict()` 长期白名单。 |
| 编码前确认 `meteor_swarm` special profile 分支位置 | 部分采纳 | 已在 router 小节补充实施锚点：替换 orchestrator 中 execute / preview 两条路径的 `meteor_swarm` 硬编码分发。未采纳记录具体行号，因为行号会随代码变化快速腐化。 |
| 编码前确认 `BattleStatusSemanticTable` 存在性与归属 | 采纳 | 已核对 `BattleStatusSemanticTable` 位于规则层，并把 sanctuary 方案改为扩展现有 semantic table 的最小 helper `find_pre_damage_prevention(unit, damage_context)`；不新建全局语义依赖，也不把 `wish_sanctuary` 写死进 `BattleDamageResolver`。 |
