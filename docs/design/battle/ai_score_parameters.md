...
- `heal_multiplier_percent < 100` 的状态使用专用治疗抑制事实，不计入 `estimated_status_count` 或 `estimated_control_count`。scorer 先确认目标阵营至少一个存活单位拥有当前等级可用的 typed heal effect，再以目标当前缺失生命加本次预期 HP 伤害为可恢复基数；已有减疗与本次状态按各自剩余 TU 分段取最小倍率，只把本次状态新增的拒疗量乘命中率，写入 `estimated_enemy_healing_denied` / `estimated_ally_healing_denied` 并复用 `heal_weight`。同一状态只替换自身时长，其他更强减疗不会被刷新；完整覆盖时边际值为零，部分覆盖只计算未覆盖时段。该链消费 canonical `BattleStatusContributionPreviewData`，不按技能或状态 id 分支。
