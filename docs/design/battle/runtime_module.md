...
`BattleMapPanel` 在进入首帧 reveal 等待前，已把同步借用的 `BattleState` 投影为 detached `BattleHudSnapshot` 与 immutable `BattleBoardRenderSnapshot`；pending payload 只保存这两类展示快照和选择参数。`BattleBoard2D` / `BattleBoardController` 不借用 live battle graph，单位局部刷新通过 `BattleBoardUnitUpdateSnapshot` 合并到当前 board snapshot，地形与 edge facts 不被重建。`HideBattle()` 和 `_ExitTree()` 清空 pending/hover 展示状态；离树时只使异步 reveal ticket 失效并归零本地 loading progress，不再向正在退出的 UI 树发布 loading signal。
