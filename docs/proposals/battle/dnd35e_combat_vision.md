...
当前实现边界以 [`project_context_units.md`](../../design/project_context_units.md) 为准；本文是长期规则蓝图，不代表当前代码已经完整采用 DND 3.5e 全量口径。
...
- 完整墙体（edge feature `wall`）阻断视线
- 友方单位提供 +4 软掩体（对远程攻击）
- 注：原设想的 `low_wall` 边特征已于 2026-08-16 从 `BattleEdgeFeatureKind` 移除，掩体不走边特征路线
