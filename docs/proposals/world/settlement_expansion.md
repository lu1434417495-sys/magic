...
当前实现边界以 [`project_context_units.md`](../../design/project_context_units.md) 为准；本文记录据点分层、服务执行器、窗口结构与后续扩展口径。
...
│   └── <shop_id>: Dictionary
│       ├── seed: int64            # 该商店最近一次独立生成库存所用的真随机 seed
│       ├── last_refresh_step: int # 该商店最近一次刷新的 world_step
│       └── current_inventory: Array
```

据点层不保存共享的商店 seed 或刷新步数镜像。每个商店独立刷新并只更新自己的子状态，避免一个商店的访问或刷新联动其他商店。
