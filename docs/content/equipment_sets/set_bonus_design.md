...
> 本文档保存早期 2/4/6 件套装机制、主题与数值，作为既有设计迁移来源；它不再是 100 套最终门槛结构，也不能直接在当前 C# `PartyEquipmentService` 中完整落地。
> 新的全 10 件、非统一门槛设计见 [既有 100 套传奇装备：套装效果重设计总则](existing_set_bonus_redesign.md) 及其三份逐套文档。
>
...
>
> **既有配置冻结**：套装编号、10 个成员、槽位、tag 与单品内容继续冻结；本文件记录的旧阈值是迁移输入，门槛数量、位置和套装奖励允许在新设计层重做。成员属性 owner 与旧效果去向审计见 [成员属性与旧效果迁移审计](existing_set_attribute_optimization.md)。
...
> **已被正式方案取代：** 下列 2/4 件内容仅保留为旧阈值来源，不是当前游戏配置。凤凰重生现有十个成员，正式阈值为 3/5/7/10；准确效果见 [装备套装系统当前实现](../../design/progression/equipment_sets.md)。
