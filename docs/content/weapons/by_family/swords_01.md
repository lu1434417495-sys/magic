...
- **properties**: `[finesse]`
- **base_price**: `80000`
- **attribute_modifiers**: `[心碎之刺: 暴击额外造成2D6 psychic伤害; 情感撕裂: 命中目标叠加60TU情感撕裂buff，最多3层，3层后目标攻击持有者时攻击检定-2; 心脏渴求: 目标HP≤30%时攻击检定+3; 心碎爆发: 每场战斗1次，对带3层情感撕裂的目标消耗层数造成4D6 psychic并震慑30TU]`
...
#### 特殊性质
- **心碎之刺**：暴击时，额外造成 2D6 psychic 伤害。
- **情感撕裂**：命中后目标获得 60TU 的"情感撕裂"buff，最多 3 层；每次命中刷新至 60TU。目标达到 3 层后，只有在攻击本次情感撕裂的来源持有者时，攻击检定承受 -2 惩罚。
- **心脏渴求**：当目标 HP 不超过其最大值的 30% 时，你对它的攻击检定获得 +3 加值（对濒死心脏的精准感知）
- **心碎爆发**：作为一个 action，可消耗带有 3 层情感撕裂的目标全部层数，造成 4D6 psychic 伤害并使其震慑（stunned）30TU；每场战斗限一次。
...
- **base_price**: `72000`
- **attribute_modifiers**: `[暮影步: 战斗时间达到70TU后授予装备技能，1AP，冷却40TU，闪现至3格内地面格; 昼夜平衡: 白天命中额外1点force，黄昏/夜间命中额外2点force并获得attack advantage; 暮光切割: 使用暮影步后的当前行动轮次内，首次命中额外2D6 psychic; 暮光守护: 黄昏/夜间攻击持有者的攻击检定-1，白天使用暮影步后当前行动轮次内攻击持有者的攻击检定+1]`
...
#### 特殊性质
- **暮影步**：装备授予主动技能 `weapon_sword_twilight_edge_twilight_step`。战斗时间达到 70TU 后可选，消耗 1AP，冷却 40TU，闪现至 3 格内地面格；使用后写入当前行动轮次状态。
- **昼夜平衡**：白天命中额外造成 1 点 `force` 伤害；黄昏/夜间命中额外造成 2 点 `force` 伤害，并获得真正的 attack advantage。
- **暮光切割**：使用暮影步后的当前行动轮次内，首次命中额外造成 2D6 `psychic` 伤害；通过 `once_scope=turn` 限制同一行动轮次只触发一次。
- **暮光守护**：黄昏/夜间，攻击持有者的攻击检定 -1；白天使用暮影步后，在当前行动轮次内攻击持有者的攻击检定 +1，用于表达防护破绽。
...
- **properties**: `[finesse, light]`
- **base_price**: `40000`
- **attribute_modifiers**: `[锈毒: 命中叠加1层180TU锈蚀最多5层; 护甲锈蚀: 达到5层且目标穿戴金属护甲时真实扣除该护甲24点耐久并施加护甲锈裂标记; 腐朽之刃: 对护甲锈裂目标伤害额外+2D6 acid; 锈粉风暴: 消耗目标5层锈蚀造成3D6 acid伤害]`
...
#### 特殊性质
- **锈毒**：每次命中敌人叠加 1 层 `rustoath_rust_corrosion`，持续 180TU，最多 5 层；V1 不做百分比移速衰减和切换目标后的特殊衰减。
- **护甲锈蚀**：当目标达到 5 层锈蚀且穿戴金属 body 护甲时，触发 `equipment_durability_damage`，真实扣除该护甲 24 点耐久，并施加 10000TU 的 `rustoath_armor_cracked` 标记；同一目标已有护甲锈裂标记时不重复扣耐久。
- **腐朽之刃**：对处于 `rustoath_armor_cracked` 的目标，每次命中额外造成 2D6 `acid` 伤害。
- **锈粉风暴**：装备授予主动技能 `weapon_sword_rustoath_rust_powder_storm`。当前 V1 为单体、1AP、需目标至少 5 层锈蚀、造成 3D6 `acid` 伤害并消耗目标锈蚀；暂不做原始文案中的锥形范围和金属护甲额外叠层。
