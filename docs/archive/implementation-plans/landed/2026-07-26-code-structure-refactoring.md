...
> 状态：Implemented / Closed（2026-07-26）。本文保留治理路线与落地记录，不代表当前实现真相。
...
| action clock / temporal remainders | timeline、casting、temporal status 与 AI exact snapshot | action progress/threshold/rate remainder 保持 TU 推进、静滞倍率和 flat snapshot；cast remainder 保持跨 cast carry 语义 | **action clock 为 P2-A/6 已完成；casting clock 为 P2-A/20 已完成** |
...
P2-A/20 将 runtime-only `cast_progress_rate_remainder` 的唯一存储迁入 plain C# `BattleUnitCastingClockState`。该 owner 只拥有 unit-level 施法进度倍率余数与跨 pending cast carry，不接管 `BattlePendingCastState` 的目标、成本、剩余进度或完成生命周期；`BattleTemporalStatusService` 继续决定每次 cast rate，并只经 typed gateway 请求 owner 计算整数 gain。零/负 base delta 或 rate 仍不消费余数，正速率仍把负 raw 余数按 `0` 参与本次计算；gameplay clone 深拷贝当前 carry，AI mutation exact 继续使用原 `cast_progress_rate_remainder` stable key并以该 key区分 owner missing。该字段仍不进入 69-key canonical codec、detached snapshot 或 save/schema，不增加兼容入口。
...
当前 P2-A 字段 owner 切片已经收敛到 `/20`；其后与本轮 temporal projection 直接相关的收口记录是：
...
3. [x] 外部合同决策完成（2026-07-26）：context-only `battle_party` / `enemy_units` 保持 canonical 69-key 单位合同，不携带 runtime-only projection，也不增加 extension/fallback。需要保留 runtime-only projection 的进程内调用方必须使用一次性 `BattleStartUnitRoster` typed handoff，并继续拒绝同阵营 typed/context 双来源。未来若出现跨进程或插件调用方的真实扩展需求，另立 schema proposal，并在获得兼容授权后评估 codec/version。
